...
const ENUMERATIONS = [
  ...
  { file: 'test/unit/live-handoff-render.test.js', extraction: "every appendTranscript call writing a plain agent turn, read out of server.js and lib/**, and the promote-handoff-message and show-delegation-divider executors read out of app.js", failLoudBy: 'count', where: "the site list is floored before it is used and the scanned-file count is floored beside it, so a pattern that stops matching fails rather than proving every turn delivered by finding none; each executor extraction asserts it was found before it is run; and the whole rule was measured by deleting the fix and watching it redden, three times, because two earlier shapes stayed green", anchor: "sanity: at least one source file writes agent turns" },
