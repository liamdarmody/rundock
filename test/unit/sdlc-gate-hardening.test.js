...
const ENUMERATIONS = [
  ...
  { file: 'test/unit/extension-install.test.js', extraction: "the onclick names inside extensionSectionHtml out of settings.js, and the extension reply dispatch case out of app.js", failLoudBy: 'count', where: 'the section match is asserted found before it is parsed, a specimen name the markup must carry is asserted in the parsed set, and the dispatch case is asserted matched, so a pattern that stops matching fails rather than comparing an empty set against the exports', anchor: 'sanity: the extension section markup was found at all' },
