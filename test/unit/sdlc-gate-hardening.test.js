...
const ENUMERATIONS = [
  ...
  { file: 'test/unit/map-foothold.test.js', extraction: "the view's link-source file kinds, out of files.js", failLoudBy: 'count', where: 'the declaration match is asserted found before it is parsed, and the parsed set is compared for equality in both directions against the index\'s own INDEXED_EXTENSIONS', anchor: "a kind missing here hides a group that should fill" },
  { file: 'test/unit/map-view.test.js', extraction: 'script tags and the rail out of index.html, and the search_index arm and the nav lists cut out of app.js', failLoudBy: 'count', where: 'every script the load-order test names is asserted found before its position is compared, and the cut arm refuses when the case or the arm is missing and is then run against both states', anchor: 'app.js no longer carries the search_index arm' },
  { file: 'test/unit/permissions.test.js', extraction: 'the discarding-redirect rule declared in the shared read-only module, and its absence from the hook and the client grader', failLoudBy: 'count', where: 'the shared declaration is asserted found before the other two files are asserted to carry none, so a pattern that stops matching fails rather than proving a copy gone by matching nothing anywhere, and the rule is then exercised for behaviour on the shapes that matter', anchor: "the shared module declares the rule" },
  { file: 'test/unit/room-context-delta.test.js', extraction: 'the mid-level parent prompt sites that must carry the catch-up, read out of engine.js', failLoudBy: 'count', where: "the collected sites are compared with an exact strictEqual against the three prompts that resume the same parent, so a site that stops matching fails rather than proving them all covered by finding none", anchor: "resume, complete and normal exit all resume the same parent" },
  { file: 'test/unit/handback-intent.test.js', extraction: 'the handoff markers named in the delegation contract, read out of prompt.js', failLoudBy: 'count', where: "the named list is floored at three before it is used, so a contract read that stops matching fails rather than proving every marker recognised by finding none", anchor: "the contract should name all three" },
  { file: 'test/unit/single-thinking-indicator.test.js', extraction: 'every site that creates the thinking indicator, read out of the client sources', failLoudBy: 'count', where: "the collected sites are compared with an exact strictEqual against the three that exist, so a site that stops matching fails rather than proving them all guarded by finding none. Two of the three were fixed before the third was found by counting them", anchor: "expected three creation sites" },
  { file: 'test/unit/advisory-outside-paths.test.js', extraction: 'every mention of the advisory field in the router, read out of http-router.js', failLoudBy: 'count', where: "the relay itself is asserted present before the mentions are walked, so a read that stops matching fails rather than proving no mention sits on a decision line by finding no mentions at all", anchor: "the field must be relayed, or the card cannot show it" },
  ...
  { file: 'test/unit/standing-allows-client.test.js', extraction: "the workspace-open send sequence out of app.js, and the revoke handler attributes out of the rendered settings rows", failLoudBy: 'count', where: "the onWorkspaceReady match is asserted found before the body is sliced, since a slice from indexOf(-1) matches nothing and would pass by having nothing to read; the collected sends are then floored and checked for membership rather than searched for as a substring, and the rendered handlers are floored at two before each is required to contain an index and nothing else", anchor: "sanity: the open sequence was read and has sends" },
  { file: 'test/unit/live-handoff-render.test.js', extraction: "every `continue` in the delegation engine's stdout handler, which is what suppresses an end-of-message envelope, and the promote-handoff-message and show-delegation-divider executors read out of app.js", failLoudBy: 'count', where: "the suppressing blocks are floored before they are judged, so a walk that stops matching fails rather than proving every branch delivers by finding none; each executor extraction asserts it was found before it is run; and the rule was measured by deleting each delivery in turn and watching it redden, after three earlier shapes stayed green", anchor: "sanity: the interception still has branches that continue" },
  ...
  { file: 'test/unit/permissions-view.test.js', extraction: "every .css file under public/styles, and the settings sidebar order out of index.html", failLoudBy: 'count', where: 'the class-coverage guard asserts the pane rendered more than five classes before it looks for orphans, so a render that silently produces nothing fails there rather than reporting an empty orphan list as a pass; the sidebar walk asserts the section was found before comparing its position', anchor: 'the pane rendered something to check' },
  { file: 'test/unit/no-live-server-in-tests.test.js', extraction: "every env object in the test tree that arms the real permission hook, read out of the test sources", failLoudBy: 'count', where: 'the armed sites are floored at four before the offender list is judged, so a pattern that stops matching fails rather than reporting no offenders and passing forever, which is the precise failure this guard exists to prevent elsewhere', anchor: "ENV_OBJECT has stopped matching" },
  ...
  { file: 'test/unit/pins-view.test.js', extraction: 'the rail, the sidebar, the editor pane and the pins pane out of index.html', failLoudBy: 'count', where: 'each match is asserted found before the shell is built, so a page that stops carrying one of them fails by name rather than yielding an empty shell that passes', anchor: 'index.html no longer carries' },
  ...
  { file: 'test/unit/extension-host.test.js', extraction: "the contract document message table and the shape fields it declares", failLoudBy: 'count', where: 'set equality both ways against the imported message tables, and per-message field equality', anchor: 'EXTENSION_MESSAGES' },
  { file: 'test/unit/renderer-registry.test.js', extraction: 'registry claims and refusals read back from the registry itself', failLoudBy: 'count', where: 'exact counts on refusals, notes and mounts', anchor: "r.refusals().length, 1" },
  ...
  { file: 'test/unit/workspace-boundary.test.js', extraction: 'the path names the boundary passage cites, out of the marked passage in ARCHITECTURE.md', failLoudBy: 'imports', where: 'set equality in both directions against the registry the hook exports, so a name in the passage with no registry entry and a registry entry the passage never names each fail; the passage markers are asserted present, so a deleted passage refuses rather than matching nothing', anchor: 'boundary-registry-start' },
