...
const ENUMERATIONS = [
  ...
  { file: 'test/unit/permissions.test.js', extraction: 'the discarding-redirect rule declared in the hook and in the client grader', failLoudBy: 'count', where: 'both matches asserted found before they are compared, so a pattern that stops matching fails rather than comparing two nulls, and the two are then exercised for behaviour on the shapes that matter', anchor: "the two copies are the same rule" },
  ...
  { file: 'test/unit/engine-live-delivery.test.js', extraction: "the engine's own statement that a delegation reached nobody, and the guard narrowing it to calls that named no subagent_type, read out of engine.js", failLoudBy: 'count', where: "each site is asserted found with indexOf before the region around it is examined, so a line that is renamed or removed fails here rather than leaving the claim unchecked by matching nothing", anchor: "the engine says out loud that no delegation happened" },
