...
const MUTATIONS = [
  ...
  [MODEL, 'the approval comes through the shared decide module, not a local copy',
    ...
    "      send: { type: 'confirm_package_install', token, requestId, approval: { schema: state.plan.schema, source: state.plan.source, manifest: state.plan.manifest, items: state.plan.items.map((item) => ({ ...item, decision: decisionsFor(state)[item.id] })) } },"],
