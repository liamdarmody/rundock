...
const MUTATIONS = [
  ...
  [MODEL, 'the approval comes through the shared decide module, not a local copy',
    ...
    "      send: { type: 'apply_package_import', requestId, sourcePath: state.sourcePath, approval: { schema: state.plan.schema, source: state.plan.source, manifest: state.plan.manifest, items: state.plan.items.map((item) => ({ ...item, decision: decisionsFor(state)[item.id] })) } },"],
