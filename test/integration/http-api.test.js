...
describe('path traversal guards', () => {
  ...
  test('/views/team.js is served as javascript (the team view module)', async () => {
    const res = await get('/views/team.js');
    assert.strictEqual(res.status, 200);
    assert.match(res.headers.get('content-type') || '', /javascript/);
    assert.ok(res.body.includes('RundockTeamView'), 'team view module content served');
  });

  test('/views/profile.js is served as javascript (the profile view module)', async () => {
    const res = await get('/views/profile.js');
    assert.strictEqual(res.status, 200);
    assert.match(res.headers.get('content-type') || '', /javascript/);
    assert.ok(res.body.includes('RundockProfileView'), 'profile view module content served');
  });

  test('/views/files.js is served as javascript (the files view module)', async () => {
    const res = await get('/views/files.js');
    assert.strictEqual(res.status, 200);
    assert.match(res.headers.get('content-type') || '', /javascript/);
    assert.ok(res.body.includes('RundockFilesView'), 'files view module content served');
  });

  test('/views/conversations.js is served as javascript (the conversations view module)', async () => {
    const res = await get('/views/conversations.js');
    assert.strictEqual(res.status, 200);
    assert.match(res.headers.get('content-type') || '', /javascript/);
    assert.ok(res.body.includes('RundockConversationsView'), 'conversations view module content served');
  });

  test('/views/chat.js is served as javascript (the chat view module)', async () => {
    const res = await get('/views/chat.js');
    assert.strictEqual(res.status, 200);
    assert.match(res.headers.get('content-type') || '', /javascript/);
  ...
  test('/views/find.js is served as javascript (the in-view find bar module)', async () => {
    const res = await get('/views/find.js');
    assert.strictEqual(res.status, 200);
    assert.match(res.headers.get('content-type') || '', /javascript/);
    assert.ok(res.body.includes('RundockFindView'), 'find view module content served');
  });

  test('/chat-markup.js is served as javascript (the shared chat markup module)', async () => {
    const res = await get('/chat-markup.js');
    assert.strictEqual(res.status, 200);
    assert.match(res.headers.get('content-type') || '', /javascript/);
    assert.ok(res.body.includes('RundockChatMarkup'), 'chat markup module content served');
  });
  ...
  test('connectorsLoad reads the workspace .mcp.json through /api/file and renders its servers', async () => {
    ...
    assert.match(page, /notion/, 'the tab read the real file and rendered what it contains');
    assert.doesNotMatch(page, /No connectors configured/, 'not the empty state, because the file has servers');
    dom.window.close();
  });
