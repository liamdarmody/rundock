...
- **Delegation loop circuit breaker:** Added a per-conversation counter that tracks consecutive agent auto-resume events with no intervening user message. After 3 consecutive agent handoffs (configurable via `MAX_CONSECUTIVE_AGENT_RESUMES`), the system auto-pauses and presents a summary to the user instead of continuing the loop. This makes the infinite Cos-Doc re-delegation loop observed in real conversations impossible by construction. The counter resets on each user message, so legitimate two-hop routing (specialist A returns out-of-scope, orchestrator routes to specialist B) works without triggering the breaker.
