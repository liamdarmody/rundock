...
- **Editing a note no longer reformats or corrupts its markdown:** the editor's save path now preserves task-list checkboxes (`- [ ]`/`- [x]`, previously escaped to `- \[ \]`), emphasis and list markers, thematic breaks, escaped punctuation, and blank-line spacing, and headings four to six and `$inline$`/`$$block$$` LaTeX survive byte-exact (previously deeper headings flattened to plain text and maths had its backslashes doubled on every save). A note round-trips through the editor byte-for-byte.
