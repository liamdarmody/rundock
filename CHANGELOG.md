...
- **Your MCP API keys can now live somewhere the rest of the team never sees:** the file listing your MCP servers, `.mcp.json`, sits in the workspace folder, and most MCP servers take an API key as a plain value written inside it. The workspace folder is the thing you share, so that key went wherever the folder went. Git was the worst of it. Rundock excludes its own state folder from git when it sets a workspace up, and nothing has ever done the same for `.mcp.json`, so on a team sharing a workspace by repository the key reached every clone and stayed in the history from the first commit, where deleting the file afterwards does not take it out again. The only protection was a line in the team guide telling you to keep secret-bearing servers out of the shared file, which is advice rather than anything that stops it happening, and which assumes you already know which of your servers take a key. There is now a place to put the values instead: `.rundock/mcp-secrets.json`, listed by server name, inside the folder Rundock already excludes. `.mcp.json` keeps the parts the team needs to agree on, which servers exist and what runs them, and you take the key out of it. The value is put back only for the moment an agent starts, and only into the server that needs it. A workspace member who has not set their own key gets a server that fails to sign in, which is loud, rather than one quietly running on somebody else's credential. **Nothing changes for a workspace you do not touch.** A key still written literally in `.mcp.json` keeps working exactly as it did, so upgrading breaks nothing and moving a key is yours to do when you want to; Rundock does not create the new file for you or prompt you about it yet. Two limits worth knowing: the per-user file can only supply values, never change which servers exist or what command runs one, so a file on one machine cannot quietly point a server somewhere else; and it can only fill in servers `.mcp.json` already lists, so a server nobody else has still belongs in the shared file. `ARCHITECTURE.md` describes the mechanism, including why the values are not passed through the environment instead.
- **You can now read what a routine actually did:** every run since routines started recording has left a record on disk naming the files it changed, and until now the only way to read one was to open the file yourself. A routine that has run carries a "View last run" link on its row, which opens that run and shows how it ended, how long it took, and every file it created or edited, each named with the full path it was recorded at. **The screen never tells you a routine changed nothing unless it knows that.** A run that changed no files says so plainly; a run whose changes could not be read says that instead, and says why in plain words, because those are different facts and only one of them means the routine is working normally. The other means something went wrong with the watching, which is what you would want to know before deciding whether to trust an unattended run or undo it. A run that Rundock was closed in the middle of is described as one whose ending never ran rather than as one that failed, because nothing recorded how it turned out and it may well have finished the work a moment before Rundock closed. Changed files cannot be opened from this screen yet: a run records each file as a full path on the computer that ran it, which is not always somewhere the workspace open now can reach, so the screen says that rather than offering a link that goes nowhere.

### Fixed

- **Reviewing a page that contains a code block no longer rewrites the page:** commenting on a document with a fenced block in it, which is what any spec, tutorial or readme with a snippet in it looks like, cut the block short at the first comment. Everything after that point fell out of the block, and because it was no longer code it was written back to disk as prose, so the asterisks, backticks and link brackets in it collected backslashes. The document was changed on save rather than only on screen, so the file on disk carried words nobody had written, and the only way back was to write the page again by hand. Two separate causes were behind it. A comment is a mark placed among the words, and a code block holds plain text and nothing else, so there was nowhere in the block for one to go and the editor made room by ending the block early. A comment anchored inside a code block is now refused instead, and the sidebar says why, keeps what you typed, and leaves the file untouched so you can move the selection somewhere the comment can live. Separately, a code block was always written back with three backticks whatever the file had used, which quietly broke any page that shows the fence itself: a block opened with four backticks came back with three, and the next time the file was opened it ended at the example inside it, taking the rest of the document out of the block with it. A block is now written back with the fence the file was written with, tildes and four backticks included, along with everything on the opening line, and the fence is made longer when the block's own contents would otherwise end it.
- **Switching workspace no longer says a routine that is still running was cut short:** if a routine was running when you switched to another workspace, the routines panel and the agent's profile immediately reported that run as interrupted, in both workspaces, for as long as the run went on. Nothing was actually wrong with the run: it was still going, it finished normally, and the display corrected itself when it did. The cause was that Rundock rebuilds its record of what has run from the workspace file on every switch, and a file on disk cannot tell a run that was still going from one left behind by a Rundock that was closed mid-run. It can now tell them apart, because the running copy knows which runs it started and has not yet finished. A run left behind by a Rundock that really did close is still reported as cut short, which is the case that reading needed to cover.
- **Upgrading no longer starts routines you never asked it to run:** if your agent files already had routines in them, written by hand before Rundock could run one, the first time Rundock ran over that workspace every one of them went live at once. For anyone who had been driving those jobs with their own scheduler, that meant the work ran twice: the morning briefing went out, and then went out again. Rundock now treats a routine that never said whether it may run as one waiting to be turned on. It is still listed, it keeps every word of its file, and it does nothing until you say so. Its row offers to turn it on and tells you what that starts, in as many words, because the thing you need to know before pressing is that Rundock will begin running it too. It also tells you when the first run lands, and only where it can say so truthfully: a routine whose scheduled time has already gone runs within the minute of being turned on, which is worth knowing when the job you are turning on already ran this morning under your own scheduler, while one whose time is still ahead is told the day and time instead. **A routine that already says whether it may run is left exactly as it is,** whether that is on or off, so this never switches off something you had deliberately turned on. `yes` and `on` count as yes and `no` and `off` count as no, the same as `true` and `false`, because those are all booleans in the format these files are written in. Only a value that is not a boolean at all leaves a routine waiting, and its row offers to turn it on. Routines you make in the routine editor are unaffected and start running the moment you save them, with nothing extra to press, because that is the one path that writes the answer for you. A routine that reaches an agent file any other way, typed into the frontmatter by hand or written there by an agent, has not said whether it may run and is listed as waiting to be turned on until its block says `enabled: true`. This also holds on a workspace Rundock cannot write to, such as one checked out read-only: it still will not start a routine there rather than starting it because it could not record the decision. A routine waiting to be turned on is also no longer told it missed its slot because Rundock was closed: it was not closed, the routine was simply not running yet, and a row that named the wrong reason would send you looking for a problem that was not there. Nothing is recorded against it while it waits either, so turning it on does not bring back a list of slots it was never going to serve.
- **Routines now say which workspace they belong to, and a second window is no longer left showing runs that will never happen:** Rundock runs one scheduler and it serves the workspace it has open. If you keep routines in more than one workspace, only the open one's routines are running and the rest are dormant, and nothing anywhere said so: the list looked complete, every row showed a next run, and a briefing you had set up in another workspace simply never arrived. It was worse with Rundock reachable from more than one device, which is what the always-on setup is for. Switching workspace on your laptop told the laptop and nothing else, so the browser on your phone went on showing the old workspace with a next run against every routine while the scheduler had moved and none of them was going to fire. The Routines list now names the workspace the routines it is showing were read from, and says whether Rundock is still serving it. When Rundock has moved elsewhere, every row on that screen drops its next run, says where Rundock went, and is offered no Turn on control, because pressing it would start nothing. Windows that did not ask to move are not moved: you stay on the workspace you were reading, and it tells you the truth about it. The scheduling step of the routine editor says the same thing while you are setting a routine up, along with the part that softens it: coming back to that workspace the same day serves a slot that went by. **This is deliberate rather than a limitation waiting to be lifted.** Running routines out of a workspace you have not opened would mean Rundock starting agents under that workspace's permissions, against files you are not looking at, which is the one thing its folder boundary exists to prevent. The reasoning, and what would change it, is written down in `docs/ROUTINES.md`.
- **A routine Rundock cannot read the schedule of now says so on its row:** Rundock understands two ways of writing a schedule, `every day at 07:00` and `every Monday at 07:00`, and anything else, a cron line most commonly, was accepted into the list and then skipped forever with no error, no warning and nothing on screen. The routine sat there looking exactly like a routine that was about to run. Its row now says Rundock cannot read the schedule and that the routine will not run, and names both forms that work so you can copy one, and it no longer shows a next run it was never going to keep. A row like that is not offered a Turn on control either, because turning it on would start nothing while the schedule is still unreadable. A routine that is simply not due yet is unchanged and still shows when it runs next, so the two are told apart at a glance. Upgrading does not rewrite these schedules or remove them: whatever you wrote stays exactly as you wrote it, and changing it is yours to do.
- **The sidebar icons now agree with what you are looking at:** the strip of icons down the left is the only thing telling you which part of Rundock you are in, and on several screens it was pointing somewhere else. Opening a routine for editing lit Team, on a screen entirely about routines. Clicking a skill lit nothing at all, so whichever icon you arrived from stayed lit over the skill you were reading. Opening a skill's own file left Skills lit over the file, with the list beside it still offering skills you could no longer see. Every screen in the app was gone through rather than the three that had been noticed, and the icon is now decided by the screen itself, so a new screen cannot arrive without one.
- **Switching to another workspace no longer loses the New conversation button:** if you switched workspace while looking at anything other than Conversations, you landed in Conversations with the button for starting one missing until you clicked another icon and came back. The reset that runs on a switch was a copy of the one the rest of the app uses, written before that button existed, and it never learned about it.
- **The workspace picker no longer appears with the sidebar still over it:** if Rundock lost track of your workspace while you were working, because the folder was moved or removed or a switch failed, the picker could open with the icon strip and the sidebar still on screen and an icon still lit, over a screen that means you have no workspace at all.
- **Text an agent brings back can no longer run code in the app:** anything an agent wrote into a conversation or a note was turned into the page as it was written, so a document could carry an instruction to the app rather than words to read. That mattered because most of what an agent produces is not the agent's own writing: it is a file from the workspace, a page it fetched, or an agent or skill file installed from somewhere else, and routines make agents produce it on a schedule with nobody watching. Six ways in were found and closed, five of them needing no interaction from you at all. Markup written into a document now shows as the characters it was written with, a callout heading reads as a label rather than as instructions, a note reference and a link to a file in your workspace carry their target as data instead of as code, and a link that names a destination the app will not follow now shows its text without becoming clickable. Nothing about ordinary markdown changed: code blocks, tables, task lists in both their forms, callouts, tags, note references, links, images and syntax highlighting all render exactly as before, which the suite checks character by character against a recorded copy of the previous output.
...
- **The always-on setup guide now describes the double run correctly, and says what to do about it:** the guide recommends keeping Rundock up on a small server with your workspace synced, and it warned that a second Rundock left running on the same workspace fires every routine again. What it got wrong was why. It said the outcome depended on whether your sync tool copied Rundock's state folder, which would mean the right tool could avoid it. It cannot: each Rundock reads the last-run guard into memory when it starts and when you switch workspace, and never re-reads that file while it is up, so **two instances that are both already running fire independently** whatever the file says and however fast it arrived. (An instance you start, or switch, after a synced file has landed does read it, and is held for that slot as usual. The always-on case is the one where both are already up.) The guide now says that, names the remedy rather than only the hazard, which is to let one machine run the routines and reach it from your other devices through the browser, and adds the half that was missing entirely: an always-on machine runs the routines of the workspace it has open and none of the routines in your other workspaces.
- **Opening a skill's own file now moves you to Files:** the file opens in the editor, which is the Files part of the app, so the icon and the sidebar go there with it. Back still returns you to the skill. Previously the icon stayed on Skills while the editor filled the screen.
...
- **A command that reaches outside your workspace now asks you first, and says that is what it is asking:** a shell command could write to your home directory, or anywhere else on the machine, with no approval card at all in a Code-mode workspace, while an ordinary file edit to the very same place asked first. The two cards ask different questions. Approving a command you cannot fully read is not the same as agreeing to let an agent out of your folder, and only the second is what Rundock promises. Rundock now reads each command for places outside your workspace and raises the boundary card naming them, listing every one in the order the command names them. Commands staying inside your workspace are untouched and still run without interruption. Windows commands are covered too, including drive letters, network shares, backslash paths and the home shorthands PowerShell uses, since those run through a separate tool that was exempt entirely. **This check is a best effort over the ways a path is normally written, and it is worth knowing where it stops.** Reading a command without running it cannot always tell where it will end up: a command can build a destination while it runs, and there are spellings the check does not recognise, such as another person's home folder written as `~someone/file` or a path glued onto a short option as `-C/tmp`. Those raise no card. Two more places it deliberately stays quiet: paths under the system program folders (`/bin`, `/sbin`, `/usr/bin`, `/usr/sbin`, `/usr/local/bin`, `/opt/homebrew/bin`) and device paths such as `/dev/null`, because they appear in a large share of ordinary commands, and carding those would put a boundary card on everyday work and teach you to click through the one that matters. And a folder you marked "always allow" still silences file access to that folder but never silences a command, because approving a command approves everything in it, not only the part touching that folder. One thing got noisier: in a knowledge workspace a read-only command naming a path outside your workspace, such as reading a file in another folder, is now a boundary card rather than an ordinary low-risk one, so it prompts where it used to pass and is no longer covered by "Always allow" for the session.
- **On macOS the operating system now refuses writes outside your workspace, and that one does not depend on reading anything:** the runtime Rundock starts is given the list of folders a command may write to, so a write anywhere else fails at the operating system level however the destination was spelled or worked out. Writable are your workspace, the npm cache, and the runtime's own default locations, which it keeps and Rundock does not remove: measured, those are its debug log folder and the temporary directory. Everything else in your home folder was measured as refused, including your documents, your SSH keys, your shell startup files and the other package caches, so a package manager other than npm will fail on its own cache, and that failure can arrive looking like a file-ownership complaint rather than a refused write. Moving, renaming or copying a workspace keeps working, including to another computer or another user account: Rundock brings the writable folders with it. **What this half does not do: it governs writes, not reads.** A command can still read anything on the machine, and nothing anywhere stops a read the command works out while it runs. It also does not guarantee you see a card: the refusal is certain, but whether the agent then asks you to override it is the agent's choice, and it may simply report the failure. This half is macOS only, and it further depends on your installed Claude Code supporting the setting, and on your workspace either having no sandbox settings of its own or only the ones Rundock wrote, since a block you edited yourself is never touched. **On Windows and Linux, and on macOS whenever any of those conditions fails, the approval card is the whole boundary,** with the limits described above. Rundock would rather say that than imply a guarantee it does not have. Linux is left out deliberately, because Rundock has not measured it there and does not ship configuration it has not run. Network access is unchanged.
