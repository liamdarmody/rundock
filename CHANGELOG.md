...
- **Rundock's own folder is graded by what a request would do there, not by treating the whole thing as off-limits:** an agent reading its own transcripts, cache, or a global skill file no longer raises a card, in either mode, because that is no different from what the same agent already reads in a terminal. Writing to something that would run again later, an agent, a skill, a plugin, a command, a hook, or the global settings file, still asks, and the request says plainly that a write there carries into every later session and every other workspace, including an unattended routine run. Your Claude account credentials are the one exception with no exception of their own: any request that touches them, reading or writing, always asks, in every mode, and nothing you have approved before can answer for it.
