...
- **Rundock stops slowing down the longer it stays open:** an agent process was kept for every conversation you touched, for as long as the app was running, and each one held its own set of tool servers. Open a dozen conversations across a working day and a dozen agent process trees were still resident that evening. Conversations idle for a while now release their processes, along with any agent parked behind a handoff, and resume with their history intact the next time you use them. Archiving a conversation previously reclaimed nothing, because archiving only set a status flag.
