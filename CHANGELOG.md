...
- **Upgrading no longer starts routines you never asked it to run:** hand-written routines predating Rundock's scheduler used to go live the moment Rundock first ran over that workspace, silently duplicating work already handled by your own scheduler. A routine that never said whether it may run now waits until you turn it on, and tells you what pressing that button will start.
