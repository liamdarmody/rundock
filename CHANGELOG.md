...
You can now add agents, skills and file renderers to a workspace from one GitHub link, keep the files you live in one click away, and see how everything in the workspace links together.

### Added

- **Install a package from one GitHub link:** paste a repository link into Settings, Packages, and Rundock fetches it and reads what it holds. Agents and skills become ordinary files in your workspace, under `.claude/agents/` and `.claude/skills/`, exactly as if you had written them, so any tool that reads that layout can use them and nothing about them is special to Rundock. A repository never built for Rundock, a plain folder of Claude Code agents and skills, is offered as exactly that: Rundock says it isn't a Rundock package, counts the agents and skills it found, and lands nothing until you choose "Add to my team". The offer says plainly that agents and skills are not sandboxed: once added they act with the same access your own agents have, and nothing runs until you add them. Cancel writes nothing. A fetch that takes more than a few seconds says it is still reading rather than leaving you to wonder.
- **A name collision is a decision, and skip is the answer that always works:** when a package carries an agent or skill with the same name as one you already have, the review lists every colliding item, each opened as skip, and overwrite is a deliberate switch you set per item. A file whose incoming bytes already match yours is marked identical and is never counted as an overwrite, since nothing would be touched. The confirm button counts what it is about to do, adds and overwrites and skips, from the server's own check of your decisions, and says "nothing added" when nothing will be written. If the workspace changes under a review, the review goes stale and returns you to a fresh plan rather than applying decisions made against files that have moved. Losing the connection mid-review keeps every decision you had made. Applying is one transaction: a failure part way through leaves the workspace exactly as it was.
- **Every install leaves a receipt:** a record of what arrived, from which link at which reference, and the decision that governed each item, kept in the workspace beside the files it describes. A receipt is written whenever you confirmed decisions, including when you skipped every item, because the decision itself is worth remembering. Installing the same package again with nothing to change leaves no receipt, since nothing was decided.
- **Extensions: a package can render a kind of file Rundock does not render itself:** an extension ships a view, and once installed, opening a file it claims shows that view in place of the raw text. The view runs in a frame with an opaque origin, and the frame is told about one thing: the opened file's path, its text as a read-only copy, and the theme the page is showing, so the view can match it. In return the view can ask Rundock for exactly four things: to say it is ready, to set its own height, to report that it has failed, or to open a workspace file the way a wikilink would. It cannot reach Rundock's page, your conversations, your permissions, the network, or the disk, and each of those is enforced rather than promised. A file too large to hand to a frame, above two million characters, is rendered plainly with the limit named. A view that throws, reports an error, or never becomes ready is torn down and the plain rendering returns with the failure named beside it, so an extension can never break the file it was mounted on. The full contract is in `docs/EXTENSION-HOST.md`.
- **An extension is installed at a pinned reference, through a trust step built from what Rundock actually read:** an extension link needs a tag or commit alongside it; a moving name such as a branch is refused before anything is fetched, because what you approved must be what you get. Before installing, Rundock shows you the very file list it derived from the fetched bytes, held on the server so the bytes described are the bytes installed, and every safety sentence on the card is computed from a fact of the host, the sandbox, the frame's network policy, the closed message table, and the read-only fields the frame is told about: one sentence per fact, and no sentence without one. Declining installs nothing and removes what was fetched. A repository carrying both an extension and a set of agents and skills installs the extension first and says so, then offers the agents and skills as a second step; nothing about them lands until you answer that too.
- **The file tree lists the files an installed extension can open:** a `.csv` appears beside the built-in kinds the moment an extension claiming it is installed and enabled, at any depth of the tree. Disable or uninstall the extension and the tree stops listing those files. An extension updated or uninstalled while its view is on screen is swapped or torn down at once, with the plain rendering drawn under a stated reason, rather than lingering until the next file open.
- **Update and uninstall trust the record, never a link you retype:** the installed record holds the source link and the pinned reference. An update check reads them from there, so nothing asks you for the link again, and reports only references it can prove come after the pin in real version order, never the listing's own. A newer tag says "Update available", and installing it goes back through the same trust step, since new bytes deserve a new decision. A pin that cannot be compared with the repository's tags, a commit hash for instance, says so rather than claiming to be up to date. Uninstall removes what the install created, the extension's directory and its record, and names what it left alone; it refuses to act on a record whose location does not resolve inside the workspace's own extensions folder, so a tampered record in a shared or synced workspace cannot direct a delete elsewhere, and the stored link is checked again before any network call is made from it.
- **Packages has its place in Settings:** the Packages section sits second in the settings navigation, directly after Workspace. It carries the install field, and states beside it, before you paste a link, that Rundock does not review packages: reading what a package contains is yours to do, and the trust step and the receipt exist to make that possible.
- **Installed extensions are yours to manage:** each installed extension is listed with its name, version, source repository and pinned reference, and its state: enabled, disabled, update available, install failed, or installed but broken, each with the reason where there is one. Disable keeps the files and stops the extension claiming anything, so its files return to the plain rendering at once; Enable brings it back. Check for update and Update sit on the row, and Uninstall rests as a quiet link that turns to the danger colour on hover, with the filled destructive button appearing only inside its confirmation: nothing is removed before you confirm, and the reply names what stayed.
- **Recently added shows what came in:** the five most recent receipts, each with the package's name, its source repository, the date, and a count of what arrived by kind, with a See all row when there are more. Each item in a receipt links to the live thing it installed: an agent opens Team on that agent, a skill opens Skills on that skill, a file opens Files on that path.
- **Two example packages to start from:** [`liamdarmody/lean-agent-team`](https://github.com/liamdarmody/lean-agent-team) is three agents, two skills and the operating rules that tie them together, with no manifest, so it arrives through the "isn't a Rundock package" offer and shows how an ordinary folder of Claude Code agents becomes a team. [`liamdarmody/rundock-csv-extension`](https://github.com/liamdarmody/rundock-csv-extension) is an extension that renders a `.csv` as a table, header row and all, in one manifest and one script with no build step, so it can be read in one sitting before you install it. Pin `v1.0.0` for the team and `v1.0.1` for the extension.
- **Pin the files you live in:** a Pins entry sits in the rail directly below Files. Pin a file from the editor header, next to Preview and Edit, or by right-clicking it in the file tree; the Pins list shows your pinned files flat, in the order you pinned them, each with its folder underneath and an unpin control on hover. Pins are yours and this machine's: they are stored in your home folder, keyed to the workspace, never written into the workspace itself, so a shared or synced workspace does not carry one person's list to everyone else. They survive restarting Rundock. A pinned file that has since been deleted or renamed stays in the list, struck through with the reason and an explicit Remove, because silently losing a pin would read as Rundock forgetting it. Opening a file from Pins keeps Pins lit; arriving at Pins with pins present opens the first one. With nothing pinned, the pane says what pinning is for and names both ways to do it.
- **Map: see your workspace as a graph:** a Map entry sits last in the rail, a lens over what the other entries contain. It draws every file as a node and every resolved link between files as an edge, on one canvas filling the pane, with the sidebar out of the way. Files that link to each other cluster together; files nothing links to sit on a rim around the outside rather than being hidden or crowding the middle. Recency is shown as light: five steps from warm, for a file changed recently, to dim, for one untouched in a long while. A node's size grows with how many links it has, and small nodes come into view as you zoom in. Type a keyword to narrow the readout to matching files, hover a node for its name, and click one to open it: it lands in the editor on the same file a link inside a document would open. Pan by dragging or two-finger scroll; zoom with a pinch or Ctrl and the wheel, anchored on the cursor. The map follows the theme. In the first moments after opening a workspace the link index is still filling, and the map says so rather than drawing a half-truth, then rebuilds itself once the index is ready; the connections list under a file says "Links are still being indexed" in that window instead of claiming there are none.
- **Run a routine now, without moving its schedule:** every routine's row carries a Run control first in its actions, present whether the routine is paused, switched off, or waiting on a changed plan, because those govern whether it runs unattended and a run you press is attended. A manual run starts the same way a scheduled one does and is recorded, watched, and stoppable exactly like one, but it leaves the schedule untouched: the next scheduled time is the one it was before, and an afternoon test run never quietly satisfies the evening's slot. The row's on-time, caught-up and missed verdicts read scheduled runs only, so a test run that fails does not turn the row red; its outcome is on its own record. That record says the run was manual, the run's page names it on the line that says when it started, and while it is going the row reads "Running now (started manually)". The only reasons a press is refused are the ones that make a run impossible, an unsupported target, nothing to send, or a run already going, and each is named on the row. Run is disabled while a run is in flight, so a second press cannot start a second run.

### Fixed

- **A reply from the server is matched to the question that asked it:** the Packages flow routed replies by their kind alone, so an error from an update check or an uninstall arriving while an install was in flight was read as that install's answer, and a decision check answering after you had confirmed, or after a newer decision had asked its own question, could land on the wrong one. Every reply now carries which request it answers, and one that names a different request changes nothing. A Packages flow shown in one window now also returns to its start when another window moves the server to a different workspace, rather than offering a confirm the server would refuse.
- **A file that is already identical is not an overwrite:** the confirm button counted a colliding item you had set to overwrite as a destructive write even when the incoming bytes already matched the file in your workspace and nothing would be touched, so you were asked to accept destroying something that was never at risk. Counts now come from the same check that decides what is written, and an identical item is marked as already present.
- **Skipping everything is remembered:** confirming a review with every item skipped wrote nothing to the workspace and, because of that, wrote no receipt either, so the decisions you had just made vanished. A receipt is now written whenever you confirmed decisions, whatever they were; only a replay that decides nothing leaves none.
- **A commit pin no longer claims to be up to date:** an extension pinned to a commit hash always reported "up to date", because the update check orders references by version and a hash has none to order, so nothing could ever count as newer. That was a claim the code could not make. The check now says the pin cannot be compared with the repository's tags and suggests pinning a tag to have updates checked.
- **Danger text on a light card meets the contrast bar:** the danger colour used as text on a light-theme card measured 2.98 to 1, under the project's own 3 to 1 minimum, and as a fill it sat so close in hue to the accent that a filled destructive button read as another accent button, a distinction the most common forms of colour blindness lose entirely. The token is now split (see Changed), the contrast of every pairing is computed from the tokens on every test run rather than asserted once, and the light override is proven on a real page in both themes.
...
- **Destructive controls take one of two danger tokens, by job:** `--danger` is the fill and the edge, the same in both themes, thirteen points darker than before so it separates from the accent by lightness (1.76 to 1 between the two, against 1.22 before) and carries white text at 5.01 to 1. `--danger-text` is for text, the fill's own value in light and a lifted value in dark, measured at 4.36 to 1 on a card and 6.01 to 1 on the base surface. Every `color` declaration in Rundock's stylesheets now takes the text token and every fill, border, shadow and tint keeps the fill, and a test refuses any text that reaches for the fill. A destructive settings button rests as the same quiet outline as its neighbours and turns to danger on hover; the filled danger button is kept for confirmation steps, where confirming and cancelling carry equal weight. `docs/DESIGN.md` names both tokens and the rule for choosing between them.
- **Pause is a state, and a paused routine says why it is paused:** pause and resume were a pair of buttons sharing the play triangle, so whether a routine was paused was discoverable only by noticing which of two icons was showing. It is now one switch: off, a pause control beside Run; on, the word Paused where the next run time would be, with the way back beside it. The play glyph appears once per row and means Run. A routine paused because something changed what it runs no longer shows an Approve link beside a pause control saying the opposite: it renders as paused in the attention tone, with a sentence naming that what it runs has changed and one action, review and resume. A pause you applied yourself keeps the idle tone and a sentence that names no change, so the two are told apart by words and by colour; drawn identically, a changed routine would be resumed reflexively, which is the outcome the consent mechanism exists to prevent.
...
- **A routine you write is scheduled the moment you write it, and speaks up only if something else changes what it does:** creating a routine is consenting to it, so nothing is asked of you at creation and it runs at the time you set. What Rundock still watches for is a change to what a routine actually does, since agents can edit routines too: if something alters which skill it runs, the instruction it runs, or where it runs, that routine stops and says so before it next fires unattended. Changing when it runs, or editing the body of a skill it calls, asks nothing. The record is kept beside the routine in the agent file, so it travels with the workspace and survives a restart, and routines you already had keep running exactly as they were.
- **See what your workspace is wired to, across both runtimes:** a Connectors tab lists every server your agents can reach, gathered from all four places they are configured: this workspace's own file, the same for the other runtime, and the two equivalents held for your whole machine. Each connector appears once, saying which runtimes reach it and whether it travels with the folder or belongs to you on this machine. Where the two runtimes define the same connector differently, the row says so and shows both, which nothing previously could tell you. Credential keys are named without ever showing their values. Configuration that is broken is shown as an error rather than as an empty list, so a typo cannot look like "nothing configured", and adding a connector will not silently replace one that already has that name. Connectors added at claude.ai are not listed, because their state cannot be read honestly from your machine; the tab says so and links to where they are managed.
- **See what links to a file:** the file view now carries a connections list, showing both what links to this file and what this file links to. Clicking an entry opens the same file that clicking the link inside a document opens.
...
- **Rundock's own folder is graded by what a request would do there, not by treating the whole thing as off-limits:** an agent reading its own transcripts, cache, a global agent file, or a global skill file no longer raises a card, in either mode, because that is no different from what the same agent already reads in a terminal. Writing to a global agent or skill is refused outright rather than asked about: Rundock only ever reads the agents and skills inside your open workspace, so an edit to the global copies would land somewhere the app never looks, report success, and change nothing, which is the bug this closes. Writing to something else that would run again later, a plugin, a command, a hook, or the global settings file, is refused outright as well when an agent edits the file directly, because Claude Code protects its own configuration folder and rejects such a write whatever anyone approves: asking you first and being overruled afterwards taught you that the question meant nothing. The refusal says so, and points at your workspace's own copy, which is what Rundock actually reads. A shell command reaching the same place still asks, and the request says plainly that a write there carries into every later session and every other workspace, including an unattended routine run. Your Claude account credentials are the one exception with no exception of their own: any request that touches them, reading or writing, always asks, in every mode, and nothing you have approved before can answer for it.
- **What the command check can and cannot see, said plainly rather than implied:** working out what a shell command will touch by reading it has no complete answer, so that check is a best effort over the spellings people actually write, and on macOS the operating-system write block is what genuinely holds the line. Some things deliberately raise no card, because a command running a program or discarding its output is not reaching into your files: the system executable directories (`/bin`, `/sbin`, `/usr/bin`, `/usr/sbin`, `/usr/local/bin`, `/opt/homebrew/bin`) and device paths such as `/dev/null`. Some things it will simply miss, such as a path written as `~someone/file`, or one tucked inside a flag as `-C/tmp`. On macOS the write block catches those anyway. On Windows and Linux the card is the whole boundary, which is why this is described as a best effort and not a guarantee.
- **A link written as a bare name now opens the one next to it:** a workspace with a README in most folders, linked as `[[README]]`, sent every one of those links to the README at the very top of the workspace. A bare name now finds the file nearest the link's own folder and only looks further out when there is nothing closer, so a link written inside a project reaches that project's own page. Writing the path out still points exactly where it says.
- **The connections list no longer moves the page when it arrives:** it took seconds to appear and shifted whatever you were reading when it did. It now takes its place immediately and fills in, and the wait itself is mostly gone: opening a file used to re-work out where every link in the entire workspace pointed, not just the ones on the page in front of you.
- **A wikilink now opens the file it names:** a fully qualified link such as `[[folder/note]]` could open a different file that happened to share the same name elsewhere in the workspace. Roughly one in three fully qualified links opened the wrong file, and had done for as long as the feature existed. Exact paths are now matched across the whole workspace before any name-only match is considered, and where two files genuinely share a name the shorter path wins. A link that works no longer shows as dead.
- **A routine created in the evening for a morning time no longer runs immediately:** a routine's next run was worked out on today's date, so one written at half past eleven at night for a midnight schedule was already sixteen hours late and ran the moment it was allowed to, which looked like the approval control having triggered a test run. A time that passed before a routine existed was never owed. One that passed while it existed still is, so a machine that slept through the morning still catches up on what it missed.
- **Your agents can add and remove connectors again:** the Connectors tab hands adding a connector to your workspace guide, and the guide could not do it, because the file connectors live in is protected from every agent's own editing tools whatever you approve. Connectors now go the same way agents and skills already do: the agent says what it wants written and Rundock writes it. A connector you add never disturbs the ones already there, a file Rundock cannot read or make sense of is left exactly as it is rather than replaced, and asking to remove something that was never configured says so instead of reporting a removal that did not happen.
- **A shell command with a pattern in quotes, or one throwing its error output away, no longer draws a card:** two parts of Rundock read command text and had learned different things about it, so an ordinary listing or a search whose pattern contained a pipe was treated as risky by one of them even after the other had been taught otherwise. Both now read a command the same way. Commands that genuinely write, including one hiding a removal behind a harmless first word, are unchanged.
- **Windows gets the same quiet as macOS:** the rules deciding whether a command merely reads had been written for one platform, so on Windows an ordinary listing of your global agents and skills was treated as a write and raised the same wall of approvals this release exists to end. Listing a hidden folder, which on Windows requires a flag that elsewhere means force, was additionally warned about as though it might overwrite or delete. Both are fixed, and a command that really can destroy is warned about exactly as before.
- **Your agents stop refusing work they are allowed to do:** the rules told every agent not to touch anything in a `.claude` folder, when the rule was only ever about two folders inside it, so agents declined ordinary edits in your own workspace and explained it with machinery that was not what stopped them. They now attempt a file operation and tell you what actually happened, rather than declining on a guess.
- **A file that cannot contain links no longer says it links to nothing:** the connections list showed an empty "Links to" section under files the link index never reads links from, which is not an empty list but a question that cannot apply. That section is now absent for those files, and the list no longer sits flush against the sidebar in the file viewer.
- **Listing your global agents and skills no longer asks permission:** a command that threw its own error output away, written as `2>/dev/null`, was read as though the discarded output were a file being written, so an ordinary listing was treated as a write to somewhere that persists and raised a card naming two places at once. Discarding output writes nothing, and neither does pointing one output stream at another, so neither counts against a command any more. Every other redirection still does, including one pointing into the folder being read.
- **A destructive command can no longer ride in behind a harmless one:** two commands joined by a single `&` were judged together by whichever came first, so a listing followed by a deletion was read as a listing and asked nothing. Each command a line runs is now judged on its own, exactly as it already was for the other ways of joining two commands, and one command that is not a plain read is enough to raise the card for the whole line.
- **Code mode no longer answers the questions that were never meant to be asked:** the refusals that protect your global agents and skills were being approved automatically in Code mode, so an edit there was allowed, landed where the app never reads, and reported success. Those refusals now run before anything else can speak for them, in every mode. Code mode still approves ordinary work inside your workspace as it always has.
- **A workspace inside Claude Code's own folder works normally again:** opening a folder there, which someone writing a plugin does, meant every ordinary file you saved was refused, with no way past it. What a workspace holds is yours to edit, whatever it sits inside.
- **Adding a connector is now a conversation:** the two boxes on the Connectors tab could not describe what a connector needs to start, since most take arguments, a web address, or credentials, so the usual result was an entry that looked accepted and could never run. The tab now hands this to your workspace guide, who can work out which kind it is, write it, and tell you what is still missing.
...
- **A comment inside a code block no longer corrupts the file:** commenting inside or around a fenced block used to close it early, turning the rest of the document into escaped prose. Rundock now refuses a comment anchored inside a fenced block with a clear reason, and writes blocks back with their original fence.
- **A routine with no prompt no longer runs its agent on the word "null":** a routine that declared a schedule but no prompt fired anyway, unattended, asking its agent to act on the four characters `null`, with no warning anywhere. Such a routine is now refused before it runs, and its row names what to add.
- **Switching workspace no longer shows a live run as cut short:** the routines panel and agent profile could report a run as interrupted for as long as it was running, in either workspace, correcting itself only once it finished. Rundock now knows which runs it started and has not yet finished.
- **Upgrading no longer starts routines you never asked it to run:** a routine written by hand before Rundock had a scheduler used to go live the moment Rundock first ran in that workspace, duplicating work your own cron job or script was already doing. Such a routine now waits until you turn it on, and says plainly what doing so will start.
- **A routine with an unreadable schedule now says so:** a cron line or other unsupported format used to be accepted and silently skipped forever. Its row now names the two schedule formats Rundock understands.
- **Routines now say which workspace they belong to:** only the open workspace's routines run; the rest sit dormant with nothing saying so. The Routines list now names the workspace its rows came from, states plainly when Rundock has moved elsewhere, and drops next-run times and the Turn-on control so it never promises work that will not happen.
- **Navigation and display fixes:** the sidebar icon now always matches the screen you're on; switching workspace no longer hides the New Conversation button; and the workspace picker no longer appears with a stale sidebar drawn underneath it.
- **Agent output can no longer run code in the app:** anything an agent wrote into a conversation or a note rendered as literal page markup, reachable without your involvement since routines make agents produce content on a schedule. Six injection paths are closed, and a tag inside a code block or at the start of a line now renders correctly too.

### Changed

- **The always-on setup guide now explains the double-run risk correctly:** it previously blamed your sync tool for a second instance firing every routine again; the actual cause is that each running Rundock reads the last-run guard only at startup. The guide now names the real mechanism and the fix: run routines on one machine and reach it from your other devices.
- **Opening a skill's own file now moves you to Files**, where the editor lives, rather than leaving the Skills icon lit over an unrelated screen.
...
- **A long link in a review reply stays inside its card:** replying to a review comment with a long web address pushed the text out through the side of the card and across whatever sat next to it. Comment bodies themselves always wrapped correctly; replies were the one place nothing told the text where it was allowed to break.
- **Accepting a review suggestion keeps its formatting:** if a suggested replacement contained bold text, code in backticks or a link to another note, accepting it wrote backslashes into your document instead: `**bold**` came back as visible asterisks and slashes. The suggestion was being put back as plain characters rather than as the formatting it was written in, so saving the file dutifully escaped it. Wikilinks were affected more quietly, arriving as text that looked right but no longer opened anything. Accepted suggestions now keep exactly the formatting they were written with, and a wikilink resolves.
- **A long agent turn keeps all of it when you reopen the conversation:** a turn where the agent wrote, used a tool, wrote again and finished with a summary showed in full while it was happening, then collapsed to its opening line once the conversation was reopened. The summary is usually the part worth keeping, so what survived was the least useful fragment. Nothing was ever lost from disk: Rundock stores such a turn as one entry while the session file splits it into several, and rebuilding the conversation took the first and stopped. It now gathers every part, in order.
- **An agent's own working files stop raising approval cards:** an agent would write a working file, read it back a step later, and the read raised an outside-the-workspace approval card for a file the agent had created seconds earlier. The card was right to appear; putting the file outside the workspace was the mistake. Working files now stay inside the workspace, and old ones are cleared away without touching a session still using them.
- **Tab no longer jumps you out of the editor:** pressing Tab anywhere the editor had nothing to indent, in a heading, an ordinary paragraph, or the first item of a list, moved you out of the document entirely and into the browser's address bar mid-sentence. Notes containing callouts made it look like a callout problem, because a callout's controls were simply the nearest thing for the cursor to land on. Tab stays in the document now, and still indents and outdents list items and moves between table cells exactly as before.

## 0.11.7: Foundations & File Tree Stability (2026-08-17)

### Fixed

- **The file tree stops flickering and losing your place while an agent works:** every time an agent created or deleted a file, the whole sidebar tree was thrown away and drawn again. Folders you had opened snapped shut, the highlight on the file you were reading was lost, and the list jumped back to the top, several times a minute during a busy conversation. The tree now updates only the part that changed, so what you had open, selected and scrolled to stays exactly where it was.
- **Skills assigned by name in an agent's instructions now work on Windows:** Rundock assigns a skill to an agent two ways, from an explicit list and by finding the skill's name written in the agent's instructions. The second one did nothing at all on Windows, silently, because of how line endings differ there. Explicit lists kept working, so the failure looked like the feature simply not matching rather than being broken.
- **Several faint labels and controls now read as intended:** the hover controls on a conversation row, the find bar's placeholder and two permission surfaces were written to use a dimmer tone that had never actually been defined. They quietly took the colour of whatever surrounded them instead, in every version since 0.5.0.
- **A callout edit that cannot be saved now stays flagged:** typing an invalid callout heading and clicking away keeps your text and marks the box, as before, but the mark used to fade after about half a second. Look away and it looked fine while the edit sat there unsaved. It now stays until you start fixing it.
- **File icons stay the right size however deep the file sits:** in a nested folder the icon in the sidebar was quietly squeezed to make room for the filename, and the deeper the file, the smaller it got. On a narrow sidebar a file four folders down had no visible icon at all. Icons now hold their size at any depth and any sidebar width, and a name too long for the row is shortened with an ellipsis instead. The full name is still there for screen readers.
- **The team's org chart no longer needs an internet connection:** the layout library behind the chart was fetched from a public CDN every time the app started, so on a machine with no connection the team view failed to draw at all. It now ships with Rundock, as the code highlighting already did.
- **The app always looks the same, on any machine and offline:** the typeface was downloaded from Google on every launch, so a slow connection or no connection quietly swapped it for whatever the operating system offered, and every launch told a third party you had opened the app. Inter now ships with Rundock. Nothing about the interface is fetched from the internet any more.
...
- **Search opens where you clicked it:** choosing the search field expands it in place into the results panel, at the same width and in the same spot, rather than opening a separate box in the middle of the screen. Escape closes it and puts the cursor back in the field.
...
- **The search filters no longer show a stray scrollbar:** the row of All / Files / Conversations / Agents / Skills filters drew a small horizontal scrollbar beneath itself on narrower windows, taking space from the results and looking like a mistake.
...
- **See and work with any file type, not just markdown:** clicking an HTML or SVG file shows the real rendered page (its own styles, layout, and fonts) in a locked-down preview where scripts never run and the page cannot phone home; a PNG, JPEG, GIF, WebP, or PDF opens as the actual image or a readable document, PDFs with the pages panel collapsed for more reading room. File types Rundock cannot preview say so clearly instead of showing garbled bytes, and the Edit toggle still shows editable raw source for text and HTML.
- **Manage Kanban boards inside Rundock:** a markdown file that is an Obsidian Kanban board opens as a real column board. Drag cards between columns, add and edit cards in place (with live formatting for bold, links, and wikilinks, and a plain editor for cards it cannot represent losslessly), tick them off, delete with a quick undo, and rename, move, sort, archive, collapse, and delete columns from a per-column menu. Card text shows styled markdown, tags, and dates rather than raw syntax, and wikilinks navigate. Everything writes back as the exact same markdown the Obsidian Kanban plugin produces, so the same board edits interchangeably in Rundock and Obsidian without either one reformatting the other, and multi-line tag and alias frontmatter is preserved untouched.
- **Review agent-produced HTML and SVG files with inline comments:** select a passage in the rendered preview, press Comment, and the same review sidebar handles threads, replies, and resolve. Feedback anchors to the exact text you selected and lives in an open JSON file under the workspace's `.rundock/reviews/` folder (the file itself is never touched); agents know how to read it, act on it, and resolve it; a comment whose passage was edited away is kept and labelled Orphaned rather than dropped; and SVG text stays visible when commented.
- **Edit frontmatter properties in the panel:** click a value to edit it in place, toggle booleans, add and remove list items inline, and follow property wikilinks (a `related:` list navigates on click; dead links look visibly dead), all without touching raw YAML. Edits are byte-honest: changing one property changes only that property's lines, quote styles are preserved, and an edit that could corrupt the frontmatter is refused rather than guessed.
- **Obsidian callouts render and edit in place:** typed, coloured admonition boxes with titles, working expand/collapse for `+`/`-` fold markers, and nested callouts as real nested boxes, never literal `[!type]` text. An edit control opens a callout's markdown inline and reparses it back to a rendered box, byte-for-byte.
- **Search and find across every surface:** Cmd+K now finds text inside HTML and SVG artifacts (visible text only, never markup, scripts, or styles) and matches frontmatter property values, not just their names; Cmd+F finds inside a rendered artifact preview, its source view, and the frontmatter panel, not just the markdown body. Snippets read cleanly with no tags, class names, or raw YAML, and the artifact preview stays fully locked down.
- **The Files sidebar creates files and keeps your place:** a plus button and right-click menu create notes, boards, and folders (and, on macOS, reveal a file in Finder), each row and tree entry carrying an icon for its type. The file you have open stays open when you switch views and comes back, and it is revealed and highlighted in the tree so you always see where it lives.
- **Files refresh live, and Rundock warns before overwriting one changed outside it:** a file you have open updates in place when it changes on disk (an agent, Obsidian, another window), and if you have unsaved edits when that happens, the next save offers a visible choice, reload theirs or keep mine, instead of silently overwriting.
- **Organise conversations with Lists:** right-click any conversation to add it to a named list (or create one on the spot); lists appear as pills beside All and Unread and filter the sidebar with the usual pinned-first ordering. A conversation can belong to several lists, lists survive reloads and workspace switches, and deleting a list never deletes conversations: they simply leave the grouping.
- **First-run setup adapts to the runtimes you have:** the wizard detects both Claude Code and Codex. A Codex user is told plainly why Claude Code is still needed (it runs your team's orchestrator and the setup guide) instead of a bare install instruction, and once Claude Code is signed in, a machine with the Codex CLI gets an optional step to sign in to Codex so specialists can run on a ChatGPT plan, skippable and available later from Settings. Machines without Codex see the setup flow exactly as before.

### Changed

- **Draft your next message while an agent is still responding:** the message box no longer locks while the agent works, so you can start writing your follow-up immediately. The button stays a Stop control during the response; to send a drafted message, stop the agent first and it becomes Send (active when the box has text). Pressing Enter while the agent is responding inserts a newline instead of sending.
- **Codex agents stream, approve, and cancel like Claude agents:** replies now type into the conversation as the agent thinks, over one long-lived runtime process per set of Codex conversations, so follow-up turns start faster and a crash recovers automatically. Any action needing extra access (a command outside the sandbox, or a write it normally cannot make, including all writes on Windows without the native sandbox config) raises the familiar permission card mid-turn, and an ignored request is declined automatically so a turn can never hang. Cancelling interrupts the running turn instantly and withdraws any open approval card, and stopping then immediately sending another message is handled cleanly rather than hanging or erroring.
...
- **Editing a note no longer reformats or corrupts its markdown:** the editor's save path now preserves task-list checkboxes (`- [ ]`/`- [x]`, previously escaped to `- \[ \]`), the bullet-list marker you used (`*`/`+`/`-`), thematic-break style (`***`/`___`/`---`), literal brackets in prose, and already-escaped punctuation, and headings four to six and `$inline$`/`$$block$$` LaTeX survive byte-exact (previously deeper headings flattened to plain text and maths had its backslashes doubled on every save). Images (`![alt](url)`), inline and block HTML, hard line breaks (both trailing-space and backslash), and tab-indented list nesting now round-trip exactly too, where each was previously dropped, entity-escaped, deleted so lines merged, or rewritten to spaces. Common markdown round-trips through the editor byte-for-byte.
- **Editing a Kanban board, searching, and editing properties no longer lose content in edge cases:** a board saved with Windows line endings renders instead of failing, malformed board settings are kept instead of wiped, a file whose frontmatter opens with a `comments`, `review`, or `suggestions` key stays fully searchable instead of having its body dropped, and a property edit that cannot be made without corrupting the frontmatter (a trailing comment, an anchor or alias) is refused rather than guessed.
- **Pressing Enter next to a wikilink no longer deletes the rest of the line:** editing a note and hitting Enter at the end of a line containing `[[wikilinks]]` kept only the first link and silently dropped the surrounding text and any other links, and autosave persisted the loss. The line is preserved and the new line is created normally.
- **Tighter approvals and safer artifact previews:** running `node -e` or `python -c` now raises a permission card instead of being auto-allowed as a read-only command; cancelling a turn withdraws its open approval cards so a cancelled action leaves no standing grant; a permission card is no longer duplicated when the connection drops and reconnects; a delete instruction quoted inside a save block is no longer executed; and in the artifact preview, external links open in your browser while internal links open in Rundock, over a further-locked-down sandbox.
- **A follow-up sent while a directly-messaged specialist hands the conversation back is delivered instead of being lost:** the queued-message rule that guards delegation handoffs now also covers the direct hand-back path, so a message racing that handoff supersedes the automatic routing prompt and drives the incoming orchestrator itself.
- **A message sent in the moment an agent handoff completes reaches the restored agent instead of being lost:** conversations pass through an explicit killing/restoring state while a handoff finishes, and any message arriving in that window is queued and replayed to the incoming agent the instant it is ready.
- **Approval cards for conversations you are not looking at queue up, badge the conversation, and appear when you switch to it:** a permission request raised in a background conversation now marks it unread and renders its card the moment you open it, answerable until it expires, instead of being silently discarded and timing out to a denial.
- **Scheduled routines fire reliably and never twice:** routine run state persists in the workspace's `.rundock/` folder, so reopening Rundock after a routine has run cannot fire it again the same day, and a routine whose scheduled time has passed now fires on the next scheduler tick (previously the timing comparison was so strict they effectively never fired on their own). An interrupted run shows as "interrupted" rather than appearing to still run.
...
- **Intel Mac support via a universal build.** The Mac download is now a single universal build that runs natively on both Apple Silicon and Intel Macs. Intel users can download and run Rundock directly instead of building from source. One download works on any Mac, so there's no architecture to choose.
