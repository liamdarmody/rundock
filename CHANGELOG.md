...
- **Routines now say which workspace they belong to, and what that means for the ones in your other workspaces:** Rundock runs one scheduler and it serves the workspace you have open. If you keep routines in more than one workspace, only the open one's routines are running and the rest are dormant, and nothing anywhere said so: the list looked complete, every row showed a next run, and a briefing you had set up in another workspace simply never arrived. The Routines list now names the workspace it is showing and says plainly that the routines in your other workspaces are not running. A row for a routine that was read out of a workspace this window does not have open shows no next run and says which workspace it is actually in, instead of promising a time nothing was going to keep. And the scheduling step of the routine editor says the same thing while you are setting one up, along with the part that softens it: coming back to that workspace the same day serves a slot that went by. **This is deliberate rather than a limitation waiting to be lifted.** Running routines out of a workspace you have not opened would mean Rundock starting agents under that workspace's permissions, against files you are not looking at, which is the one thing its folder boundary exists to prevent. The reasoning, and what would change it, is written down in `docs/ROUTINES.md`.
