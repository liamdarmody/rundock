...
- **Cards stop asking about work that never left your workspace.** A command that stepped into a subfolder and then referred to something beside it was read as reaching outside, so an agent looking at your own agents and skills asked permission to do it, naming a file that in some cases did not exist. Summarising your team or your skills was treated as changing them, because a loop was graded as a write whatever it did inside. Reading your own permission settings asked as well. All three now go through without a card, and a folder you approve stops asking on the very next command instead of the next conversation.

- **"Always allow this folder" does what it says.** It used to remember your answer for file access only, so the next command touching the same folder asked again, and the folder appeared nowhere in Settings afterwards. It now names the folder as a working folder, which covers commands too, and the folder shows up in the list where you would look for it. The offer also appears on the cards where you actually meet the problem: most approvals come from commands, and those never offered it at all.
