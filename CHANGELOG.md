...
- **Every agent knows the basics about Rundock.** Two-tier addition. Tier 1 prepends a short identity line to every agent's system prompt: agents now know what product they're inside and where to point users for deeper meta questions (docs.rundock.ai, or route to Doc). This means any specialist asked "what is Rundock?" answers with the core tagline rather than "I don't know". Tier 2 rewrites Doc's "What you know" section in `scaffold/rundock-guide.md` into a slim About Rundock subsection (three bullets covering what it is, the licence, and the creator with three canonical surfaces) and a Workspaces subsection preserving the existing workspace definition. Doc now has the literal answer to "who created Rundock?" embedded in its scaffold (Built by Liam Darmody) rather than only a pointer at the README. Existing scaffolded workspaces pick up the Tier 1 change on app update; the Tier 2 scaffold change reaches them only on re-scaffold or manual edit.

### Changed

- **Conversation sidebar surfaces what needs attention.** The sidebar reorganises into three predictable sections. Pinned conversations sort by last activity (most-recently-active pinned conversation at the top), so the conversation you just used is where you expect it. Active conversations (non-pinned, non-done) sort by a three-tier compound rule — working agents first, then unread messages, then idle — each tier ordered by last activity. Done stays as the collapsed third section, with an unread dot on its header when any done conversation has unread messages. Pinned conversations whose agent is currently working get a subtle border highlight so the busy state is visible without forcing them out of the pinned section. Tier transitions in the active section animate over ~600ms via CSS view transitions where supported, so a conversation that finishes processing slides to its new position rather than snapping. Conversations now record `lastActiveAt` on every agent finish (previously only on user message send), so the sort key reflects real activity even after long agent runs.

### Fixed

- **Spawn failures surface as a chat message, not a connection drop.** Every spawned Claude Code child process used to run without an `'error'` event listener. When `spawn` failed for any reason — binary missing, permission denied, malformed system prompt, auth failure — the unhandled error tore down the WebSocket and the user saw "Disconnected. Reconnecting..." with no clue what actually went wrong. The `spawnClaude` wrapper now attaches a baseline error listener that always runs inside try/catch (so a fault in the handler can't tear down the connection), and chat-flavour callsites pass an `onError` callback that surfaces a `system/info` message scoped to the conversation with copy that varies by error code (ENOENT, EACCES, fallback). Cancelled processes skip the user-facing message. Consecutive identical errors per conversation are deduped within a 30-second window. The close handler now gates its emissions on a `spawnFailed` flag so error and close don't double-fire. This is the latent bug that made the CRLF parsing failure look like a connection problem rather than a parser problem during the 2026-05-05 troubleshooting session; future spawn failures (auth expired, binary moved, permission denied) will surface clearly rather than masquerading as a disconnect.

- **No phantom duplicate Doc when the scaffold file is broken.** The built-in Doc fallback used to fire whenever no agent in the discovered list had `type: 'platform'`, which misfired when a `rundock-guide.md` was present on disk but its frontmatter failed to parse. The result was two `rundock-guide` entries with the same id, and lookups returned the broken one. The fallback now also checks that no agent with id `rundock-guide` was discovered before pushing. The CRLF fix removes the only known trigger today; this guard closes the broader class of "broken scaffold produces phantom duplicates" so future parser issues do not reproduce the symptom.
