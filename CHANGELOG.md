...
- **Upgrading no longer starts routines you never asked it to run:** if your agent files already had routines in them, written by hand before Rundock could run one, the first time Rundock ran over that workspace every one of them went live at once. For anyone who had been driving those jobs with their own scheduler, that meant the work ran twice: the morning briefing went out, and then went out again. Rundock now treats a routine that never said whether it may run as one waiting to be turned on. It is still listed, it keeps every word of its file, and it does nothing until you say so. Its row offers to turn it on and tells you what that starts, in as many words, because the thing you need to know before pressing is that Rundock will begin running it too. It also tells you when: if today's time has already gone, turning it on runs the routine within the minute rather than waiting for tomorrow, which is worth knowing when the job you are turning on already ran this morning under your own scheduler. **A routine that says `enabled: true` or `enabled: false` is left exactly as it is,** so this never switches off something you had deliberately turned on. Those two words are the only ones read: a routine written `enabled: yes` waits to be turned on like one that said nothing, which is the safe direction and something its row tells you. Routines you make in the routine editor are unaffected and start running the moment you save them, with nothing extra to press, because that is the one path that writes the answer for you. A routine that reaches an agent file any other way, typed into the frontmatter by hand or written there by an agent, has not said whether it may run and is listed as waiting to be turned on until its block says `enabled: true`. This also holds on a workspace Rundock cannot write to, such as one checked out read-only: it still will not start a routine there rather than starting it because it could not record the decision. A routine waiting to be turned on is also no longer told it missed its slot because Rundock was closed: it was not closed, the routine was simply not running yet, and a row that named the wrong reason would send you looking for a problem that was not there.
