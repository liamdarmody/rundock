...
## 0.13.0: Permissions & Connections (2026-09-05)
...
- **See what your workspace is wired to:** a Connectors tab lists the servers configured for this workspace, read from the same file the runtime itself reads, and says what each one starts or talks to. Credential keys are named without ever showing their values. Configuration that is broken is shown as an error rather than as an empty list, so a typo cannot look like "nothing configured", and adding a connector will not silently replace one that already has that name.
