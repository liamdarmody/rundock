...
- **Edit frontmatter properties in the panel:** click a value to edit it in place, toggle booleans, add and remove list items, without touching raw YAML. Edits are byte-honest: changing one property changes only that property's lines in the file, quote styles are preserved, and an edit that could corrupt the frontmatter is refused rather than guessed.
- **Rundock warns before overwriting a file that changed outside it:** if a file you are editing changes on disk (Obsidian, an agent, anything), the next save shows a visible choice, reload theirs or keep mine, instead of silently overwriting. Nothing changes for normal editing.
...
- **Search now covers HTML files and property values:** Cmd+K finds text inside your HTML and SVG artifacts (their visible text only, never the markup, scripts, or styles) and matches on the values of frontmatter properties, not just their names. Results read cleanly: no tags, class names, or raw YAML ever appear in a snippet.
- **In-view find reaches every surface:** Cmd+F now finds text inside a rendered HTML artifact preview, its source-edit (Code) view, and the frontmatter properties panel, not just the markdown body. The artifact preview stays fully locked down: find reads the sandboxed page without loosening it, and scripts still never run. In the source-edit view the matches are highlighted behind the text so you can see every one.
- **Manage Kanban boards inside Rundock:** a markdown file that is an Obsidian Kanban board now opens as a real column board. Drag cards between columns, add and edit cards in place, tick them off, and delete them with a quick undo; a per-column menu renames, moves, sorts, archives, and deletes lists, and columns collapse to a rail. Card text shows styled markdown rather than raw syntax. Everything writes back as the exact same markdown the Obsidian Kanban plugin produces, so the same board can be edited in Rundock and Obsidian interchangeably without either one reformatting the other, and your frontmatter (including multi-line tag and alias lists) is preserved untouched.
- **The Files sidebar keeps your place:** the file you have open stays open when you switch to another view and come back, and it is revealed in the tree (its folders expand and the row scrolls into view and highlights) so you can always see where it lives. The editor back arrow now appears only when it leads somewhere, rather than dead-ending on an empty pane.
...
- **Adding a frontmatter list item no longer hides the existing ones:** the input for a new tag or list value now appears inline after the current items instead of replacing the whole value, so you can see what is already there while you type. The fold arrow on collapsible callouts is also a little larger and easier to hit.
- **Settings no longer freeze the app on first open on Windows:** the runtime version checks behind the Settings panel could each hang for their full five-second timeout on Windows (the Codex CLI stalls when asked for its version with an open input pipe), stacking up to ~13 seconds during which every conversation stopped responding. The checks now close that pipe, answer instantly, and repeat Settings opens reuse the recent result instead of re-running them.
- **A stopped Codex reply can no longer paste itself into your next answer:** on some machines (seen live on Windows ARM with codex-cli 0.144.4) the runtime accepts the stop request but ignores it, so the "cancelled" reply keeps running to completion in the background. Its late output used to be attributed to the conversation's next turn, and your next answer could arrive with the entire cancelled reply pasted in front of it. Every event is now matched to the exact turn it belongs to: the stopped turn's stragglers are discarded (and logged), the stop request is re-sent once as a nudge in case the first was lost, and your next answer contains only itself.
- **Stopping a Codex reply and immediately sending another message now works, or asks you to resend in a moment; never an error card:** two gaps closed. If the runtime is still wrapping up the stopped turn when your next message arrives (its session file has not been flushed yet, or the stop acknowledgment was lost in transit), Rundock retries briefly and then shows "Resend your message in a moment" while keeping the conversation's memory intact, instead of a raw error that repeated on every retry. And a stop whose acknowledgment never arrives no longer wedges the conversation for tens of seconds: the turn slot is released after a bounded wait, so the follow-up goes through. A genuinely missing session still recovers by starting fresh, exactly as before.
- **A follow-up sent while a directly-messaged specialist hands the conversation back is delivered instead of being lost:** the queued-message rule that guards delegation handoffs now also covers the direct hand-back path, so a message racing that handoff supersedes the automatic routing prompt and drives the incoming orchestrator itself. Previously the routing prompt fired anyway and, when it re-delegated, the queued message died unread with the orchestrator's process.
- **Approval cards for conversations you are not looking at queue up, badge the conversation, and appear when you switch to it:** a permission request raised in a background conversation now marks it unread in the sidebar and renders its card the moment you open it, still answerable until the request expires (expired or cancelled requests can never be answered from a stale card). Previously the card was silently discarded, the agent waited out the two-minute timeout, and the action was denied without the user ever seeing anything.
