...
- **A command that reaches outside your workspace now asks you, and says that is what it is asking:** a shell command could write to your home directory, or anywhere else on the machine, with no approval card at all in a Code-mode workspace, while an ordinary file edit to the very same place asked first. The two cards ask different questions. Approving a command you cannot fully read is not the same as agreeing to let an agent out of your folder, and only the second is what Rundock promises. **A command whose target can be seen in its text** now raises the boundary card naming that target, and if it reaches several places outside, the card lists all of them rather than the first. Commands that stay inside are unaffected and still run without interruption. Windows commands are covered as well, including drive letters, network shares, backslash paths and the home shorthands PowerShell uses, since Windows commands run through a different tool that was exempt entirely.
- **What this does not claim.** A target a command computes rather than spells out cannot be seen in its text, and no amount of reading the command will find it. On macOS the operating system catches those, described next. **On Windows and Linux it does not, and there the approval card is the whole boundary:** a command that builds its target out of variables can still reach outside without a card. Rundock would rather say that than imply a guarantee it does not have.
- **On macOS the operating system now enforces the folder boundary, not just the question:** the runtime Rundock starts is given a list of folders a command may write to, so a write anywhere else fails at the operating system level, whether or not anything could read the intention out of the command first. If an agent needs to step outside, it asks, and that request is the boundary card rather than a command card. That card names no single folder, because the operating system refused the command rather than a path.
- **What is writable on macOS, exactly.** Your workspace, the npm cache, and the runtime's own default locations, which it keeps and Rundock does not remove: measured, those are its debug log folder and the temporary directory. Everything else in your home folder was measured as refused, including your documents, your SSH keys, your shell startup files and the other package caches. **A package manager other than npm will therefore fail on its own cache,** and that failure can arrive looking like a file-ownership complaint rather than a refused write, so it may not read as a boundary problem at all. Network access is unchanged.
- **Linux is deliberately left out of the operating-system half.** The runtime supports it; Rundock has not measured it there and does not ship configuration it has not run. On Linux, as on Windows, the approval card is the whole boundary.
- **One thing got noisier in knowledge workspaces:** a read-only command naming a path outside your workspace, such as reading a file in another folder, used to go through as an ordinary low-risk command and could be covered by "Always allow" for the session. It is now a boundary card, because where it reads is the point rather than what it runs, and boundary cards are never auto-approved and never carry a session-wide "Always allow". Commands that stay inside your workspace are unchanged.
