...
- **A conversation with several agents now reads like a conversation.** Agents handed work to each other in silence: the thread jumped to a new name with nothing saying who was taking over or why, and the brief they had written for each other was never shown to you at all. Now whoever is handing over says what they are passing on and to whom, in their own words, at the moment it happens, and the work reaches whoever they named instead of stopping where it was while the handover reads as done. Whoever picks it up is told what they walked into, whether they are joining for the first time or coming back, so they stop repeating work that is already done. An agent arriving is announced once; an agent already in the conversation simply speaks, because the name on the message says who it is. Alongside that: a specialist can finish its part without ending the whole request, each agent's work appears in its own bubble rather than inside the previous agent's, and a handover that was in flight when you quit the app still works when you come back. A handover also costs the same whichever way it started, instead of occasionally handing the next agent the whole conversation.

- **You answer a permission question once.** Answering "Always allow" held only until you refreshed the page, and then the same card came back; it is now kept with your workspace, and Settings lists the tools you have allowed, with a way to take any of those back. Naming your working folders and switching to Code mode was meant to end the cards for those folders and for some people did not, because Rundock controls only one of several places the operating system sandbox is configured; it now states your folders whichever mode you are in. A file an agent saved and read back could raise a card every single time, with no way to answer once. And an approval for a command now tells you when that command reaches outside your workspace, rather than only asking whether it may run. A command that only reads no longer asks at all: naming a working folder ended the folder question, but an ordinary search, listing or file read still raised a card, and so did any of them preceded by a change of directory, because a second check graded a command on its name against a much shorter list than the one the boundary already used. The file holding your permissions is also no longer something an agent can quietly edit: changing it asks you first, every time, with no option to stop being asked. And a command that only looks like it reads now asks as well: a search or a listing can be told to delete or to run things as it goes, and those spellings were being waved through as ordinary reads.

- **Cards stop asking about work that never left your workspace.** A command that stepped into a subfolder and then referred to something beside it was read as reaching outside, so an agent looking at your own agents and skills asked permission to do it, naming a file that in some cases did not exist. Summarising your team or your skills was treated as changing them, because a loop was graded as a write whatever it did inside. Reading your own permission settings asked as well. And asking what folders are in a directory raised a card naming something like `/$`, because the pattern the command searched for was read as a place it was reaching: the ordinary way to list folders produced a card about a path that does not exist. All of these now go through without a card, and a folder you approve stops asking on the very next command instead of the next conversation.

- **"Always allow this folder" does what it says.** It used to remember your answer for file access only, so the next command touching the same folder asked again, and the folder appeared nowhere in Settings afterwards. It now names the folder as a working folder, which covers commands too, and the folder shows up in the list where you would look for it. The offer also appears on the cards where you actually meet the problem: most approvals come from commands, and those never offered it at all. Because it now carries that much more weight, it is no longer offered for hidden folders in your home directory such as `~/.ssh` or `~/.aws`: a question about one file in there used to come with a button that would have allowed the whole folder, keys included, forever. Those still ask, and connecting to a server, copying files or pushing to git never asked in the first place, so nothing you do is slower. If you want agents working in a folder like that, you can still name it yourself under Settings.

- **Commands that cannot be undone still ask, even in Code mode.** Code mode approves ordinary work without interrupting you, which is the point of it, but deleting, forcing and elevating were going through on the same ticket: `rm -rf` inside your own workspace ran with nothing shown and nothing recorded. Those now reach you, while running tests, checking status and reading files stay silent. If Rundock cannot ask you because it is not running, they are refused rather than run. Agents were also being told, and repeating to you, that such commands were not supported and would never reach you for approval. That was never something an agent could know, since from inside a turn an approval you gave and a question never asked look the same, and it is no longer in what they are told.

- **Code mode turns the operating-system write block off, as it has always said it does.** It previously stayed silent about the setting instead of switching it off, and silence loses to any other place the sandbox is configured, so tools that start their own processes, such as a headless browser, kept failing in the mode documented as the one where they work. Where your sandbox is set up outside Rundock, Settings now tells you so rather than promising a change it cannot make.

- **Agents can read their own configuration without asking.** Reading the settings for the Codex runtime raised a card while the same file for Claude did not, so the answer to "what are my current permissions" depended on which runtime happened to be asking. Credentials still ask, in both, and cannot be covered by approving the folder around them.

- **Confirmations appear in the conversation that earned them**, and approvals appear beside the work they interrupted. Creating or deleting an agent or a skill used to announce itself in whichever conversation happened to be on screen, and an approval could appear below a response that had not been written yet.

- **An agent's message reads the way the agent wrote it.** Paragraphs either side of a tool call ran together, so the last word of one joined the first word of the next, and reopening a conversation could show a fragment of Rundock's own record of the tools an agent ran sitting in front of its words. Both are gone, and the second is fixed for conversations you already have.

- **Text that isn't formatting stays text.** A run of equals signs in an agent's message was partly swallowed and partly left behind, so a line came out looking broken. It now reads exactly as written.
