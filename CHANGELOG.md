...
You can now schedule a skill to run on a cadence through a form, with no hand-editing or conversation required, and this release makes that schedule something you can trust: agent-controlled content can no longer inject executable markup into the interface, and MCP credentials can be kept out of the file your team shares.

### Added

- **Routines are now set up through a form, not written by hand:** pick an agent, the skill you want it to run, and a daily or weekly cadence, and the routine editor writes the schedule into the agent's file for you. Before this, creating one meant hand-editing frontmatter or asking Doc to do it in conversation. Scheduling itself runs on Rundock's own server rather than your operating system's scheduler, so a routine behaves the same on macOS, Windows and Linux instead of depending on each platform's own cron equivalent.
- **You can now read what a routine actually did:** every run leaves a record naming the files it changed. A routine that has run carries a "View last run" link showing how it ended, how long it took, and every file it touched. A run cut short by Rundock closing is described as one whose ending never ran, not as a failure, since it may have finished a moment before closing.
- **Your MCP API keys can now live somewhere the rest of the team never sees:** `.mcp.json` sits in the shared workspace folder, so a key written into it travelled with every git clone and stayed in history even after deletion. Values now live in `.rundock/mcp-secrets.json`, which Rundock already excludes from git, and are injected only at the moment an agent starts. A key already written literally in `.mcp.json` keeps working; moving it is yours to do when ready.
- **A running routine can now be stopped without quitting Rundock:** a routine that had gone wrong could only be stopped by quitting the whole application, since nothing held a reference to what it had started once the function that spawned it returned. A run is now reachable from the moment it opens, including the brief window before its process has actually started, so a stop lands even then. Stopping is real: the whole process tree the run started is signalled, its record shows it was stopped rather than that it failed, and a run that has already finished cannot be stopped after the fact.

### Fixed

- **A routine with no prompt no longer runs its agent on the word "null":** a routine that declared a schedule but no prompt did not fail to start; it fired on time, unattended, and asked its agent to act on the four characters `null`, with nothing on any screen suggesting anything was wrong. Such a routine is now refused before it runs, and its row says so, naming what to add rather than promising a run that will never come.
- **Reviewing a page with a code block no longer rewrites it:** commenting inside or around a fenced block used to close it early, turning the rest of the document into escaped prose with backslashes in front of every asterisk and backtick. A comment anchored inside a code block is now refused with a clear reason instead, and a block is written back with the exact fence it was written with.
- **Switching workspace no longer reports a still-running routine as cut short:** the routines panel and agent profile could show a live run as interrupted in both workspaces for as long as it ran, correcting itself only once it finished. Rundock now knows which runs it started and has not yet finished, so it stops guessing.
- **Upgrading no longer starts routines you never asked it to run:** hand-written routines predating Rundock's scheduler used to go live the moment Rundock first ran over that workspace, silently duplicating work already handled by your own scheduler. A routine that never said whether it may run now waits until you turn it on, and names the risk directly: if a cron job or script already runs it, turning it on here runs it twice until you turn the other one off.
- **The Routines list and the routine editor read as finished, not as a first pass:** the scheduling sentence and a run's own file path could wrap onto extra lines even in a wide window, purely from how narrow the two screens were; both are wider now. The dropdown fields in the scheduling sentence, and the pill showing which skill is being scheduled, now read as real controls rather than blending into the sentence around them, in both light and dark mode. The Routines list's header no longer reads as an undersized icon beside a wall of text. A skill's own schedule box is now labelled Routine, matching the one routine most skills will have, and still says Routines where a skill genuinely has more than one.
- **A run's own record now says when it happened:** the "View last run" screen described how a run ended, and how long it took, without ever saying when it started, whatever the outcome. It now states the run's own date and time, and the box's text no longer sits flush against its own edge.
- **The scheduling step says less:** what happens to a routine while another workspace is open, and which time zone it reads its schedule in, used to sit in three separate paragraphs of near-identical grey text below the schedule picker. They are now one paragraph, and shorter, with every fact the original said still in it.
- **Routines now say which workspace they belong to:** with routines in more than one workspace, only the open one runs and the rest sit dormant with no indication of that anywhere. The Routines list now names the workspace its rows came from and says plainly when Rundock has moved elsewhere, dropping next-run times and the Turn-on control rather than promising work that will not happen. This is deliberate: running routines out of a workspace you have not opened would mean agents acting under permissions for files you are not looking at.
- **A routine with a schedule Rundock cannot read now says so:** a cron line or other unsupported format used to be accepted and then silently skipped forever, with no warning and a routine that looked like it was about to run. Its row now says the schedule is unreadable and names the two formats that work.
- **A workspace whose only skill has no agent assigned is now told to assign it, not build a new one:** the routines empty state could not tell "no skill exists" from "a skill exists and nobody has it," so it always suggested building one. It now asks the narrower question, and a skill's own page explains why scheduling is unavailable when no agent holds it yet.
- **Navigation and display fixes:** the sidebar icon now always matches the screen you are on; switching workspace no longer hides the New Conversation button; and the workspace picker no longer appears with a stale sidebar drawn underneath it.
- **Agent output can no longer run code in the app:** anything an agent wrote into a conversation or a note was rendered as literal page markup rather than words to read, reachable even without your involvement since routines make agents produce content on a schedule. Six injection paths are closed; ordinary markdown renders exactly as before, checked character by character against the previous output. A tag or highlight inside a fenced code block, and a tag at the start of a line, are also no longer mishandled by the renderer.

### Changed

- **The always-on setup guide now correctly explains the double-run risk:** it previously blamed your sync tool for a second running instance firing every routine again; the real cause is that each running Rundock only reads the last-run guard at startup. The guide now names the actual mechanism and the fix: let one machine run routines and reach it from your other devices.
