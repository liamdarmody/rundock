...
- **Approve a routine's plan once, not on every run:** the first run of a new or edited routine shows you what it is about to do and asks you to approve it, and then never asks again unless the plan itself changes. The approval is recorded beside the routine in the agent file, so it travels with the workspace and survives restarting Rundock, and it lapses the moment an edit changes what would actually run. Routines you already had keep running as they are: approval applies to plans made or changed from here on, not as a toll on upgrading.
