...
Its provenance is checkable rather than asserted.
`node test/tools/regenerate-benign-before.js --check` cuts the markdown section
out of `public/app.js` as it stands at the branch base, reads it out of git
history, evaluates it against the same marked build the browser gets, and exits
non-zero if the committed fixture is not what that code produces. The test
`the frozen before fixture really is the old renderer's output` runs it, so the
comparison is part of the suite and not a claim in a document.
...
its reason in the source. The fourth is a fix that review caught being described
as a preservation: the old path read the href back out of finished HTML, where
...
`Renderer guard mutation` job runs it on every pull request. It is deliberately
...
One row was queried in review and holds, with a correction to the reasoning
against it. The `href written as an attribute value` row was read as untestable
on the grounds that every payload in `a script scheme cannot reach an href or a
src` is refused by `isNavigableHref` before an `<a href>` is written. That is
...
`javascript:` href. Running the mutation shows the test go red. The row was
right and hard to see, so `an ordinary link keeps its destination exactly as the
...
```
markdown-render.js      |  99.83 |    84.69 |   96.97 | 90
all files               |  98.49 |    87.22 |   95.85 |
```
