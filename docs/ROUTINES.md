...
Three surfaces say so. The Routines list names the workspace it is showing and states that the routines in your other workspaces are not running. A row for a routine read out of a workspace this window does not have open shows no next-run time and names the workspace the routine is actually in, and is offered no Turn on control, because pressing it would start nothing. And the scheduling step of the routine editor says the same thing to anybody setting a routine up, which is the surface every route into a new routine passes through.
...
**Your sync tool does not change that, and it is worth being clear about why.** The guard the tick reads is the in-memory copy described under [Scheduler behaviour](#scheduler-behaviour): each instance fills it when it starts, and again if you switch workspace, and writes it back in full after each of its own runs. Nothing on the tick path reads the file again. So the second instance decides using a copy that was current when it started and has not moved since, and it fires whether or not `.rundock/routine-state.json` reached it, and whether it reached it in a second or an hour. (Pinned by `test/unit/scheduler-lib.test.js`, which runs two schedulers against one shared directory and watches both fire.) For the record, a workspace shared through git does not carry `.rundock/` at all: Rundock adds it to the workspace `.gitignore` when it sets one up.
