...
function scaffoldWorkspace(dir, opts = {}) {
  ...
  try {
    ...
    if (!('sandbox' in settingsLocal)) {
      if (desired) { settingsLocal.sandbox = desired; dirtySandbox = true; }
    } else if (ours && desired && !sameSandbox(settingsLocal.sandbox, desired)) {
      settingsLocal.sandbox = desired;
      dirtySandbox = true;
    } else if (ours && !desired) {
      // Withdrawn, not just updated. A workspace scaffolded on macOS and
      // opened on a platform Rundock writes no block for was still handing
      // the runtime a block with a macOS absolute root. Recognising our own
      // block only in order to update it left it in place exactly where it
...
function isRundockSandbox(block, platform = 'darwin') {
  ...
  for (let i = 0; i < expectedHead.length; i += 1) {
    if (roots[i] !== expectedHead[i]) return false;
  }
...
// both directions.
function setSandboxOptOut(dir, off) {
  ...
  let dirty = false;
  if (!('sandbox' in settingsLocal)) {
    if (desired) { settingsLocal.sandbox = desired; dirty = true; }
  } else if (ours && desired && !sameSandbox(settingsLocal.sandbox, desired)) {
    settingsLocal.sandbox = desired; dirty = true;
  } else if (ours && !desired) {
    delete settingsLocal.sandbox; dirty = true;
  }
  if (dirty) {
