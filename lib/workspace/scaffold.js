...
function isRundockSandbox(block, platform = 'darwin') {
  ...
  // raw temp-directory name alone, or that name paired with its own
  // /private real path (macOS's /var and /tmp are symlinks into /private).
  // A root a person appended is neither, which is what stops it from being
  // structurally indistinguishable from a legitimate second tail entry.
...
// docs/ARCHITECTURE.md's boundary section): fourteen subsystems under
...
// setSandboxOptOut: this is the rule that decides whether Rundock ever
// touches a block a person wrote, and having it live twice meant a fourth
// case or a changed authorship test added to one copy could leave the other
// deciding differently, so a workspace could reconcile one way on open and
// another way through the switch.
//
...
// The per-workspace opt-out for the operating-system write block. Read from
// the workspace's own state file directly rather than through the store
// module, because this runs during scaffolding, before the workspace is
// necessarily the current one, and the setting must belong to the folder it
// describes.
//
// WHAT OPTING OUT WITHDRAWS, so the setting's copy has something true to
// say: the macOS write refusal at the operating-system level. The approval
// card is untouched; it remains the whole boundary, exactly as it already is
// on Windows and Linux. And what the block can never help either way:
// a command sandbox refuses some process-launch primitives categorically
// (a headless browser fails at its startup check-in before any file is
// touched), which no writable-roots entry can reach, so a workspace whose
// tooling launches browsers is the workspace this switch exists for.
function sandboxOptedOut(dir) {
  try {
    const state = JSON.parse(fs.readFileSync(path.join(dir, '.rundock', 'state.json'), 'utf-8'));
    return state.sandboxOptOut === true;
  } catch (e) { return false; }
}

// The switch's writer, and the immediate reconcile that makes it take effect
// without waiting for the next workspace open. Only Rundock's own block is
// ever removed; a block a person wrote stays exactly as they wrote it, in
// both directions.
// `platform` is the same defaulted seam every sibling in this feature takes
// (sandboxSettings, isRundockSandbox's caller in scaffoldWorkspace,
// handleGetSandboxMode/handleSetSandboxMode): a test can drive either arm on
// any host. Without it here, the platform argument handleSetSandboxMode was
// explicitly given got honoured only for its own refusal check and silently
// dropped for the write this function performs, so a call that passed
// 'darwin' on a non-darwin test host still computed a null desired block.
function setSandboxOptOut(dir, off, platform = process.platform) {
  const stateFile = path.join(dir, '.rundock', 'state.json');
  let state = {};
  try { state = JSON.parse(fs.readFileSync(stateFile, 'utf-8')); } catch (e) { /* fresh */ }
  if (off) state.sandboxOptOut = true;
  else delete state.sandboxOptOut;
  fs.mkdirSync(path.dirname(stateFile), { recursive: true });
  fs.writeFileSync(stateFile, JSON.stringify(state, null, 2));

  const settingsLocalPath = path.join(dir, '.claude', 'settings.local.json');
  let settingsLocal = {};
  try { settingsLocal = JSON.parse(fs.readFileSync(settingsLocalPath, 'utf-8')); } catch (e) { /* fresh */ }
  ...
  return { optedOut: !!off, blockPresent: 'sandbox' in settingsLocal };
}

module.exports = {
  muteHooks, isEmptyWorkspace, detectWorkspaceMode, scaffoldDefaults, scaffoldWorkspace,
  sandboxSettings, isRundockSandbox, wireScaffoldDeps,
  sandboxOptedOut, setSandboxOptOut, tempRoots,
};
