...
function scaffoldWorkspace(dir, opts = {}) {
  ...
  try {
    ...
    if (!settingsLocal.sandbox) {
      const sandbox = sandboxSettings(dir);
...
//
// THE HOOK AND THIS ARE DIFFERENT INSTRUMENTS AND BOTH ARE NEEDED. The hook
// classifies a tool call before it runs, which means reading the command
// text, which cannot decide what a shell command writes. This decides at
// syscall time and reads nothing. Where it is available it is the guarantee;
// where it is not, the hook's escalation is the best that exists and the seam
// is stated in the product copy rather than papered over.
//
// EVERY VALUE HERE WAS MEASURED against the CLI on 2026-08-22, because the
// question "does the pinned version expose this" had no answer: Rundock does
// not pin a version at all. It runs whatever `claude` resolves to on the
// user's PATH (see resolveClaudeBin in lib/runtime/claude.js), so the setting
// is available or not on the USER'S machine, not on one this project chose.
// A version that ignores the block ignores it harmlessly.
//
// WHY THE NETWORK IS LEFT OPEN, and it is not an oversight. Enabling the
// sandbox also isolates the network. Measured with no allowedDomains, EVERY
// outbound host is refused for shell commands: npm install, git push and
// curl all fail, and a network denial does not produce the retry a filesystem
// denial does, so the agent simply reports failure. "*" restores the network
// while leaving the filesystem boundary fully intact. Narrowing it later is a
// deliberate product decision about which hosts an agent may reach, not a
// tightening of this boundary, and it breaks working commands on the day it
// lands.
//
// WHY THE PACKAGE CACHE IS WRITABLE. Measured: without it `npm install`
// fails inside its own cache directory, which npm reports as a file-ownership
// error rather than a permission denial, so it does not even read as a
// sandbox problem. Only npm's cache is listed. Other ecosystems keep caches
// elsewhere and will meet the boundary; that is a real limit, stated in the
