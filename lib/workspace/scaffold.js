...
function workspaceModeFor(dir) {
  try {
    ...
    return state.workspaceMode === 'code' ? 'code' : 'knowledge';
...
// opt-out state of its own, and no route to the block other than mode. Only
// Rundock's own block is ever touched; a block a person wrote stays exactly
// as they wrote it, in both directions.
...
function reconcileSandboxForMode(dir, mode, platform = process.platform) {
  ...
  return { blockPresent: 'sandbox' in settingsLocal };
}
...
module.exports = {
  ...
  workspaceModeFor, reconcileSandboxForMode, tempRoots,
};
