...
function isRundockSandbox(block, platform = 'darwin') {
  ...
  // No count check here: the comparison at the end is against the WHOLE
  // block, so a block carrying an extra root fails it already. A length
  // guard was written here and removed when mutating it turned no test red.
  ...
  // labelled so nobody credits it with safety: mutating it turns no test red,
  // because a wrong home simply regenerates a block that fails the comparison
  // below. The comparison is what decides, here as everywhere in this
  // function.
  const suffix = path.sep + '.npm';
  ...
  // Rundock is the author only if this is EXACTLY what it would have written
  // for that workspace and that home. Anything else is somebody's edit and
  // is not ours to touch.
  return sameSandbox(block, sandboxSettings(claimedWorkspace, platform, claimedHome));
}
