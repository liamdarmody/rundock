...
function isRundockSandbox(block, platform = 'darwin') {
  ...
  return priorDenyShapes(rebuilt).some((shape) => sameSandbox(block, shape));
}
...
function denyWriteFor(workspaceDir) {
  return [
    path.posix.join(workspaceDir, '.rundock', 'state.json'),
    // permissions.json holds the standing grants: folders the user allowed
    // outside the workspace, and the tools they allowed without a card. Both
    // are answers a person gave to a permission question, and an agent that
    // could write them could answer those questions on the user's behalf.
    // Same reasoning as state.json, same protection.
    path.posix.join(workspaceDir, '.rundock', 'permissions.json'),
  ];
}
