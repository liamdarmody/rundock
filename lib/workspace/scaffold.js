...
function scaffoldWorkspace(dir, opts = {}) {
  ...
  try {
    ...
    } else if (desired && isRundockSandbox(settingsLocal.sandbox, platform)
        && !sameSandbox(settingsLocal.sandbox, desired)) {
...
function isRundockSandbox(block, platform = process.platform, home = os.homedir()) {
  if (!block || typeof block !== 'object' || Array.isArray(block)) return false;
  const roots = block.filesystem && block.filesystem.allowWrite;
  const claimed = Array.isArray(roots) ? roots[0] : null;
  if (typeof claimed !== 'string' || !claimed) return false;
  return sameSandbox(block, sandboxSettings(claimed, platform, home));
}
