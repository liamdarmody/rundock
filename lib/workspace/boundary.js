...
const { canonicalize, isSecretPath, isWorkspaceAnswerFile } = require('../../scripts/permission-hook.js');
...
function crossingCovered(crossing, home = os.homedir()) {
  ...
  // NOR THE WORKSPACE'S OWN ANSWER FILES, for the same reason and by the same
  // shape as the secrets short-circuit above.
  //
  // A stored folder grant covers its whole subtree. A person who granted a
  // parent of their workspace, which is an ordinary thing to do, would have
  // silenced the card for the two files that hold their own permission
  // answers: the grant would be answering for the mechanism that records the
  // answers. Recomputed here from the path and the current workspace rather
  // than read from any flag the hook attached, because this is the point of
  // decision and it must not depend on what arrived over the wire.
  if (isWorkspaceAnswerFile(crossing.path, getWorkspace())) return false;
