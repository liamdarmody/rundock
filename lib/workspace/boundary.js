...
function addToolAllow(key) {
  try {
    ...
    const next = { ...readPermissionsFile(), allowedDirs: readBoundaryGrants(), allowedTools: [...current, k] };
    if (!writePermissionsFile(next)) return current;
    console.log(`[Permission] Standing tool allow added for this workspace: ${k}`);
    return next.allowedTools;
  } catch (e) {
...
function removeToolAllow(key) {
  try {
    ...
    if (!writePermissionsFile({ ...readPermissionsFile(), allowedDirs: readBoundaryGrants(), allowedTools: kept })) return current;
