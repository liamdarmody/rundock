...
// The directories the state lives in, deepest first, so a run that had to
// create them can record that fact and recovery can remove exactly those.
...
// The one containment rule every boundary in this module uses: is `child`
// strictly inside `parent`? Never true for equality, an escape, or a path on
// another root.
...
// Containment above is lexical, and a symlinked directory between the root
// and a destination would let a lexically-inside path act on a resolved path
// outside the workspace. So every existing component on the way down is
// lstatted, and a symlink anywhere refuses the path.
function assertNoSymlinkComponents(root, resolved, raise) {
  let current = root;
...
function assertInside(root, destination, label) {
  ...
  if (!strictlyInside(root, resolved)) throw new TypeError(`${label} must be inside the workspace`);
  // A destination may not be the transaction state, live inside it, or
  // contain it: a write that reaches `.rundock` owns the journal and the
  // backups of its own run, and can destroy its own recovery.
  const state = importRoot(root);
  if (resolved === state || strictlyInside(state, resolved) || strictlyInside(resolved, state)) {
    throw new TypeError(`${label} cannot use transaction state`);
  }
  assertNoSymlinkComponents(root, resolved, (why) => {
...
function normalisePlan(workspace, writes, options) {
  ...
  for (const entry of entries) {
    for (const other of entries) {
      if (entry !== other && strictlyInside(entry.destination, other.destination)) {
...
function fromRelative(root, relative, label) {
  ...
  if (!strictlyInside(root, resolved)) throw invalidJournal(`${label} escapes the workspace`);
  const state = importRoot(root);
  if (resolved === state || strictlyInside(state, resolved) || strictlyInside(resolved, state)) {
    throw invalidJournal(`${label} uses transaction state`);
  }
  assertNoSymlinkComponents(root, resolved, (why) => {
...
// The exclusive create IS the single-transaction guard: it fails on anything
// already at the journal path, so two runs racing this line cannot both win,
// and the loser leaves whatever is there byte-for-byte untouched.
...
// A journal update always lands whole: written beside its final name under a
// temporary name unique to this run, then renamed over it, so no reader can
// observe a half-written journal and no other run can collide on the name.
...
// restore that cannot finish is a restore that never started.
...
function writeAsUnit(workspace, writes, options = {}) {
  ...
  try {
    ...
    // A run directory without a journal is leftover temporary state from a
    // run that never became recoverable; it holds no originals, so clear it.
    fs.rmSync(runRoot(root), { recursive: true, force: true });
  ...
  try {
    ...
    // The preflight runs before the first removal, so a rollback that could
    // not complete never destroys an original; if it refuses, the journal
    // stays on disk and recovery owns the repair.
