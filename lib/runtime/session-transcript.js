...
function decipher(tool, result) {
  ...
  const path = result[FILE_TOOLS[tool].result];
  ...
  if (result.type === 'create') return { path, change: 'created' };
  if (result.type === 'update') return { path, change: 'edited' };
  // No type at all is the edit shape. A Write always reports one, so a Write
  // arriving without it is a shape that has moved.
  if (result.type === undefined && tool !== 'Write') return { path, change: 'edited' };
  return null;
}
...
function payloadFor(entry, block, results, asked) {
  ...
  const match = payloads.find(p => p && typeof p === 'object' && call && p[FILE_TOOLS[call.tool].result] === call.path);
 ...
 * file), or 'unresolved' (a write was asked for and never came back).
 *
...
module.exports = { readSessionTranscript, FILE_TOOLS, KNOWN_BLOCK_TYPES };
