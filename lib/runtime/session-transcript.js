  ...
  for (const line of text.split('\n')) {
    ...
    for (const block of blocks) {
      ...
      if (block.type === 'text' && typeof block.text === 'string' && block.text.trim()) {
        ...
        // capture witnesses on a refused write. It is checked with === true
        // rather than for truthiness so that a field which stops being a
        // boolean is not read as a refusal by accident.
