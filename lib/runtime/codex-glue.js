...
async function openCodexThread(entry, convoId, resumeThreadId, model) {
  ...
  // NAMED WORKING FOLDERS DO NOT REACH CODEX, and this is the reason.
  // `workspace-write` is scoped by the CLI to `cwd` alone; the folders a
  // workspace names are carried to Claude agents through the Rundock
  // permission hook, which Codex never runs. So a Codex thread reaching a
  // named folder still raises the CLI's own approval, exactly as before.
  // Extending it means an app-server option for additional writable roots,
  // which is a different surface and unverified against a live CLI, so it is
  // deliberately not guessed at here. Recorded rather than left to be
  // rediscovered by someone wondering why the setting appears to do nothing.
  const threadOpts = {
    cwd: getWorkspace(),
