...
function buildToolSummary(toolCalls) {
  ...
  for (const tc of toolCalls) {
    ...
    seen.add(key);
    // ON ONE LINE, ALWAYS. The argument is the agent's own command text, and a
    // heredoc or a `python3 -c` block carries real newlines. Embedded verbatim,
    // the agent's newline ended the summary's first line inside a group and
    // left its bracket unclosed, so the line no longer read as a summary and
    // the whole of it was shown to the reader as the agent's words. Measured on
    // 173 real summary-carrying turns: this was the cause of both failures.
    //
    // Whitespace is collapsed rather than the newline dropped, so a multi-line
    // command reads as one line instead of losing the gap between its words.
    // AND NO BRACKETS OF ITS OWN. Brackets are this function's delimiters, and
    // the argument is arbitrary command text, so any bracket inside it is
    // indistinguishable from a group boundary. Two ways that bit: an argument
    // TRUNCATED mid-class left `[^` unclosed, and a command carrying a lone
    // `]` closed a group early. Either way the line stopped reading as a
    // summary and the whole of it was shown as the agent's words.
    //
    // Substituted rather than removed, so a regex in a command is still legible
    // as a regex. A summary is already a lossy view of what ran, being both
    // truncated and de-duplicated; this makes it a parseable one.
    const arg = tc.arg
      ? String(tc.arg).replace(/\s+/g, ' ').replace(/\[/g, '(').replace(/\]/g, ')').trim()
      : '';
    parts.push(arg ? `[${tc.tool} ${arg}]` : `[${tc.tool}]`);
 ...
 */
function deltaSince(transcript, agentId, cap = DELTA_CAP_CHARS, alsoExclude = [], names = null, arriving = false) {
  ...
  // NEVER HERE BEFORE. It has missed nothing in the sense this function was
  // written for, and the comment here used to say the cold-spawn path gives a
  // first-time delegate the history it needs. That was true of a delegate
  // spawned from a chat message and false of one brought in by an intercepted
  // Agent call, which receives the orchestrator's brief and nothing else.
  //
  // A brief is written at one moment about one task. Observed: a lead arrived
  // knowing only "research this site", was later asked to hand work back for
  // the next iteration of a post written earlier in the same conversation,
  // could not see that post, and declared the work finished. The agent that
  // did hold it was then silenced on the strength of that declaration.
  //
  // `arriving` asks for the whole conversation instead of nothing, through the
  // same capping, attribution and omission notice. The caller decides, because
  // only the caller knows whether a full transcript is already going.
  if (lastOwn === -1 && !arriving) return none;
  ...
  const skip = new Set([
    // AN ARRIVING AGENT HAS NO TURNS OF ITS OWN TO LEAVE OUT, whether or not
    // the transcript happens to contain some.
    //
    // Excluding by id was first gated on `lastOwn === -1`, which is only the
    // case where the agent has never spoken. An agent that HAS spoken and is
    // now arriving cold, by an intercepted spawn with no session to resume,
    // holds none of that in context: it is a new process that has never seen
    // the conversation. Skipping its earlier turns took away exactly the part
    // it most needed, which is the defect this whole change exists to fix,
    // reappearing for an agent the second time it is delegated to.
    ...(arriving || lastOwn === -1 ? [] : [agentId]),
    ...(Array.isArray(alsoExclude) ? alsoExclude : []),
  ].filter(Boolean));
  ...
  // FROM THE BEGINNING FOR AN ARRIVAL, from its own last turn for a return.
  // The slice and the skip above answer the same question and must agree: a
  // resume carries its own history and needs only what it missed, while an
  // arrival carries nothing and needs the conversation. Slicing at lastOwn for
  // an arriving agent handed it the tail of a conversation whose beginning it
  // had never seen.
  const missed = transcript.slice(arriving ? 0 : lastOwn + 1)
...
// THE ONE WAY TO DROP A TOOL SUMMARY, used by everything that needs to.
//
// A turn is stored as Rundock's own summary of the tools it ran, then a
// newline, then what the agent wrote. Dropping that prefix by pattern does not
// work: a regex of the shape `^(\[.*?\]\s*)+` stops at the FIRST `]`, which
// inside `[Bash grep -o '<title>[^<]*</title>' ...]` is the one closing the
// character class, and the rest of the summary then survives as visible text.
// Measured on a real turn, where it was shown to the reader as the agent's
// words and also fed to the rebuilder as the prefix it looks the turn up by,
// so the lookup failed and fell back to the same broken strip.
//
// Bracket DEPTH answers it, which isToolSummaryLine already does. Only the
// first line is considered, because that is where the summary is written.
//
// KNOWN LIMIT, and it is the honest boundary of this approach. Depth counting
// needs the brackets to balance, so a command carrying an UNBALANCED literal
// one, `sed -n '/\]/p'` being the real example, still reads as not-a-summary
// and the whole prefix is shown. That is a different failure from the regex's
// (which showed a fragment) and it is not fixed here: a summary cannot be
// parsed back out of arbitrary command text reliably, and the sound answer is
// to stop gluing it onto the text at all and give it its own field. That is a
// transcript-format change and its own card. What this function does is make
// the common case right and the remaining case visible, rather than leaving
// both wrong in ways nobody had written down.
function stripToolSummaryPrefix(text) {
  const raw = String(text || '');
  const nl = raw.indexOf('\n');
  const first = nl === -1 ? raw : raw.slice(0, nl);
  if (!isToolSummaryLine(first)) return raw;
...
function renderDeltaEntryBody(t) {
  ...
  const body = stripToolSummaryPrefix(t.text);
...
module.exports = {
  ...
  deltaSince, DELTA_CAP_CHARS, stripToolSummaryPrefix, isToolSummaryLine,
};
