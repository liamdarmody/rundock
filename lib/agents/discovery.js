  ...
  for (const agent of agents) {
    if (agent.routines) {
      for (const r of agent.routines) {
        ...
        // WHETHER A RUN OF IT IS GOING, stamped by this same enrichment for
        // the same reason the refusal is: the row's Run control promises to
        // start a run, and disables itself while one is in flight, and the
        // only copy of that fact that cannot drift is the scheduler's own.
        // Keyed the way the hold is keyed, so namesakes share it as they
        // share the hold. Null when nothing is going, never absent, so a
        // roster from a server that predates the field is told apart from a
        // routine that is idle. Carries the trigger so the row can say a
        // pressed run is going without opening the record; the state slot
        // says nothing about a pressed run by design.
