...
// An agent runs on Codex only if it asked to AND is allowed to: orchestrators
// and platform agents are forced onto Claude Code because delegation needs
// Claude Code's Agent tool. Expressed once so the runtime and model decisions
// cannot disagree about which agents those are.
function isCodexAgent(meta) {
  return meta.type !== 'orchestrator' && meta.type !== 'platform'
    && String(meta.runtime || '').toLowerCase() === 'codex';
}

// Names nothing, or names `inherit`: the same statement either way, turned into
// whichever spelling of "no model" the agent's runtime uses. The predicate
// itself is owned in lib/config.js, shared with the spawn layer.
function resolveModel(meta, codex) {
  if (namesNoModel(meta.model)) return codex ? null : 'inherit';
  return meta.model;
}
...
function discoverAgents() {
    ...
    for (const file of files) {
      try {
        ...
        agents.push({
          ...
          runtime: isCodexAgent(meta) ? 'codex' : 'claude',
          // `inherit` is a Claude Code value: it means "pass no --model".
          // Codex has no such token and receives the model field verbatim
          // (lib/runtime/codex-glue.js openCodexThread), so `inherit` there
          // becomes a literal model name Codex cannot resolve. Its equivalent
          // is null, which omits the field, so normalise rather than trust
          // three prose files to keep agreeing with each other.
          // Naming nothing and naming `inherit` are the same statement, and
          // resolve identically so no surface has to know which the file used.
          // Each runtime spells "no model" its own way: Codex omits the field
          // (null), and the Claude runtime carries `inherit` through to
          // modelArgs, which passes no --model. Codex receives the model field
          // verbatim, so the literal word `inherit` must never reach it: that
          // normalisation lives here, in code, because three prose files were
          // relied on for it and disagreed with each other.
          model: resolveModel(meta, isCodexAgent(meta)),
          order: orderNum,
  ...
  if (!agents.find(a => a.isDefault)) {
    if (fs.existsSync(claudeMdPath)) {
      ...
      agents.unshift({
        ...
        prompts: null,
        runtime: 'claude',
        // Synthesised in code with fileName: null, so a user cannot edit this
        // agent's frontmatter to name their own model. It inherits instead:
        // on a machine that reaches models only through a gateway, an agent
        // nobody can edit and that hardcodes a model nobody can serve is an
  ...
  if (!agents.find(a => a.type === 'platform') && !agents.find(a => a.id === 'rundock-guide')) {
    agents.push({
      ...
      runtime: 'claude',
      // Same reasoning as the default agent above: no file, so inherit.
      // This is the fallback Doc, the first agent a new user meets.
      model: 'inherit',
      order: 99,
      instructions: '',
      isDefault: false,
  ...
  const { routineState, routineDisplayFacts, routineRefusal, runningRuns } = require('../scheduler.js');
  // The runs going right now, read once per pass from the scheduler's own
  // live set, which is the set the single-flight hold is released through.
  ...
  for (const agent of agents) {
    if (agent.routines) {
      for (const r of agent.routines) {
        ...
        // Whether the scheduler's own grammar can read this routine's
        // schedule. Carried so the row can say a routine will never fire,
        // which until now was known only to a tick that skipped it in silence.
        r.scheduleReadable = facts.scheduleReadable;
        // THE TICK'S OWN REFUSAL, published rather than mirrored: the row's
        // offer consumes this one answer, so a reason added to the scheduler
        // reaches the row through the roster instead of through a comment.
        r.refusal = routineRefusal(r);
        // WHETHER A RUN OF IT IS GOING, stamped here for the reason the
        // refusal is: the only copy that cannot drift is the scheduler's.
        // Keyed as the hold is, so namesakes share it. Null when idle, never
        // absent, so a roster predating the field is told apart. Carries the
        // trigger, already normalised by the scheduler at the start, because
        // the row's live line reads it; nothing else until a reader exists.
        const going = live.find(run => run.key === key);
        r.running = going ? { trigger: going.trigger } : null;
        // THE WORKSPACE THIS ROUTINE WAS READ OUT OF, which is the one it
        // would run in: the tick runs a routine with the workspace root as its
        // working directory, and there is one tick.
        //
        // Carried because the roster and the workspace root are two facts that
        // reach a client in two different messages, and a client can hold a
        // roster taken from a workspace it did not open itself. Every
        // connected client is sent the roster when a routine's state changes,
        // so a second window sees the first window's workspace. Without this
        // field a row there says when a routine runs next, and it is not the
        // routine of the workspace that window is showing.
        r.workspace = ws;
      }
    }
  }
...
/**
 * The roster message, with the workspace the roster was read out of.
 *
 * ONE BUILDER, BECAUSE THE TWO HALVES ONLY MEAN ANYTHING TOGETHER. A client
 * decides whether a routine is being served by comparing the workspace its
 * roster came from against the workspace the server says it is serving, and it
 * reads an absent workspace as silence rather than as a fault. So a roster sent
 * without one does not fail anywhere: it quietly turns the check off for that
 * window, and no test at that call site notices.
 *
 * The roster reaches clients from eight places across the two protocol handler
 * modules and the scheduler's broadcast. Hand-built, the ninth would be written
 * without the field by somebody who had no reason to know it mattered. Built
 * here, it cannot be.
 *
 * @param {any[]} [agents] the roster to send; discovered afresh when omitted,
 *   which is what most callers want and all of them used to write out.
 */
function rosterMessage(agents) {
  return {
    type: 'agents',
    agents: agents || discoverAgents(),
    // Read at USE time, like everything else that names the root, so a switch
    // redirects the very next message.
...
// The names people call the agents by, keyed by the id a transcript stores.
//
// Lives here because it is a roster question, not a delegation one, and the
// delegation module's export surface is deliberately frozen to its factory and
// deps: widening it so a test could reach this function would trade an
// architectural boundary for a test's convenience.
//
// Built per call rather than cached: a workspace's roster changes while it is
// open, and a delta labelled with a name that has since changed is worse than
// one labelled with an id, because it looks right.
function agentDisplayNames() {
  const names = {};
  try {
    for (const a of discoverAgents()) {
      if (a && a.id) names[a.id] = a.displayName || a.name || a.id;
    }
  } catch (e) { /* a delta labelled by id still reads; this is a nicety */ }
  return names;
}

module.exports = {
  agentDisplayNames,
  discoverAgents, rosterMessage, invalidateAgentCache, agentIsDefault,
