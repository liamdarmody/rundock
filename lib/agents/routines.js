...
function appendRoutineBlock(content, routine) {
  const name = routine && routine.name === undefined ? null : unquote(routine && routine.name);
  ...
  if (!section) {
    // No section yet. It goes at the END of the frontmatter, which is where a
    // hand edit would put a new key and which leaves every existing key in the
  ...
  return updateRoutineBlock(withBlock, name, {
