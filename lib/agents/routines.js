...
function appendRoutineBlock(content, routine) {
  ...
  const next = updateRoutineBlock(withBlock, name, {
    ...
    // BORN APPROVED, because writing a routine is consenting to it. There is
    // nobody else whose agreement was being collected here, and a routine its
    // author is unsure about can be paused. Arriving pending meant a row that
    // had just been created said "Waiting for your approval" beside a pause
    // icon implying it was already live, and the reader could not tell whether
    // it would fire at the time they had just set.
    //
    // THE LAPSE ON CHANGE IS UNTOUCHED, and it is the half that protects
    // somebody rather than interrupting them: agents can edit routines, and an
    // edited plan is a question nobody has answered. This path APPENDS a block
    // and editing UPDATES one, so consent recorded at birth can never stand in
    // for consent to a later change. The grandfather in the migration still
    // never fires for this block, because the key is present from birth.
...
// ===== HAS THE APPROVAL FEATURE PASSED OVER THIS WORKSPACE =====
//
// The grandfather can only grandfather what genuinely predates the feature,
// and absence of an approval record cannot tell a pre-feature routine from
// one whose record was hand-stripped after the feature: both are key-less.
// The durable signal that resolves it is a per-workspace marker written the
// moment the feature is known to have operated here. Once it exists, a
// key-less routine block is a later edit and reads as pending, never
// grandfathered; while it is absent, the workspace is in its one-time
// pre-feature state and existing routines carry their consent over.
//
// It lives in `.rundock/`, which is machine-local and gitignored: it is a
// fact about what this installation has done, not about the files, and the
// synced routines carry their own approval keys across machines. A machine
// that reads a synced file already bearing approval keys learns the feature
// ran (see migrateAgentRoutines), so it sets its own marker and stops
// grandfathering there too.
const APPROVAL_FEATURE_MARKER = path.join('.rundock', 'approval-feature-ran');

function workspaceOfAgentFile(filePath) {
  // <workspace>/.claude/agents/<name>.md -> <workspace>
  return path.resolve(path.dirname(filePath), '..', '..');
}

function approvalFeatureRan(workspaceDir) {
  try { return fs.existsSync(path.join(workspaceDir, APPROVAL_FEATURE_MARKER)); }
  catch (e) { return false; }
}

function markApprovalFeatureRan(workspaceDir) {
  try {
    const marker = path.join(workspaceDir, APPROVAL_FEATURE_MARKER);
    if (fs.existsSync(marker)) return;
    fs.mkdirSync(path.dirname(marker), { recursive: true });
    fs.writeFileSync(marker, new Date().toISOString() + '\n');
  } catch (e) { /* a workspace we cannot write is one we cannot mark; the read side fails safe */ }
}
...
function stampPendingApprovals(content) {
  ...
  let next = content;
  // Addressed by the UNQUOTED name and counted by it, exactly as the writer
  // and the migration match: a block written `- name: "morning briefing"`,
  // or a namesake written with different quoting, must be found and stamped
  // individually, not silently skipped. `unquote` is what readString and the
  // writer's own locator apply.
  const seen = new Map();
  for (const block of blocks) {
    const name = unquote(block.name);
    const occurrence = seen.get(name) || 0;
    seen.set(name, occurrence + 1);
    if (block.planApprovedHash !== undefined) continue;
    next = updateRoutineBlock(next, name, { planApprovedHash: APPROVAL_PENDING }, occurrence);
  }
  // VERIFY RATHER THAN TRUST. A file written unstamped is a file the
  // migration will grandfather into approval, so the one failure this cannot
  // wave through is a block that should carry the sentinel and does not:
  // a frontmatter this module cannot address (CRLF), or a block the writer
  // would not match. The caller is told by a throw, so it can refuse the
  // save rather than write a file that reads as consented-to.
  const after = parseRoutineBlocks(next.match(/^---\n([\s\S]*?)\n---/)[1]);
  for (const block of after) {
    if (block.planApprovedHash === undefined) {
      throw new Error('a routine block could not be stamped for approval; refusing to write it unstamped');
    }
  }
  return next;
}
...
function migrateAgentRoutines(filePath, content, opts = {}) {
  ...
  // Has the approval feature already passed over this file, captured ONCE
  // before anything is stamped. Two independent signals, either sufficient:
  // the workspace marker a writer sets when the feature operates, and the
  // file already carrying an approval record on any block (proof in its own
  // bytes, which also covers a file synced from a machine that migrated it).
  // Read only, never written here: a migration that mutated shared marker
  // state let one file's pass reach another's, and a genuinely pre-feature
  // routine must not lose consent because a sibling gained a record in the
  // same pass. The marker is owned by the writer paths, which know the real
  // workspace.
  const featureHasRun = approvalFeatureRan(workspaceOfAgentFile(filePath)) || fileHasApproval;
  const seen = new Map();
  for (const block of blocks) {
    ...
    for (const key of MIGRATED_KEYS) {
      ...
      // is refused, which is the lost-record rule. Only a wholly
      // key-less file is grandfathered.
      if (key === 'planApprovedHash') {
        // Grandfather only in the genuine one-time pre-feature state,
        // decided once for the whole pass: a key-less block on a file the
        // feature has already touched is a later edit or a lost record and
        // meets the step; a key-less block on a genuinely pre-feature file
        // carries its consent over, every routine in the file alike.
...
module.exports = {
  ...
  markApprovalFeatureRan, approvalFeatureRan,
