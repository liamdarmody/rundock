...
function migrateAgentRoutines(filePath, content, opts = {}) {
  ...
  for (const block of blocks) {
    ...
    for (const key of MIGRATED_KEYS) {
      ...
      // predates the feature is neither. Its person wrote the plan, or has
      // been running it, and an upgrade that halted or re-questioned any of
      // it would be the predating-routines defect in mirror image: a release
      // changing what your own files mean. So migration approves every
      // pre-existing routine's plan as it stands, switched on or not; the
      // approval step is met by plans made or changed from here on. No
      // planApprovedAt is invented: the honest record of when the person
      // approved a grandfathered plan is that there is no such moment.
