...
function findDirectReportMatch(agentId, toolInput) {
  ...
  //
  // BOTH FIELDS, because the handoff line is where the name now goes. This
  // release made `description` the sentence shown to the person, and told
  // agents to say who is taking the work in it. They do: a real call arrived
  // with no subagent_type, `description` reading "Handing to Sage to
  // fact-check...", and 3,526 characters of `prompt` that never named her. The
  // scan read the prompt alone, matched nothing, and the delegation silently
  // did not happen while the agent told the person it had. Reading only the
  // field that predates the handoff line is the whole of that defect.
