...
function buildSystemPrompt(agentData) {
  ...
  if (isOrchestrator) {
    ...
    delegationSection = [
      ...
      '- When you delegate, call the Agent tool in the same response, and say in one line who you are handing to and why ("Handing to Sage to verify the claims."). That line accompanies the tool call, it does not replace it. It is not optional: without it a team member appears in the conversation with no explanation, which reads to the person watching as though you vanished mid-task. It is also not the brief, which the next rule forbids narrating: who and why belongs in the chat, what to do belongs in the tool call.',
