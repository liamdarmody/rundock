...
// Who reports to this agent, by the one membership rule. Split out of
// findDirectReportMatch so a caller can ask whether an agent leads anybody at
// all without asking it to match something, and so the two answers can never
// come from two different ideas of the roster.
function directReportsOf(agentId) {
  const allAgents = discoverAgents();
...
function hasDirectReports(agentId) {
  return directReportsOf(agentId).length > 0;
}

function findDirectReportMatch(agentId, toolInput) {
  const directReports = directReportsOf(agentId);
  ...
  // Check the call's own words for a teammate's name, on a word boundary so a
  // substring cannot false-match.
  //
  // BOTH FIELDS, because either one can carry the name. `description` is the
  // handoff line shown to the person, and agents are told to say there who is
  // taking the work; `prompt` is the brief, and a name can appear in that
  // instead. Scanning only one of them makes the field a caller happened to
  // use decide whether its teammate is reachable, which nobody intended and
  // which fails silently: no delegation happens while the agent's own handoff
  // line says one did.
  const promptText = `${toolInput.prompt || ''}\n${toolInput.description || ''}`.toLowerCase();
  const escapeRegex = (s) => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  // Name, id and displayName, the same three the subagent_type branch accepts.
  // Matching fewer here meant the field a caller used decided whether its
  // teammate could be found, which is not a distinction anyone intended.
  for (const dr of directReports) {
    for (const candidate of [dr.name, dr.id, dr.displayName]) {
      if (!candidate) continue;
      const re = new RegExp(`\\b${escapeRegex(String(candidate).toLowerCase())}\\b`);
      if (re.test(promptText)) return dr;
    }
  }
...
function buildSystemPrompt(agentData) {
  ...
  const baseRules = [
    ...
    // WHICH MARKUP, not just which words. The rules above govern the prose and
    // said nothing about the syntax carrying it, so an agent wanting a
    // sub-heading inside a message had nothing to go on and reached for
    // MediaWiki (`=== Heading ===`), which this product does not render and
    // which Markdown has no equivalent of. Same shape as the FILE LINKS rule
    // below: name the syntax that renders here, and the ones that do not.
    '- Write Markdown, as used by Obsidian. Headings are # through ######, emphasis is * or _, and ==text== highlights. Never use the syntax of another markup language: no MediaWiki (=== Heading ===), no reStructuredText, no BBCode, no Textile. They are not rendered and appear to the reader as the raw characters you typed.',
    '',
    'PLATFORM RULES:',
    isCodeMode
    ...
    '',
    'SCRATCH FILES:',
    'For a working file you write and read back yourself, use the path in $TMPDIR (TEMP or TMP on Windows). Rundock points it at a scratch folder inside this workspace, so a file you put there is already inside the boundary and needs no approval to read again. Never write a literal /tmp path. A file written to /tmp sits outside the workspace, so reading back a file you created seconds earlier raises an approval card, and it raises a new one every time because a shell command that reaches outside cannot be covered by a standing folder grant. Clean up when you are finished.',
    '',
    'FILES IN .claude/ DIRECTORY:',
    'Files inside .claude/agents/ and .claude/skills/ are managed through SAVE_AGENT and SAVE_SKILL markers, not through Write, Edit, or Bash: use the markers for those two folders rather than writing them directly. THAT IS THE WHOLE OF THE RULE, and it has been over-read: it names those two folders, not the rest of .claude/, and it is about which mechanism edits an agent, not about a directory being untouchable. Everything else in this workspace, .claude/ included, is ordinary work.',
    '',
    'FILE LINKS:',
    ...
    'Rundock has a Connectors tab under Settings. It LISTS what this workspace can reach, gathered from four places: this workspace\'s own .mcp.json, the equivalent file the other runtime reads, and the two user-global versions of those, saying for each connector which runtimes reach it and whether it travels with the folder. It is a view, not a form: there is no field to fill in, and the tab hands adding a connector to the workspace guide, which may be you.',
    'To add or remove a workspace connector, use the SAVE_CONNECTOR and DELETE_CONNECTOR markers. DO NOT try to edit .mcp.json with Write, Edit or a shell command: that file is protected wherever it lives and every such attempt is refused, whatever the user approves, so trying and reporting the refusal wastes their time. The markers are how Rundock writes it for you, and they merge rather than replace, so a connector you add does not disturb the ones already there. To add: <!-- RUNDOCK:SAVE_CONNECTOR name=<server> --> then the entry as one JSON object, then <!-- /RUNDOCK:SAVE_CONNECTOR -->. To remove: <!-- RUNDOCK:DELETE_CONNECTOR name=<server> -->.',
    'Ask for what the server actually needs before writing one: a command and its arguments, or a URL, and whatever credentials it requires. CREDENTIAL VALUES DO NOT GO IN .mcp.json: that file travels with the workspace folder, git included, so a key written there reaches every clone and stays in the history. Their values belong in .rundock/mcp-secrets.json, keyed by server name, which is per-user and does not travel; name the key in the connector entry and put its value there. A connector is read when an agent starts, so it takes effect on the next one rather than in the conversation you are having, and say so instead of implying it is already live. Say plainly what is still missing if something is.',
    'Account connectors added at claude.ai are a different thing: they reach every workspace on the machine, Rundock cannot read their state, and they are managed at claude.ai under Settings then Connectors. If a connector tool you expect is missing or unavailable, say so plainly and name the connector: its authorisation has usually lapsed or was never completed, and reconnecting there then starting a fresh conversation picks the tools up. Never invent Rundock settings, panels, or menus beyond the Connectors tab described above.',
    '',
    'TIMEZONE:',
  ...
  const bashRules = [
    ...
    'The same applies to reading and writing files: attempt the operation rather than declining in advance on a guess about what is permitted. The marker rule above is the one real exception, and it names two folders. Beyond it, a refusal you invent before trying states as fact something you have not checked, and it has been wrong in both directions: declining to touch this workspace at all, and explaining the refusal with machinery that is not what stopped it. If something is genuinely refused you will be told so, with a reason worth repeating. If you are unsure whether something is allowed, the way to find out is to try it.',
    '',
    // WAS A LIST OF COMMANDS DECLARED UNSUPPORTED, and it contradicted both
    // rules above it before it also became untrue.
    //
    // It said destructive commands and piped install scripts "are not supported
    // and will not reach the user for approval", which invited exactly the
    // pre-emptive refusal the paragraph above forbids: agents substituted `rm`
    // and `rmdir` for `rm -rf` rather than attempting it, and then told the user
    // the product does not stop destructive commands, because from inside a turn
    // an approved card is indistinguishable from no card at all.
    //
    // Then 0.13.3 made those commands reach the user, and the sentence was
    // false as well as harmful: an agent repeating it tells a person something
    // untrue about their own permission system, with Rundock's authority
    // behind it.
    //
    // NOTHING REPLACES IT. The rule above already covers this correctly:
    // attempt the command and let the user decide. A special case here only
    // invites an agent to theorise about machinery it cannot see.
  ].join('\n');
  ...
  // NOT named hasDirectReports: this asks whether a roster was rendered for the
  // prompt, which the exported hasDirectReports() does not. Sharing the name
  // shadowed the function inside this scope, so a later call to it here would
  // have found a boolean and failed on a line that looked correct.
  const hasRenderedRoster = !!directReportRoster;
  ...
  if (isOrchestrator) {
    ...
    if (roster) {
      delegationSection = [
        ...
        '- The Agent tool\'s `description` field IS YOUR HANDOFF LINE and the user sees it in the conversation. Write it as a full sentence in your own voice naming who you are handing to and why ("Handing to Penn to draft the post."), not a task label. The `prompt` field is the private brief for the specialist and is never shown to the user.',
        '- A one-sentence handoff in your visible response is welcome too, and your own words win when you write them: the field is the floor, not a replacement.',
    ...
    delegationSection = [
      ...
      '- When you delegate, the Agent tool\'s `description` field IS YOUR HANDOFF LINE and the user sees it in the conversation. Write it as a full sentence in your own voice naming who you are handing to and why ("Handing to Sage to verify the claims before this goes out."), not a task label. Without it a team member appears with no explanation, which reads to the person watching as though you vanished mid-task.',
      '- The `prompt` field is the private brief for your team member and is never shown to the user. Who and why goes in `description`; what to do goes in `prompt`.',
      '- A handoff sentence in your visible response is welcome too, and your own words win when you write them: the field is the floor, not a replacement.',
      ...
      '- Only delegate to the team members listed in YOUR SUPPORT TEAM below. Other agents work in this workspace and you will see their names in the conversation, but they are not yours to call: a call to one is refused, and the turn is spent finding that out. If the work needs someone who is not on your team, say so plainly and hand back so their leader can route it. That is the whole mechanism for reaching them.',
      ...
      '- Hand control back to the orchestrator using one of three markers, on its own line, as the very last thing in your response (after any final summary):',
  ...
  //
  // MODELS are stated for every platform agent, Codex or no Codex. They used to
  // sit inside the Codex branch, which meant the users who most needed them,
  // gateway users with no Codex installed, received no model guidance at all
  // and fell back to whatever the scaffolded skill taught. Runtime advice is
  // machine-specific; model advice is not.
  let runtimeSection = '';
  if (agentData && agentData.type === 'platform') {
    ...
    if (cx.installed && cx.authenticated) {
      lines.push(
        ...
        'Never recommend a runtime that is not listed here.',
    ...
    lines.push(
      'MODELS:',
      'A model may be any identifier the configured runtime serves, including one only the user can name, such as a gateway\'s (for example `my-gateway/claude-model-id`). Write what the user gives you verbatim rather than steering them to a name you recognise, and never rewrite a model you do not recognise: it is probably their gateway\'s.',
      'When the user names no model, write `model: inherit`, which runs the agent on whatever model their runtime resolves. Leaving the field out resolves identically on both runtimes; write `inherit` anyway so the file states its intent.',
    );
    if (cx.installed && cx.authenticated) {
      lines.push('For a Codex agent OMIT the model field entirely instead, so its ChatGPT account default applies. `inherit` is Claude Code vocabulary; Rundock tolerates it on a Codex agent by treating it as omission, but omitting the field is the clearer statement and the one to write.');
    }
    runtimeSection = lines.join('\n');
  }
