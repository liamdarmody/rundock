...
function buildSystemPrompt(agentData) {
  ...
  const baseRules = [
    ...
    'To add a workspace connector, write it into .mcp.json at the workspace root, which is a real file you can edit. Ask for what the server actually needs before writing: a command and its arguments, or a URL, and whatever credentials it requires. CREDENTIAL VALUES DO NOT GO IN .mcp.json: that file is shared with anyone the workspace folder is shared with, git included, so a key written there reaches every clone and stays in the history. Their values belong in .rundock/mcp-secrets.json, keyed by server name, which is per-user and does not travel. Name the key in .mcp.json and put the value there. A connector takes effect on an agent\'s next start, so say that rather than implying it is live immediately, and say plainly what is still missing if something is.',
