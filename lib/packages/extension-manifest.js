...
function deriveFacts(snapshotRoot, manifest) {
  ...
  return { agents, skills, match: manifest.match, files: files.map((f) => f.rel).sort() };
}

module.exports = { MANIFEST_NAME, SLUG, readExtensionManifest, classifySnapshot, deriveFacts, extensionFileSet };
