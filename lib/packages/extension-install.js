...
const { readExtensionManifest, deriveFacts, extensionFileSet, SLUG } = require('./extension-manifest.js');
const {
...
function uninstallExtension(workspace, name) {
  ...
  // The removal target is derived from the install-time layout, never from
  // the persisted root: EXTENSIONS_ROOT plus the record's own name is
  // exactly where installExtension writes, and nothing else is ever a
  // legitimate target for THIS install, including the extensions root
  // itself (never what one install created) and another installed
  // extension's directory. The header above says the record travels with
  // the workspace, so a shared or copied records file can carry anything in
  // root or even in name, and both are checked against that fixed layout
  // rather than trusted, before the one operation in this file that removes
  // a whole directory tree runs.
  ...
  const expectedAbsolute = path.join(workspace, ...EXTENSIONS_ROOT.split('/'), record.name);
  const targetAbsolute = path.resolve(workspace, ...record.root.split('/'));
  ...
  fs.rmSync(targetAbsolute, { recursive: true, force: true });
  return {
    name,
    removed: record.root,
