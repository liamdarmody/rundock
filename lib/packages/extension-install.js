...
function uninstallExtension(workspace, name) {
  ...
  // The persisted root is never trusted for a recursive delete. The header
  // above says this record travels with the workspace, so a shared or
  // copied records file can carry anything there, and the one operation in
  // this file that removes a whole directory tree must not let a stranger's
  // edit decide what that tree is. The target has to resolve inside the
  // extensions root this install owns, or there is nothing safe to remove.
  ...
  const extensionsRootAbsolute = path.join(workspace, ...EXTENSIONS_ROOT.split('/'));
