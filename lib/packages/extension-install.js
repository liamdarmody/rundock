...
function uninstallExtension(workspace, name) {
  ...
  // The removal target is the install-time layout, EXTENSIONS_ROOT plus the
  // record's own name, which is exactly where installExtension writes and
  // the only legitimate target for THIS install. The persisted root is
  // checked against it and never used as the target: the records reader has
  // already refused a record whose name or root is missing or malformed, and
  // a root that resolves anywhere else (the extensions root itself, another
  // install's directory, a path outside the workspace) is refused here
  // before the one operation in this file that removes a directory tree.
  const target = path.join(workspace, ...EXTENSIONS_ROOT.split('/'), record.name);
  ...
  const recordsPath = path.join(workspace, ...RECORDS_PATH.split('/'));
  ...
  // never a recorded extension whose files are gone. A removal the
  // filesystem refuses (a file held open, a read-only parent) puts the
  // record back before the refusal is reported, so what the person is told
  // is true and a retry still finds the extension to remove.
  ...
  try {
    ...
    refuse(`the files of "${name}" could not be removed (${e.code || e.message}); the extension is still installed`, 'remove-failed');
  }
  return {
    name,
    removed: record.root,
