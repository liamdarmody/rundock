...
//
// THE PIN IS REQUIRED FOR CODE, NOT DEFAULTED. "Run whatever is on main" and
// "run this exact snapshot" are different promises, and only the second is
// one an install screen can honestly make: the bytes the trust step derives
// its facts from must be the bytes that land. Agents and skills are files,
// not code that runs at install, so a link may arrive without a pin and be
// read at the repository's head for the content it offers; the moment the
// bytes classify as an extension, requirePin refuses with the reason rather
// than quietly pointing the install at a branch. The well-known moving names
// are refused by name whenever they are supplied, because typing "main" into
// a field labelled as a pin is the same wish spelled out.
...
function acquireWithGit(source) {
  ...
  try {
    ...
    // and any extension install.
    git(['fetch', '--quiet', '--depth', '1', 'origin', source.reference || 'HEAD']);
...
module.exports = { parseGitHubSource, requirePin, acquireWithGit, discardAcquisition, listRefsWithGit, MOVING_NAMES };
