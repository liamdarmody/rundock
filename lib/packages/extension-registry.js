 ...
 * the extension's directory and nowhere else. Every failure, an unreadable
 * records file included, is a refusal carrying its reason, never a throw:
 * the payload path answers the same way the roster path does.
 *
 * `styles` and `resources` are shape-only placeholders, always empty: an
 * extension declares one entry and one match rule, and nothing else is
 * declared or served today.
 *
 * @returns {{ ok: true, entry: string, styles: [], resources: [] }
...
function uiPayload(workspace, extensionId, rendererId) {
  ...
  let records;
  try { records = readRecords(workspace); } catch (e) {
