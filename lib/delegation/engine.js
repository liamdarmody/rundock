...
function handleDelegation(msg, processes) {
  ...
  const missed = (priorSessionId && !isCodexDelegate)
    ? deltaSince(loadTranscript(convoId) || [], targetAgent.id, undefined, [], agentDisplayNames())
    : { text: null, truncated: 0, clipped: 0 };
  ...
  const contextWithHistory = buildDelegateContext({ transcript, missed, brief: msg.context });
