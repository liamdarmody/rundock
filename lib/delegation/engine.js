...
function handleScopeReturn(specialistEntry, convoId, handbackMode = 'return') {
  ...
  const orchMissed = orchestratorSession
    ? deltaSince(loadTranscript(convoId) || [], orchestrator.id, undefined, [specialistEntry.agentId])
...
function handleDelegation(msg, processes) {
  ...
  const missed = (priorSessionId && !isCodexDelegate)
    ? deltaSince(loadTranscript(convoId) || [], targetAgent.id)
    : { text: null, truncated: 0, clipped: 0 };
  ...
  const finishDelegateClose = (code) => {
    ...
    if (delegateEntry.isIntercepted) {
      ...
      const parentMissed = parentSessionId
        ? deltaSince(loadTranscript(convoId) || [], parentAgentId, undefined, [delegateEntry.agentId])
        : { text: null, truncated: 0, clipped: 0 };
