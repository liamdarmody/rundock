...
const { resolveMarkers, noteHandoffMarker, HANDOFF_MODES } = require('./markers.js');
const { createDelegationRecord, attachDelegationRecord } = require('./state.js');
const { lastSessionFor, deltaNote, catchUpPrefix, buildDelegateContext, buildScopeReturnPrompt } = require('./catch-up.js');
const { discoverAgents, agentDisplayNames } = require('../agents/discovery.js');
const { buildSystemPrompt, buildTeamRoster, findDirectReportMatch, findOffRosterWorkspaceMatch, hasDirectReports } = require('../agents/prompt.js');
const { buildToolSummary, deltaSince, loadTranscript } = require('../store/transcripts.js');
...
let appendTranscript, buildHandbackPayload;
...
// A text block is opening, so the text already accumulated ended where the
// agent meant it to end. Restores the boundary the stream carries and this
// field otherwise loses: without it the last word of one block runs into the
// first word of the next.
//
// Nothing is added before the first block of a turn, and nothing is added where
// the agent's own text already ends in a break, so a turn cannot gain leading
// whitespace or a doubled gap.
function openTextBlock(entry) {
  if (entry.responseText && !/\n\s*$/.test(entry.responseText)) entry.responseText += '\n\n';
}
  ...
  entry.process.stdout.on('data', (chunk) => {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        // A BLOCK BOUNDARY IS A BREAK THE AGENT WROTE, and it has to survive.
        //
        // An agent writing two paragraphs either side of a tool call sends two
        // text blocks. Appending them with nothing between joins the last word
        // of one to the first word of the next: measured as
        // "above the hero.Now handing to Sage". The session keeps the blocks
        // apart, so the conversation reads correctly while this copy does not,
        // and this copy is what history.js rebuilds a conversation from and
        // what deltaSince hands an arriving agent as its catch-up.
        //
        // Only at a block START, never between the deltas WITHIN a block: a
        // block arrives in fragments and is one piece of text, so separating
        // those would put a gap inside every ordinary sentence.
        if (parsed.type === 'stream_event' && parsed.event?.type === 'content_block_start' && parsed.event?.content_block?.type === 'text') {
          openTextBlock(entry);
        }
        if (parsed.type === 'stream_event' && parsed.event?.type === 'content_block_delta' && parsed.event?.delta?.type === 'text_delta' && parsed.event.delta.text) {
          ...
          for (const block of parsed.message.content) {
            if (block.type === 'text' && block.text) {
              // The SAME function, not the same rule typed twice. Two copies
              // agree only for as long as nobody edits one of them, and TB-4
              // is the requirement that they cannot disagree about a turn.
              openTextBlock(entry);
        ...
        if (enableInterception && messageEnded && entry.pendingAgentTools && entry.pendingAgentTools.length) {
          ...
          // Filled in this same pass: see the block below for what a miss is
          // and why it is recorded rather than blocked. Gathered here so each
          // call's roster match is worked out once, not once to pick the
          // target and again to count the misses, which are two answers to the
          // same question and could drift apart.
          const missedTargets = [];
          const leadsSomebody = hasDirectReports(entry.agentId);
          for (const input of agentCalls) {
            const match = findDirectReportMatch(entry.agentId, input);
            if (!match && !input.subagent_type && leadsSomebody) missedTargets.push(input);
            if (!target && match) {
          ...
          // A CALL THAT MEANT TO DELEGATE AND REACHED NOBODY, said out loud.
          //
          // An agent with direct reports made an Agent call, named no
          // subagent_type, and nothing in its words matched a teammate. That is
          // the shape of a delegation that did not happen: the agent believed
          // it had handed the work on, told the person so in its handoff line,
          // and the work stayed where it was with nothing contradicting it.
          //
          // Recorded rather than blocked. An explicit subagent_type is a
          // deliberate choice and is left alone; a call with none is ambiguous,
          // and killing the turn would take a legitimate generic subagent with
          // it, which is a worse fault than the one being fixed. What this
          // guarantees is that the miss is findable afterwards instead of
          // silent, which is how this reached a person looking like a
          // permissions fault when it was not.
          // Only a call that named no subagent_type is ambiguous: one that named
          // a built-in deliberately chose it and is nobody's missed delegation.
          //
          // AND ONLY FROM AN AGENT THAT LEADS SOMEBODY. A specialist with no
          // reports cannot delegate at all, so its generic subagent is not a
          // failed handover and saying so would be noise on a line that has to
          // stay worth reading.
          // PER CALL, not per turn. A turn can make several Agent calls, and one
          // of them matching a report says nothing about the others: each one
          // that named nobody is its own handover that did not happen, and
          // recording only when the whole turn missed would have hidden every
          // one that shared a turn with a successful delegation.
          for (const input of missedTargets) {
            const said = (input.description || input.prompt || '').slice(0, 120).replace(/\s+/g, ' ').trim() || 'nothing named';
            console.log(`[AgentIntercept] convo=${convoId} agent=${entry.agentId} Agent call named no target the roster matched, so no delegation happened: ${said}`);
            // What it asked for travels ON THE RECORD, not only in a log line
            // that scrolls away. A tick saying a handover failed does not tell
            // anyone WHICH handover, and this defect reached a person looking
            // like a permissions fault precisely because nothing kept said what
            // had been attempted.
            //
            // This is the one event in the signal layer that carries text, and
            // the exception is deliberate rather than an oversight: the layer's
            // rule is that events carry structure so they stay countable, and
            // here the structure alone cannot answer the only question a reader
            // will have. It is bounded to 120 characters, whitespace collapsed,
            // and written only on a miss, which has never once occurred in the
            // recorded history of any workspace. Whether the layer should hold
            // text at all is a real question, and a bigger one than this card.
            recordEvent('delegation_error', {
              conv: convoId, agent: entry.agentId,
              d: { reason: 'no_target_matched', asked: said },
            });
          }
          ...
          if (target) {
            ...
            //
            // THE HANDOFF LINE TRAVELS IN A FIELD. Asking for prose alongside
            // the tool call is a second, optional act, and under load models do
            // the required one and drop it: measured as a coin flip across three
            // hand tests. The Agent tool's `description` is a field of the call
            // the agent is already making, so it cannot be dropped the same way.
            // Measured against the shipped runtime, four runs out of four, an
            // agent told that field is what the user sees wrote a full sentence
            // into it despite its schema saying "3-5 words".
            //
            // It is used ONLY when the agent wrote no prose of its own. An agent
            // that spoke is never overwritten: its own words win, and this is a
            // floor rather than a replacement.
            //
            // NOT ENFORCEMENT. Nothing here refuses a delegation whose
            // description is a three-word label; that is the delegation object's
            // job. This turns silence into a sentence, which is a different and
            // smaller claim.
            const handoffLine = typeof targetInput.description === 'string'
              ? targetInput.description.trim()
              : '';
            // TRIMMED, because raw truthiness is not prose. A stray newline
            // emitted before the tool_use block makes responseText a non-empty
            // string carrying nothing, and that would win over the handoff line
            // and render a blank turn: worse than either branch alone, and the
            // exact shape this card exists to remove.
            const ownProse = (entry.responseText || '').trim();
            // WHETHER THE CLIENT HAS ALREADY SEEN THIS TEXT, which is the only
            // question that decides whether it has to be sent.
            //
            // Having prose is not the same as having streamed it. responseText
            // is filled from text deltas when they arrive, and from the
            // assistant message's blocks when they do not (the `!sawTextDelta`
            // branch where it accumulates). In that second case the client
            // received no deltas, built no bubble, and would show nothing:
            // the agent's own words would be as invisible as the handoff line
            // was. Gating on the prose rather than on the streaming was the
            // same assumption this card exists to remove, one layer along.
            const streamedToClient = !!entry.sawTextDelta;
            let liveHandoffText = null;
            if (ownProse) {
              ...
              if (!streamedToClient) liveHandoffText = entry.responseText;
            } else if (handoffLine) {
              const toolSummary = buildToolSummary(entry.toolCalls);
              const withTools = toolSummary ? toolSummary + '\n' + handoffLine : handoffLine;
              appendTranscript(convoId, 'agent', entry.agentId, withTools, undefined, entry);
              // THE WORDS ALONE, not the tool summary with them.
              //
              // A reload does not render the transcript: it renders the Claude
              // session, and the `[Agent]` prefix is Rundock's own bookkeeping,
              // added only on the way into the transcript. Sending the prefixed
              // string made the live bubble read `[Agent]Handing to Vox...`
              // while the same turn after a reload read `Handing to Vox...`,
              // which is the divergence this card forbids rather than the one
              // it was closing. Measured in a browser, both ways round; no test
              // in this repository could see it, because they all stop at the
              // strings and this is about what the two render paths do with
              // them.
              liveHandoffText = handoffLine;
            } else {
              // Neither prose nor a line: the turn is still recorded so it is not
              // invisible on rehydrate, exactly as before.
              const toolSummary = buildToolSummary(entry.toolCalls);
            ...
            handleDelegation({
              ...
              // CARRIED TO THE CLIENT, not only to the transcript.
              //
              // appendTranscript above writes this line to disk and tells no
              // connected client, and the live path can only show a delegating
              // agent's words by promoting a streaming bubble it already has.
              // An agent that emits a bare tool_use block streams nothing,
              // which is exactly the turn this line exists for, so the line was
              // correct on reload and absent while it happened. Only sent when
              // the agent said nothing itself: when it did speak, its own
              // streamed text is already on screen and must not be replaced.
              _handoffLine: liveHandoffText
            }, chatProcesses);
          ...
          if (offRoster) {
            ...
            // THE TURN'S OWN WORDS, SENT BEFORE THE PROCESS DIES.
            //
            // This branch suppresses the end-of-message envelope and kills the
            // process, so no result ever arrives for this turn. The assistant
            // line carrying the text was forwarded, but the client produces no
            // effects for one: text that never streamed renders only through a
            // result. So the agent's own words were written to the transcript
            // and shown to nobody until a reload, exactly as the handoff line
            // was. Measured with the engine harness and the reducer, not
            // reasoned about: `assistant` in, zero effects out.
            //
            // Sent only when the text did not stream, because streamed text is
            // already on screen and sending it again would draw it twice.
            if (entry.responseText && !entry.sawTextDelta) {
              safeSend(JSON.stringify({
                type: 'system', subtype: 'agent_turn', _conversationId: convoId,
                _agent: entry.agentId, _processId: entry.processId,
...
// WHETHER AN ARRIVAL IS DRAWN, in one place, for every path that restores an
// agent.
//
// Four restoration paths each decide this, and the decision was written out
// four times. The first fix for the phantom arrival reached one of them and
// nothing failed, because four independent spellings of one rule cannot
// disagree loudly. They now share this, so a path that forgets is a path that
// never called it.
//
// The switch itself is ALWAYS sent. It is what clears the departing agent's
// working indicator, moves the active agent and updates the header; withholding
// it left the conversation marked delegated with the departed specialist still
// spinning. Only the drawing is suppressed.
//
// Takes whether the restored agent will produce a visible turn; returns the
// payload fragment to spread into the agent_switch message.
function switchSilence(willSpeak) {
  return willSpeak ? {} : { silent: true };
}

function handleScopeReturn(specialistEntry, convoId, handbackMode = 'return') {
  // The third argument was a boolean until CONTINUE existed, and a stray `true`
  // reaching the mode comparison would read as "not complete" and silently pick
  // the out-of-scope prompt. Normalised rather than trusted, because that
  // failure would be invisible: a working conversation with the wrong prompt.
  const mode = typeof handbackMode === 'boolean'
    ? (handbackMode ? 'complete' : 'return')
    : (handbackMode || 'return');
  const wasPipelineComplete = mode === 'complete';
  ...
  // RESUMED, NOT COLD. Spawned with no session, the orchestrator knew only what
  // the specialist had just said and nothing about the original request, and
  // re-briefed the next delegate from that empty picture: a short blog post came
  // back, after two handoffs, as a request for a LinkedIn post.
  let orchestratorSession = null;
  try {
    orchestratorSession = lastSessionFor(readConversations(), convoId, orchestrator.id);
  } catch (e) { /* cold spawn on failure, which is the old behaviour */ }

  // AND TOLD WHAT IT MISSED: its session ends at "I am delegating this", so the
  // specialists' work is otherwise invisible. Only when actually resumed, since
  // "since your last turn" means nothing to a cold spawn; and never including
  // the returning specialist, whose message the output block already carries.
  const orchMissed = orchestratorSession
    ? deltaSince(loadTranscript(convoId) || [], orchestrator.id, undefined, [specialistEntry.agentId], agentDisplayNames())
    : { text: null, truncated: 0, clipped: 0 };
  ...
  const args = [...getBareArgs(), ...modelArgs(orchestrator), '--output-format', 'stream-json', '--input-format', 'stream-json',
    ...
    ...(orchestratorSession ? ['--resume', orchestratorSession] : []),
    '--agent', orchestrator.name];

  console.log(`[ScopeReturn] convo=${convoId} from=${specialistEntry.agentId} to=${orchestrator.id} proc=${processId}`
    + `${orchestratorSession ? ` resume=${orchestratorSession}` : ' resume=none'}`);
  ...
  recordEvent('handback', {
    ...
    d: { kind: mode, to: orchestrator.id },
  });
  const orchEntry = {
  ...
  // NO ARRIVAL DRAWN FOR AN AGENT THAT WILL SAY NOTHING, BUT THE SWITCH IS
  // STILL SENT.
  //
  // On a COMPLETE handback the orchestrator is spawned only to park, so it does
  // not repeat the specialist's answer back at the user. The switch announced
  // the arrival anyway, so the conversation showed an agent joining and then
  // doing nothing: visually identical to a hang, and reported as one.
  //
  // Suppressing the MESSAGE to fix that was wrong, and the comment justifying
  // it ("nothing downstream depends on having said so") was false. The switch
  // is not only an announcement: reduceAgentSwitch sets activeAgentId, clears
  // delegationActive, and emits clear-outgoing-working. Withhold it and the
  // conversation stays marked delegated, the specialist stays the active agent,
  // and ITS working indicator never stops, which is the same hang the
  // suppression was written to remove, moved onto the agent that just left.
  //
  // So the message always goes and the DRAWING is what is suppressed, by the
  // same silent flag process_started already carries. The reducer runs either
  // way; only the divider is skipped.
  safeSend(JSON.stringify({
    type: 'system', subtype: 'agent_switch', _conversationId: convoId,
    _processId: processId,
    fromAgent: specialistEntry.agentId, toAgent: orchestrator.id, returning: true,
    ...switchSilence(!wasPipelineComplete)
  }));
  ...
  if (!wasPipelineComplete && bufferedFollowUpTakesOver(convoId, orchEntry, 'scope-return routing prompt')) {
    ...
    if (resumeCount >= MAX_CONSECUTIVE_AGENT_RESUMES) {
      ...
      const prompt = buildScopeReturnPrompt({
        mode, orchMissed, specialistId: specialistEntry.agentId, outputBlock,
        pendingRequest: specialistEntry.lastUserMessage || ''
      });

      proc.stdin.write(JSON.stringify({ type: 'user', message: { role: 'user', content: prompt } }) + '\n');
      // LOGGED WHERE IT IS SENT, not where it is computed. Two branches above
      // this one park the prompt instead of writing it: a buffered user
      // message that supersedes it, and the auto-resume circuit breaker. A log
      // line emitted beside the spawn claimed a delta had gone in both cases.
      // A instrument that reports work it did not do is worse than no
      // instrument, because it is believed.
      console.log(`[ScopeReturn] convo=${convoId} to=${orchestrator.id} prompt sent`
        + deltaNote(orchMissed, Boolean(orchestratorSession)));
    }
  }
...
function spawnResumedProcess(convoId, agentId, sessionId, processes, opts = {}) {
  ...
  wireProcessHandlers(entry, convoId, null, {
    enableInterception: true,
    onResult: (e) => {
      // KNOWN LIMITATION: a respawned orchestrator that emits its own handoff marker here is self-treated as a scope-return. Low/narrow.
      noteHandoffMarker(e, e.responseText, (mode) => {
        console.log(`[ScopeReturn] convo=${convoId} agent=${e.agentId} ${mode} marker on respawned process`);
...
function handleDelegation(msg, processes) {
  ...
  if (!isPlatformDelegate) {
    try {
      priorSessionId = lastSessionFor(readConversations(), convoId, targetAgent.id);
  ...
  // SAYS WHETHER THE DELTA WENT, AND HOW BIG IT WAS. Without this a reader
  // cannot tell a delegate that received no catch-up from one that received it
  // and ignored it, which are different faults with different fixes.
  // ARRIVING COUNTS TOO, not only returning.
  //
  // A delegate brought in by an intercepted Agent call was given the brief and
  // nothing else, on the reasoning that the orchestrator's brief is
  // sufficient. The brief is written at one moment about one task, and the
  // conversation moves on around it.
  //
  // Observed: an orchestrator wrote a post itself because the content writer
  // only covers another channel. A research lead was then delegated to, saw
  // only "research this site", and when the user asked for the work to go back
  // for the next iteration of that post, looked for a draft, could not see
  // one, said so, and declared the pipeline finished. The orchestrator, which
  // had written the post and still held it, was resumed under the
  // pipeline-complete prompt and ordered to stay silent. The agent with the
  // context was silenced by an agent without it.
  //
  // deltaSince with no prior turn for this agent already returns the whole
  // conversation, capped, attributed and reporting what it dropped: exactly
  // what someone walking in needs. The reason this was withheld was cost, and
  // the cap is the answer to cost.
  //
  // NOT GATED ON ANYTHING. Every delegate gets the delta, whichever way the
  // delegation started and whichever runtime it lands on.
  //
  // Two gates used to stand here and both were wrong. Excluding Codex was
  // implementation convenience dressed as a rule ("its prompt assembly is
  // separate"), and it is not separate for this: both Codex branches below
  // already send contextWithHistory, which is what carries the catch-up.
  //
  // The second gate excluded a cold spawn that had not come through an
  // intercepted Agent call, because that one was handed the whole conversation
  // rendered in full instead, and sending both would have been the same text
  // twice. Rendering it in full was the fault. The two ways a delegation can
  // start differ only in who asked for it, and they were handing the arriving
  // agent contexts of completely different sizes: the delta is bounded at
  // DELTA_CAP_CHARS, and the full rendering had no character bound at all,
  // only the transcript's own 1000-entry soft cap, with every tool summary
  // included. On the longest real three-agent conversation on disk that was 47
  // million tokens against 89 thousand. Nothing in the interface said which
  // one a delegation had taken.
  // ARRIVING MEANS A RESUME WILL NOT HAPPEN, not "no id was stored".
  //
  // Codex validates the stored thread id before resuming (codexResumeId
  // below). A MALFORMED id is a non-empty string that fails that check, so the
  // delegate gets a FRESH thread while `!priorSessionId` says it is returning.
  //
  // An EXPIRED id is not caught here and cannot be: isValidThreadId reads the
  // string's shape, not the app-server's state. That case is handled where it
  // becomes knowable, at the fresh-thread fallback below, which carries an
  // arrival's catch-up precisely because this check cannot see it coming. It was then handed "since your last turn" over a slice of a
  // conversation it had never seen: the exact defect this change exists to
  // fix, reached by the one path that looks like a resume and is not.
  //
  // One decision, read by both, so the heading can never disagree with the
  // thread actually receiving it.
  const willResume = isCodexDelegate
    ? codexRuntime.isValidThreadId(priorSessionId)
    : Boolean(priorSessionId);
  const arriving = !willResume;
  const missed = deltaSince(loadTranscript(convoId) || [], targetAgent.id, undefined, [], agentDisplayNames(), arriving);
  console.log(`[Delegate] convo=${convoId} from=${originalAgentId} to=${targetAgent.id} proc=${delegateProcessId} runtime=${targetAgent.runtime}${willResume ? ` resume=${priorSessionId}` : ''}`);
  // The delta is reported where it is sent, below. Nothing can skip this
  // delegate's write today, so logging it here would not lie: it is placed at
  // the send anyway so the rule is the same at all three sites and there is no
  // exception for a future reader to know about. Two of these logs already
  // drifted into claiming deltas that never went.
  const noteDelegateSend = () => console.log(
    `[Delegate] convo=${convoId} to=${targetAgent.id} prompt sent`
    + deltaNote(missed, willResume));
  ...
  const codexResumeId = isCodexDelegate && willResume ? priorSessionId : null;
  ...
  // Notify client of agent switch. The handoff line rides along because this
  // message is already sent synchronously, already names who is leaving, and
  // already drives the divider the line has to appear above.
  // The only switch without `returning`: this is the forward delegation, and
  // every other one restores an agent that already had the work.
  safeSend(JSON.stringify({
    ...
    ...(msg._handoffLine ? { handoffLine: msg._handoffLine } : {})
  }));
  ...
  //
  // A resumed delegate carries its own history in its session and is given the
  // orchestrator's brief, and until now nothing else: it could not see what the
  // other agents did between its turns. A specialist came back to her own draft
  // after two other agents had worked, and rebuilt it from scratch, because
  // nothing told her any of it had happened.
  //
  // TWO CASES, and one bound over both of them.
  //
  //   - An ARRIVING delegate, with no thread to resume, gets the brief
  //     prefixed by the conversation so far.
  //   - A RESUMED delegate gets the brief prefixed by what it missed since its
  //     own last turn, because its session already holds the rest.
  //
  // How the delegation was asked for does not appear in that list, and that is
  // the point: an agent walking into a conversation needs the same thing
  // whether another agent called for it through the Agent tool or wrote the
  // handoff marker into its own reply.
  //
  // The comment here used to say "only for a resumed delegate... a first-time
  // delegate has missed nothing", which was the premise an earlier change
  // overturned. A first-time delegate has missed everything: it is the
  // conversation it never saw, not turns it took and forgot.
  const contextWithHistory = buildDelegateContext({ missed, brief: msg.context, arriving });
  ...
  if (isCodexDelegate) {
    ...
    // the full prompt.
    // THE FALLBACK PROMPT IS ALWAYS AN ARRIVAL'S PROMPT.
    //
    // isValidThreadId checks the SHAPE of the stored id and nothing else: a
    // thread that has genuinely expired is still well-formed, so it passes and
    // `arriving` reads false. wireCodexDelegate then discovers the resume fails
    // and falls back to a fresh thread, which holds nothing, while this prompt
    // would have told the delegate what it missed "since its last turn" on a
    // thread it does not have. That is this card's own defect, reached by the
    // one path where the check cannot know the answer in advance.
    //
    // So the fallback carries a catch-up built as an arrival. It costs a second
    // delta only for a codex delegate that is attempting a resume, and it is
    // bounded by the same cap as the first.
    const codexArrivingContext = codexResumeId
      ? buildDelegateContext({
          missed: deltaSince(loadTranscript(convoId) || [], targetAgent.id, undefined, [], agentDisplayNames(), true),
          brief: msg.context,
          arriving: true,
        })
      : contextWithHistory;
    const codexFreshPrompt = [readAgentInstructions(targetAgent), fullPrompt, codexArrivingContext].filter(Boolean).join('\n\n');
    ...
    // Reports delta=none, which is the truth here: a resumed Codex thread
    // already carries the history, so no delta is built for it. Said out loud
    // rather than left to inference from a missing line.
    noteDelegateSend();
  } else {
  ...
  wireProcessHandlers(delegateEntry, convoId, null, {
    enableInterception: true,
    onResult: (e) => {
      // THE RESOLVER DECIDES, NOT THIS SITE. This used to name the two markers
      // it knew about and rebuild their precedence by hand, so a third marker
      // was inert here: a delegate emitting CONTINUE set neither flag, no
      // handoff was seen, and the auto-return never fired. The marker existed
      // everywhere except the place it is consumed. Reading `mode` means a
      // fourth marker cannot repeat that.
      const markers = resolveMarkers(e.responseText);
      const { hasReturn: hasOutOfScope, hasComplete, hasCrudMarker } = markers;
      const hasHandoff = Boolean(markers.mode);
      const shouldAutoReturn = e.isPlatformDelegate
        ? (hasHandoff || hasCrudMarker)
        : hasHandoff;

      if (markers.mode === 'continue') {
        e.returnMarkerSeen = 'continue';
        console.log(`[Delegate] convo=${convoId} agent=${e.agentId} CONTINUE marker detected (its part done, request is not)`);
      } else if (hasComplete) {
  ...
  const finishDelegateClose = (code) => {
    ...
    // WHAT THE SPECIALIST DECLARED, derived once for every restoration branch
    // below. This lived inside the intercepted branch, so the non-intercepted
    // one could not see it and parked its parent in silence whatever the
    // specialist had said. Deriving it twice would have worked and would have
    // been the fourth copy of this rule in the file.
    //
    // RETURN means the user asked for something outside the specialist's
    // domain; COMPLETE means the delegated pipeline finished end-to-end;
    // CONTINUE means this specialist's part is done and the request is not.
    let returnMarkerSeen = delegateEntry.returnMarkerSeen || null;
    if (!returnMarkerSeen) {
      // Tail scan for a marker the onResult handler never saw (e.g. the
      // process died after streaming it). Same single resolver, same precedence.
      returnMarkerSeen = resolveMarkers(delegateEntry.finalResponseText || delegateEntry.responseText).mode;
    }

    // WHAT THEY DELIVERED, for every branch that promises it. Each restoration
    // path tells the agent it is resuming to act on the specialist's work.
    // This was computed inside one branch only, so another said "read what they
    // delivered above" to an orchestrator that had never been sent it: an agent
    // told to act on work it was never shown, which is the defect this change
    // exists to fix, written into the fix itself.
    const handbackOutput = buildHandbackPayload(delegateEntry, convoId);
    const handbackBlock = handbackOutput
    ...
    if (delegateEntry.isIntercepted) {
      ...
      // A nested delegate can hand back with work outstanding too. Without this
      // it falls through to the normal-exit branch below, which parks the parent
      // silently: the same defect CONTINUE exists to fix, one level down.
      const isContinue = returnMarkerSeen === 'continue';
      // Both mean "the parent must act", so both take the gate and the breaker.
      const parentMustAct = isOutOfScope || isContinue;
      ...
      if (hasHandoffMarker && orchestratorEntry && !orchestratorEntry.exited) {
        ...
        // SILENT WHENEVER THE AGENT BEING RESTORED WILL NOT SPEAK, and
        // reachability is part of that, not a separate condition the wake adds
        // later. On the COMPLETE gate below this orchestrator is deliberately
        // left idle; with a dead stdin it cannot be driven at all. Either way
        // drawing its arrival shows an agent joining and doing nothing, which
        // reads as a hang.
        //
        // The switch is still SENT, because it is what clears the departing
        // delegate's working indicator and moves the active agent; only the
        // divider is suppressed.
        //
        // Declared here rather than inline in the branch below so that drawing
        // an arrival and waking the orchestrator read ONE answer. Left apart,
        // the wake asked a question the drawing did not, which is exactly how
        // this went wrong on the sibling path.
        const orchestratorReachable = Boolean(orchestratorEntry.process
          && orchestratorEntry.process.stdin
          && orchestratorEntry.process.stdin.writable
          && !orchestratorEntry.process.killed);
        // Asked once, ahead of the send, so the drawing and the waking share the
        // answer. A buffered message drives this orchestrator just as an
        // auto-continue would, so it counts as speaking and therefore draws;
        // reading reachability alone suppressed an arrival the replay justified.
        // Same shape as the non-intercepted restore below, and the third path
        // where this pattern was missed rather than absent.
        const orchestratorBufferedTookOver = !isPipelineComplete
          && bufferedFollowUpTakesOver(convoId, orchestratorEntry, 'RETURN auto-continue');
        const orchestratorWillSpeak = !isPipelineComplete
          && (orchestratorBufferedTookOver || orchestratorReachable);
        safeSend(JSON.stringify({
          type: 'system', subtype: 'agent_switch', _conversationId: convoId,
          fromAgent: delegateEntry.agentId, toAgent: orchestratorAgentId, returning: true,
          ...switchSilence(orchestratorWillSpeak)
        }));
        ...
        } else if (orchestratorBufferedTookOver) {
          // orchestrator stays idle; the replayed message drives it
        } else if (orchestratorReachable) {
          ...
          recordEvent('circuit_breaker', { conv: convoId, d: { count: resumeCount } });
            ...
            setTimeout(() => {
              if (!orchestratorEntry.exited) {
                ...
                // BY THE MARKER, NOT BY ASSUMPTION. This said "outside their
                // scope" whatever had actually happened, so an orchestrator
                // resumed after a CONTINUE was told the specialist could not do
                // the work: the opposite of true, since it did the work and is
                // asking for the next step. The specialist's own declaration
                // decides the wording, as it does on every other handback path.
                const prompt = returnMarkerSeen === 'continue'
                  ? buildScopeReturnPrompt({
                    mode: 'continue', orchMissed: { text: null },
                    specialistId: delegateEntry.agentId,
                    outputBlock: handbackBlock + deferredNote
                  })
                  : pendingRequest
      ...
      // Silent on exactly the branches that park the parent. Both the
      // pipeline-complete and the no-marker exits below restart it with the
      // specialist's output for context and then park it, and both already
      // send their process_started with `silent: true`, which is the same
      // statement about the same turn. The switch says control moved; it does
      // not claim anyone is about to speak.
      const parentWillSpeak = parentMustAct;
      safeSend(JSON.stringify({
        type: 'system', subtype: 'agent_switch', _conversationId: convoId,
        fromAgent: delegateEntry.agentId, toAgent: parentAgentId, returning: true,
        ...switchSilence(parentWillSpeak)
      }));

      // THE THIRD RESUME PATH, and the one that goes deepest. A mid-level
      // parent (a specialist that has its own direct reports) is resumed here
      // when its sub-delegate hands back. It was given the sub-delegate's own
      // output and nothing else: the same blindness the orchestrator and the
      // delegate paths just had, one level further down. Observed shape:
      // Roo delegates to Ren, Ren delegates to Sage, Sage returns, and Ren
      // comes back with no idea what happened in the conversation while she
      // was waiting.
      //
      // The returning delegate is excluded because delegateOutputBlock below
      // already carries its message; without that it arrives twice.
      const parentMissed = parentSessionId
        ? deltaSince(loadTranscript(convoId) || [], parentAgentId, undefined, [delegateEntry.agentId], agentDisplayNames())
        : { text: null, truncated: 0, clipped: 0 };
      const parentCatchUp = catchUpPrefix(parentMissed);
      ...
      // The delta is reported at each send site below, never here. Computed
      // beside the spawn, this line claimed a delta had gone in the two cases
      // where the prompt carrying it is never written: a buffered user message
      // that supersedes it, and the auto-resume circuit breaker. The
      // orchestrator's log was moved for exactly this reason earlier in this
      // same change, and the sibling path was left behind.
      const noteParentSend = () => console.log(
        `[AgentIntercept] convo=${convoId} parent ${parentAgentId} prompt sent`
        + deltaNote(parentMissed, Boolean(parentSessionId)));
      ...
      const delegateOutput = handbackOutput;
      const delegateOutputBlock = delegateOutput
        ? `\n\n--- ${delegateEntry.agentId} ---\n${delegateOutput}\n---`
        : '';

      if (parentMustAct && bufferedFollowUpTakesOver(convoId, resumeEntry, isContinue ? 'CONTINUE prompt' : 'RETURN routing prompt')) {
        // parked by the gate; the replayed message drives the resumed parent
      } else if (parentMustAct) {
        const resumeCount = incrementAutoResume(convoId);
        if (resumeCount >= MAX_CONSECUTIVE_AGENT_RESUMES) {
          ...
          const resumePrompt = isContinue
            ? buildScopeReturnPrompt({
              mode: 'continue', orchMissed: { text: null },
              specialistId: delegateEntry.agentId,
              outputBlock: delegateOutputBlock + deferredNote
            })
            : `[SYSTEM: ${delegateEntry.agentId} returned because the request was outside their scope. Here is what they said:${delegateOutputBlock}\n\nThe user's latest request was: "${delegateEntry.lastUserMessage || 'continue'}". Respond with full awareness of what ${delegateEntry.agentId} delivered. Do not re-delegate work already done. Route to the right specialist using the Agent tool.${deferredNote}]`;
          resumeProc.stdin.write(JSON.stringify({ type: 'user', message: { role: 'user', content: parentCatchUp + resumePrompt } }) + '\n');
          noteParentSend();
        }
        ...
        resumeProc.stdin.write(JSON.stringify({ type: 'user', message: { role: 'user', content: parentCatchUp + completePrompt } }) + '\n');
        noteParentSend();
        resumeEntry.idle = true; resumeEntry.idleSince = Date.now();
        ...
        resumeProc.stdin.write(JSON.stringify({ type: 'user', message: { role: 'user', content: parentCatchUp + normalPrompt } }) + '\n');
        noteParentSend();
        resumeEntry.idle = true; resumeEntry.idleSince = Date.now();
      ...
      wireProcessHandlers(resumeEntry, convoId, null, {
        ...
        onResult: (e) => {
          ...
          noteHandoffMarker(e, e.responseText, (mode) => {
            console.log(`[ScopeReturn] convo=${convoId} agent=${e.agentId} ${mode} marker on resumed parent`);
      ...
      resumeProc.on('close', (rCode) => {
        ...
        if (resumeEntry.scopeReturn && cur === resumeEntry) {
          console.log(`[ScopeReturn] convo=${convoId} resumed parent ${resumeEntry.agentId} exited with ${resumeEntry.scopeReturnMode} marker, spawning orchestrator (mode=${resumeEntry.scopeReturnMode})`);
          handleScopeReturn(resumeEntry, convoId, resumeEntry.scopeReturnMode);
          return;
        }
      ...
      // Hoisted from below so the send and the decision to wake read the same
      // expression rather than two that have to agree. Conservative by
      // design: a buffered message can still drive the parent from inside
      // bufferedFollowUpTakesOver, which cannot be asked without acting, so
      // an arrival is DRAWN whenever there is any reason to think the parent
      // will speak. Over-drawing shows a divider nobody needed;
      // under-drawing hides a real arrival, which is the worse of the two.
      const parentShouldAct = returnMarkerSeen === 'continue';
      // WHAT DECIDES A TURN, ASKED ONCE AND IN THE RIGHT ORDER.
      //
      // Three things bear on whether this parent produces a visible turn:
      // whether the marker and any follow-up say it should act, whether a
      // buffered message is waiting to drive it instead, and whether it can be
      // written to at all.
      //
      // bufferedFollowUpTakesOver is consulted here rather than inside the
      // guard below so its answer is available to BOTH decisions. It is safe
      // to move: it checks for a buffered chat, marks the entry idle and logs.
      // The replay itself happens later in endConvoTransition, so nothing
      // client-visible is reordered. What it must not be is skipped: gating it
      // on reachability left the entry unparked for a message that was about
      // to be replayed into it.
      //
      // A buffered takeover still produces a turn, so it DRAWS. That is why
      // the draw reads `bufferedTookOver || parentReachable` rather than
      // reachability alone: a stricter draw would suppress an arrival that a
      // replayed message then justifies.
      const parentShouldWake = !delegateEntry.isPlatformDelegate
        && (delegateEntry.receivedFollowUp || parentShouldAct);
      const parentReachable = Boolean(orig.process && orig.process.stdin && orig.process.stdin.writable);
      const bufferedTookOver = parentShouldWake
        && bufferedFollowUpTakesOver(convoId, orig, 'specialist-return auto-continue');
      const restoredWillSpeak = parentShouldWake && (bufferedTookOver || parentReachable);
      safeSend(JSON.stringify({
        type: 'system', subtype: 'agent_switch', _conversationId: convoId,
        fromAgent: delegateEntry.agentId, toAgent: delegateEntry.delegation.originalAgentId, returning: true,
        ...switchSilence(restoredWillSpeak)
      }));
      ...
      // THE FIFTH RESTORATION BRANCH, and the last one that ignored the
      // marker entirely. Reached when the delegation was started without
      // interception (the WS delegate route), which leaves the parent parked
      // alive. Its only reason to wake the parent was whether the user had
      // happened to send a message during the delegation, so a specialist
      // declaring that work remains was parked in silence exactly as if no
      // marker existed: the reported defect, by a different mechanism.
      //
      // Only CONTINUE is added here. What RETURN and COMPLETE do on this path
      // is existing behaviour and is left alone.
      //
      // `parentShouldAct` and `restoredWillSpeak` are declared above the
      // agent_switch, not here, so the decision to DRAW an arrival and the
      // decision to WAKE the parent read ONE expression rather than two that
      // have to be kept in agreement. The reachability check moved into it for
      // that reason: left out here it was a second condition on waking that the
      // drawing did not share.
      //
      // bufferedFollowUpTakesOver stays outside it, and cannot be folded in: it
      // acts as it answers, so asking it twice would replay a message twice.
      // Leaving it out only ever draws an arrival that a replayed message then
      // justifies, which is visible activity either way.
      if (parentShouldWake && !bufferedTookOver && parentReachable) {
        const resumeCount = incrementAutoResume(convoId);
        if (resumeCount >= MAX_CONSECUTIVE_AGENT_RESUMES) {
...
module.exports = {
  // Exported so a test can drive the real mapping rather than hand-build one:
  // the fallback chain and the failure path are the parts that decide what a
  // returning agent is called.
  createDelegationEngine, DEP_NAMES, recordControlReturnedTo };
