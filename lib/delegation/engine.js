...
function handleDelegation(msg, processes) {
  ...
  const finishDelegateClose = (code) => {
    ...
    if (delegateEntry.isIntercepted) {
      ...
      if (!delegateEntry.isPlatformDelegate && (delegateEntry.receivedFollowUp || parentShouldAct) && !bufferedFollowUpTakesOver(convoId, orig, 'specialist-return auto-continue') && orig.process && orig.process.stdin && orig.process.stdin.writable) {
        const resumeCount = incrementAutoResume(convoId);
        if (resumeCount >= MAX_CONSECUTIVE_AGENT_RESUMES) {
