...
function handleDelegation(msg, processes) {
  ...
  const missed = (priorSessionId && !isCodexDelegate)
    ...
    : { text: null, truncated: 0, clipped: 0 };
  console.log(`[Delegate] convo=${convoId} from=${originalAgentId} to=${targetAgent.id} proc=${delegateProcessId} runtime=${targetAgent.runtime}${priorSessionId ? ` resume=${priorSessionId}` : ''}${deltaNote(missed, Boolean(priorSessionId))}`);
