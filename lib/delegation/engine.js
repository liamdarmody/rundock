...
function handleDelegation(msg, processes) {
  ...
  // - Resumed delegate: what it missed since its own last turn
  // - Intercepted cold spawn: the conversation so far, capped
  ...
  //
  // GATED ON WHETHER A TRANSCRIPT IS ALREADY GOING, NOT ON RUNTIME. Excluding
  // Codex was implementation convenience dressed as a rule ("its prompt
  // assembly is separate"), and it is not separate for this: both Codex
  // branches below already send contextWithHistory, which is what carries the
  // catch-up. A Codex delegate arriving cold had exactly the defect this change
  // exists to fix, and the only thing withholding it was this line.
  //
  // Still not when a full transcript is already going (the non-intercepted cold
  // spawn below), which would otherwise send the same conversation twice.
  const needsCatchUp = !needsTranscript;
  ...
  const missed = needsCatchUp
    ? deltaSince(loadTranscript(convoId) || [], targetAgent.id, undefined, [], agentDisplayNames(), arriving)
    : { text: null, truncated: 0, clipped: 0 };
  ...
  //
  // THREE CASES, and the middle one is what this change added.
  //
  //   - A non-intercepted cold spawn gets the formatted transcript above, so
  //     it is handed no catch-up here: it would be the same conversation twice.
  //   - An ARRIVING delegate, brought in by an intercepted Agent call with no
  //     thread to resume, gets the brief prefixed by the conversation so far.
  ...
  // delegate has missed nothing", which was the premise this change overturned.
  // A first-time delegate has missed everything: it is the conversation it
  // never saw, not turns it took and forgot.
  const contextWithHistory = buildDelegateContext({ transcript, missed, brief: msg.context, arriving });
