...
function handleScopeReturn(specialistEntry, convoId, wasPipelineComplete = false) {
  ...
  try {
    ...
    const match = (convo && convo.sessionIds || []).filter((x) => x.agentId === orchestrator.id).pop();
    if (match) orchestratorSession = match.sessionId;
  ...
  const orchMissed = orchestratorSession
    ? deltaSince(loadTranscript(convoId) || [], orchestrator.id)
    : { text: null, truncated: 0 };
  ...
  console.log(`[ScopeReturn] convo=${convoId} from=${specialistEntry.agentId} to=${orchestrator.id} proc=${processId}`
    ...
    + `${orchMissed.text ? ` delta=${orchMissed.text.length}chars` : ' delta=none'}`);
  ...
  if (!wasPipelineComplete && bufferedFollowUpTakesOver(convoId, orchEntry, 'scope-return routing prompt')) {
    ...
    if (resumeCount >= MAX_CONSECUTIVE_AGENT_RESUMES) {
      ...
      const orchCatchUp = orchMissed.text
        ? `SINCE YOUR LAST TURN IN THIS CONVERSATION:\n${orchMissed.text}\n\n`
        : '';
...
function handleDelegation(msg, processes) {
  ...
  const missed = (priorSessionId && !isCodexDelegate)
    ...
    : { text: null, truncated: 0 };
  const deltaNote = missed.text
    ? ` delta=${missed.text.length}chars${missed.truncated ? `/truncated${missed.truncated}` : ''}`
    : (priorSessionId ? ' delta=none' : '');
  console.log(`[Delegate] convo=${convoId} from=${originalAgentId} to=${targetAgent.id} proc=${delegateProcessId} runtime=${targetAgent.runtime}${priorSessionId ? ` resume=${priorSessionId}` : ''}${deltaNote}`);
  ...
  const sinceYouWereHere = missed.text
    ? `SINCE YOUR LAST TURN IN THIS CONVERSATION:\n${missed.text}\n\n`
    : '';

  const contextWithHistory = transcript
    ? `CONVERSATION SO FAR:\n${transcript}\n\nYOUR TASK:\n${msg.context}`
    : `${sinceYouWereHere}[DELEGATION BRIEF]\n${msg.context}`;
