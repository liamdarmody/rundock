...
function handleScopeReturn(specialistEntry, convoId, wasPipelineComplete = false) {
  ...
  // RESUMED, NOT COLD. This spawned the orchestrator with no session at all: no
  // history, only its system prompt and the handback text. It therefore came
  // back knowing what the specialist had just said and nothing about what the
  // user originally asked for, and re-briefed the next delegate from that empty
  // picture. Observed: a request for a short blog post came back, after two
  // handoffs, as a delegation asking for a LinkedIn post.
  //
  // That is the "asks for things already asked and completed" symptom reported
  // from daily use, seen from the orchestrator's side.
  ...
  // AND TOLD WHAT IT MISSED, for the same reason a returning delegate is. Its
  // own session ends at "I am delegating this"; everything the specialists did
  // since is invisible to it without this.
  // ONLY WHEN IT IS ACTUALLY RESUMED. Told "since your last turn" while
  // cold-spawned, the orchestrator hears about a turn it has no memory of, and
  // the delta deliberately excludes its own turns on the grounds that its
  // session carries them: true when --resume fired, false when it did not. That
  // is worse than the old cold spawn, which at least claimed nothing.
  //
  // AND NOT THE SPECIALIST THAT IS HANDING BACK. Its final message is already
  // going in this same prompt as its own block, a few lines below. Without
  // excluding it here the orchestrator reads the same text twice in one turn,
  // on every handback where it has a session to resume.
  ...
  if (!wasPipelineComplete && bufferedFollowUpTakesOver(convoId, orchEntry, 'scope-return routing prompt')) {
    ...
    if (resumeCount >= MAX_CONSECUTIVE_AGENT_RESUMES) {
      ...
      if (wasPipelineComplete) {
        prompt = buildScopeReturnPrompt({
          complete: true, orchMissed, specialistId: specialistEntry.agentId, outputBlock
        });
      } else {
        const pendingRequest = specialistEntry.lastUserMessage || '';
        prompt = buildScopeReturnPrompt({
          complete: false, orchMissed, specialistId: specialistEntry.agentId, outputBlock, pendingRequest
        });
      }
