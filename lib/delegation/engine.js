  ...
  const finishDelegateClose = (code) => {
    ...
    if (delegateEntry.isIntercepted) {
      ...
      if (hasHandoffMarker && orchestratorEntry && !orchestratorEntry.exited) {
        ...
        // SILENT WHENEVER THE AGENT BEING RESTORED WILL NOT SPEAK. On the
        // COMPLETE gate below this orchestrator is deliberately left idle, so
        // drawing its arrival shows an agent joining and doing nothing, which
        // reads as a hang. The switch is still SENT, because it is what clears
        // the departing delegate's working indicator and moves the active
        // agent; only the divider is suppressed.
        safeSend(JSON.stringify({
          type: 'system', subtype: 'agent_switch', _conversationId: convoId,
          fromAgent: delegateEntry.agentId, toAgent: orchestratorAgentId,
          ...(isPipelineComplete ? { silent: true } : {})
        }));
