    ...
    if (delegateEntry.isIntercepted) {
      ...
      if (hasHandoffMarker && orchestratorEntry && !orchestratorEntry.exited) {
        ...
        safeSend(JSON.stringify({
          ...
          ...(orchestratorWillSpeak ? {} : { silent: true })
        }));
      ...
      // REACHABILITY IS PART OF "WILL SPEAK", not a separate condition the
      // wake guard adds later. A parent whose stdin has gone cannot be driven,
      // so an arrival drawn for it announces a turn that will never come, which
      // is the same wrong claim as suppressing a turn that does.
      const parentReachable = Boolean(orig.process && orig.process.stdin && orig.process.stdin.writable);
      const restoredWillSpeak = !delegateEntry.isPlatformDelegate
      ...
      if (restoredWillSpeak && !bufferedFollowUpTakesOver(convoId, orig, 'specialist-return auto-continue')) {
        const resumeCount = incrementAutoResume(convoId);
        if (resumeCount >= MAX_CONSECUTIVE_AGENT_RESUMES) {
