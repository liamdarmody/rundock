...
function handleDelegation(msg, processes) {
  ...
  // NOT FOR CODEX. Its prompt assembly is separate and a resumed thread already
  // carries the history, so a delta there would duplicate what the runtime
  // provides. Gated here rather than at the injection point, because
  // contextWithHistory feeds both runtimes.
  ...
  const noteDelegateSend = () => console.log(
    ...
    + deltaNote(missed, Boolean(priorSessionId)));
  ...
  //
  // Only for a resumed delegate. A cold spawn already receives the full
  // transcript above, and a first-time delegate has missed nothing.
