    ...
    for (const line of lines) {
      ...
      try {
        ...
        if (enableInterception && messageEnded && entry.pendingAgentTools && entry.pendingAgentTools.length) {
          ...
          if (!target && unnamed.length) {
            const said = unnamed
              .map(input => (input.description || '').slice(0, 80) || 'nothing named')
              .join('; ');
            ...
            recordEvent('delegation_error', { conv: convoId, agent: entry.agentId, d: { reason: 'no_target_matched' } });
          }
