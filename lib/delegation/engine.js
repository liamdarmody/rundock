...
function handleDelegation(msg, processes) {
  ...
  const finishDelegateClose = (code) => {
    ...
    if (delegateEntry.isIntercepted) {
      ...
      // decision to WAKE the parent read one expression instead of two that
      // have to be kept in agreement.
      if (restoredWillSpeak && !bufferedFollowUpTakesOver(convoId, orig, 'specialist-return auto-continue') && orig.process && orig.process.stdin && orig.process.stdin.writable) {
        const resumeCount = incrementAutoResume(convoId);
        if (resumeCount >= MAX_CONSECUTIVE_AGENT_RESUMES) {
