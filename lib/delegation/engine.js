      ...
      try {
        ...
        if (enableInterception && messageEnded && entry.pendingAgentTools && entry.pendingAgentTools.length) {
          ...
          for (const input of missedTargets) {
            ...
            // The event carries the FACT, the log line carries the WORDS, and
            // they are split deliberately. The signal layer's contract is that
            // events carry structure and never content: no message text, no
            // prompt text, no tool arguments. `said` is tool-call content, so
            // putting it in the payload would breach that contract at one call
            // site and quietly make the whole layer untrue. The event makes the
            // miss countable and findable; the log line beside it says which
            // handover a person was told about and never got.
            recordEvent('delegation_error', {
              conv: convoId, agent: entry.agentId,
              d: { reason: 'no_target_matched' },
            });
          }
