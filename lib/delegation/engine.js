    ...
    for (const line of lines) {
      ...
      try {
        ...
        if (enableInterception && messageEnded && entry.pendingAgentTools && entry.pendingAgentTools.length) {
          ...
          // stay worth reading.
          const unnamed = agentCalls.filter(input => !input.subagent_type);
          if (!target && unnamed.length && hasDirectReports(entry.agentId)) {
            const said = unnamed
              .map(input => (input.description || input.prompt || '').slice(0, 120).replace(/\s+/g, ' ').trim() || 'nothing named')
              .join('; ');
            ...
            recordEvent('delegation_error', {
              ...
              d: { reason: 'no_target_matched', asked: said, calls: unnamed.length },
            });
          }
