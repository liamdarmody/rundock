    ...
    for (const line of lines) {
      ...
      try {
        ...
        if (enableInterception && messageEnded && entry.pendingAgentTools && entry.pendingAgentTools.length) {
          ...
          const missedTargets = hasDirectReports(entry.agentId)
            ? agentCalls.filter(input => !input.subagent_type && !findDirectReportMatch(entry.agentId, input))
            : [];
          for (const input of missedTargets) {
            ...
            // What it asked for travels with the record, not only in the log.
            // A count of misses says something went wrong; the words say which
