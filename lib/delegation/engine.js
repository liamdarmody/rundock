...
function handleDelegation(msg, processes) {
  ...
  const finishDelegateClose = (code) => {
    ...
    if (delegateEntry.isIntercepted) {
      ...
      // TWO QUESTIONS, KEPT APART ON PURPOSE, because one of them has a side
      // effect and the other does not.
      //
      // `parentShouldWake` is what the specialist's marker and any follow-up
      // say about whether this parent is meant to act. It governs whether
      // bufferedFollowUpTakesOver is CONSULTED, and that call consumes and
      // replays a buffered message: folding reachability into it stopped that
      // replay happening for an unreachable parent, which silently dropped a
      // message the user had already sent. Reachability must not gate it.
      //
      // `parentReachable` is whether the parent can be written to at all.
      //
      // The arrival is drawn from BOTH, because both must hold for a turn to
      // appear, and neither can be asked twice: bufferedFollowUpTakesOver acts
      // as it answers, so the divider decision reads everything knowable
      // without side effects and the wake additionally consults the one thing
      // that cannot be asked again. Drawing when the buffered message takes
      // over is correct anyway: that message drives the parent, so a turn does
      // follow.
      ...
      const restoredWillSpeak = parentShouldWake && parentReachable;
      safeSend(JSON.stringify({
        type: 'system', subtype: 'agent_switch', _conversationId: convoId,
      ...
      if (parentShouldWake && !bufferedFollowUpTakesOver(convoId, orig, 'specialist-return auto-continue') && parentReachable) {
        const resumeCount = incrementAutoResume(convoId);
        if (resumeCount >= MAX_CONSECUTIVE_AGENT_RESUMES) {
