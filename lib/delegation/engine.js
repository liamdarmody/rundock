...
function handleDelegation(msg, processes) {
  ...
  //
  // Still not for Codex, whose prompt assembly is separate, and still not when
  // a full transcript is already going (the non-intercepted cold spawn below),
  // which would otherwise send the same conversation twice.
  const needsCatchUp = !isCodexDelegate && !needsTranscript;
