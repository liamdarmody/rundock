        ...
        if (enableInterception && messageEnded && entry.pendingAgentTools && entry.pendingAgentTools.length) {
          ...
          if (target) {
            ...
            if (ownProse) {
              ...
              if (!streamedToClient) liveHandoffText = textWithTools;
            } else if (handoffLine) {
              const toolSummary = buildToolSummary(entry.toolCalls);
              ...
              // THE SAME STRING THE TRANSCRIPT GOT, not the bare line. What is
              // shown live and what a reload shows have to be the same turn, and
              // the tool summary is part of it.
              liveHandoffText = withTools;
            } else {
