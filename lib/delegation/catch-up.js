 ...
 */
function catchUpPrefix(missed, arriving = false) {
  const text = missed && missed.text;
  if (!text) return '';
  // THE HEADING HAS TO BE TRUE. "Since your last turn" told an agent joining
  // for the first time that it had been here before, which is the same class
  // of error as telling a cold-spawned orchestrator it had missed something:
  // an agent reasoning from a false premise about its own history reasons
  // badly about everything downstream of it.
  const heading = arriving
    ? 'THE CONVERSATION SO FAR, BEFORE YOU JOINED:'
...
/**
 * The full context a delegate is sent: the brief, prefixed by what it missed.
 *
 * THE CONVERSATION ONLY ENTERS HERE THROUGH `missed`, which is the one place a
 * cap can be applied and is applied. This used to take a whole rendered
 * transcript as well, and preferred it when it was given one, so the size of
 * what a delegate received depended on which of two callers had built it. A
 * route that wants to hand an agent more than the cap allows now has nowhere
 * to put it.
 *
 * Pure, so a test can assert the delegate's own prior turn is absent, another
 * agent's intervening turn is present, and the whole thing stays inside the
 * bound whatever the conversation holds.
 */
function buildDelegateContext({ missed, brief, arriving = false }) {
  return `${catchUpPrefix(missed, arriving)}[DELEGATION BRIEF]\n${brief}`;
}
