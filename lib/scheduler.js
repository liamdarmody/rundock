...
function beginRun(agent, routine, key, run, startedAt) {
  ...
  const recordOutcome = (ok, err) => {
    ...
    // watched is not a slot served. The word is decided once, in an object
    // literal the writer vocabulary walk can read, so the two roads record
    // the same ending.
    const ending = { status: run.cancelled ? 'cancelled' : (ok ? 'completed' : 'failed') };
    if (run.trigger !== 'manual') {
      recordRoutineRun(key, {
    ...
    // state, so its word is the one decided above.
    const outcome = run.trigger === 'manual' ? ending.status : routineState[key].status;
