...
function loadRoutineState() {
  ...
  try {
    ...
    for (const [key, state] of Object.entries(saved)) {
      ...
      // two, and a run whose process really did die still meets the rewrite,
      // because nothing in a fresh process answers for it.
      //
...
// unlike 'interrupted'.
//
// THE ROUTINE STATE TAKES THE SAME WORD, which is the same ruling
// 'interrupted' got and made for the same reason: two stores describing one
...
// Ask a run to stop, once, and never let that ask reach the caller as a throw.
//
...
//
// ONCE, because signalling a tree twice on two requests is a second SIGTERM to
// pids that may by then belong to somebody else. The flag stays set either
// way, so a second request still ends up recorded as a stop.
function stopLiveRun(run) {
  ...
  try {
    ...
    if (asked && typeof asked.catch === 'function') {
      asked.catch((e) => console.error('[Scheduler] Failed to stop the run:', e && e.message ? e.message : e));
    }
  } catch (e) {
...
function beginRunRecord(agent, routine, startedAt) {
  ...
  // Registered AFTER the write, so the set never claims a run whose record was
  // never opened. Paired with the removal in endRunRecord, this is what tells
  // a record open in this process from one a dead process abandoned.
...
function executeRoutine(agent, routine, key, now) {
  ...
  try {
    ...
    // impossible; the stopper is armed into this entry when there is one.
    // Inside the same guard as the start, so the release below covers it.
    run.key = key;
    run.cancelled = false;
    run.stop = null;
...
function beginRun(agent, routine, key, run, startedAt) {
  ...
  const recordOutcome = (ok, err) => {
    ...
    const outcome = run.cancelled ? 'cancelled' : (ok ? 'completed' : 'failed');
    recordRoutineRun(key, {
  ...
  if (agent.runtime === 'codex') {
    ...
    (async () => {
      ...
      // already waiting on. Armed after the turn exists, because interrupting
      // a thread with no active turn is refused.
