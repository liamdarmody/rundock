...
function stopLiveRun(run) {
  if (!run.stop || run.stopSent) return;
  try {
...
function beginRunRecord(agent, routine, key, startedAt) {
  const run = {
    ...
    key,
    // Whether a stop has been asked for, the thing that can send one, and
    // whether one has gone out. Three separate facts, because a single field
    // cannot hold them: `stop` null would mean both "nothing has been armed
    // yet" and "the signal has already left", which are opposite instructions
    // to anything deciding whether to send. So `stop` is only ever the
    // stopper, if there is one yet, and `stopSent` is the send-once guard.
    //
