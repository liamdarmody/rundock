 ...
 * was.
 *
 * THE ANCHOR IS THE SLOT STORE'S `due`, which is the slot of the period `now`
 * falls in. The slot store leaves an already-passed slot on TODAY rather than
 * rolling it forward, because the tick's `now >= nextRun` check is what fires
 * a same-day catch-up. That rule is the reason a missed row pairs with a next
 * run today and never tomorrow: a slot missed today would be caught up within
 * a minute by an open Rundock and would render as caught up instead, so the
 * only slot that survives to render as missed is one from an earlier day, and
 * the slot ahead of it has not moved.
 *
 ...
 * and only that way. Nothing computed in this function reaches the
 * suppression.
 *
 * The walk is bounded for the same reason the slot walk is: an anchor left
 * years behind by a restored backup or a clock set wrong would otherwise cost
 * thousands of steps to produce one line of a list.
 */
...
/**
 * The three facts a routine row needs that neither the agent file nor the run
 * state carries on its own, as ISO instants.
 *
 * `lastSlot` is the slot the last run served, which is what lets a row tell a
 * punctual run from one that caught up late. `missedSlot` is the most recent
 * slot recorded as passed unserved, which is what lets a row say the machine
 * was closed. Which of the two is the LAST thing that happened is decided by
 * the view, because that is a question about words rather than about time.
 */
function routineDisplayFacts(key, schedule) {
  const parsed = parseSchedule(schedule);
  if (!parsed) return { nextRun: null, lastSlot: null, missedSlot: null };
  const stamped = routineState[key] && routineState[key].lastRun;
  const lastRun = stamped ? new Date(stamped) : null;
  const entry = routineSlots.routines[key];
...
// are separate stores by design. 'queued'
...
// nothing tracks changed files, a routine's output is discarded at the spawn
// (see the comment there, which says what has to be true before anything reads
// it), and there is no handle to cancel through. A record says only what this
// file can vouch for at the moment it writes.
//
...
module.exports = {
  ...
  nextRunFor, routineDisplayFacts,
