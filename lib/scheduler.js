...
function beginRunRecord(agent, routine, startedAt) {
  const run = {
    ...
    // THE RUN'S OWN ID IS THE SESSION IT RUNS UNDER, and that is the whole of
    // how a transcript is tied to a run. The claude spawn passes it as
    // --session-id, the agent tool names the transcript for it, and the record
    // carries it so a reader never has to know the two are the same string.
    // Nothing anywhere reaches for the newest transcript: the wrong one would
    // yield a plausible file list belonging to another run, and nothing
...
function endRunRecord(run, ok, endedAt, err) {
  ...
  //
  // The reader cannot throw and must not: this runs inside the handler that
  // ends a run and, on the failed-start path, inside the catch that keeps the
  // whole tick alive. A transcript that is missing, unreadable or in a shape
  // nothing here understands costs the file list and nothing else.
  //
  ...
  const observed = readSessionTranscript(run.sessionId);
  writeRunRecord(run.dir, {
    id: run.id,
    agent: run.agent,
    routine: run.routine,
...
function readRunProgress(runId) {
  ...
  return { status: observed.status, reason: observed.reason, activity: observed.activity };
}
