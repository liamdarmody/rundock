...
function loadRoutineState() {
  ...
  try {
    ...
    for (const [key, state] of Object.entries(saved)) {
      ...
      // It names every routine whose run THIS process started and has not yet
      // ended, it is keyed the same way this state is, and it is deliberately
      // not cleared by this load, for the reason recorded where it is defined.
      // That is the same evidence, in the same load, that the run records
      // beside it have always been closed on: whether this process opened
      // them. The two stores now answer a live run in one voice instead of
      // two, and a run whose process really did die meets the rewrite as it
      // always did, because nothing answers for it.
      //
      // THE BOUND ON THAT LAST SENTENCE, named rather than left to be found.
      // These keys are workspace-local and collide freely, so it holds
      // absolutely at boot, where nothing is in flight, and holds on a switch
      // except for one case: leaving a workspace where `agent:routine` is
      // live, into a workspace whose file holds a genuinely dead 'running'
      // leftover under that same name. That leftover is held at 'running'
      // until the other workspace's child exits, and is then overwritten by
      // that child's outcome. This is the SAME bleed the in-flight set already
      // carries and documents where it is defined, arriving through the same
      // collision rather than through a new one, and closing it properly means
      // telling runs apart by the workspace they belong to, which is the
      // cross-process and cross-workspace locking work rather than this. What
      // is not affected is the field the suppression reads: the leftover keeps
      // whatever lastRun it had on disk either way.
      //
...
function routineDisplayFacts(key, schedule) {
  const parsed = parseSchedule(schedule);
  if (!parsed) return { nextRun: null, lastStart: null, lastSlot: null, missedSlot: null };
...
// recognise, which is a fallback each of those surfaces already has and
// already tests, and giving them their own words is theirs to do rather than
// this file's.
//
...
// Runs THIS process opened and has not yet closed, by run id.
//
// WHY IT EXISTS. Startup closes every run record left open by a process that
// died mid-run, and startup is also the workspace-switch path, which happens
// while runs of this process are still going. Those records are open for a
// true reason and have to survive it. inFlight cannot answer the question: it
// is keyed by `agentId:routineName`, and this is a question about one RUN
// rather than about one routine.
//
// DELIBERATELY NOT RESET BY loadRoutineState, for the same reason inFlight is
// not. A switch drops what describes the workspace being left, and a child
// process that is still running is not that. Clearing it would let the very
// next switch close the record of a run whose child is alive, which is the one
// fault this set exists to prevent.
//
// In-process only, which is the whole of what it claims. Two copies of Rundock
// over one workspace cannot see each other's runs, so each would close the
// other's open records at startup. That is the limit inFlight already carries
// and the cross-process locking card's work rather than this one's.
const openRunRecords = new Set();

// ===== RUNS THAT CAN BE REACHED =====
// Runs this process started and has not yet ended, by run id, each holding the
// one thing that can stop it.
//
// WHY IT EXISTS. The child of a routine used to be a local of the function
// that spawned it, so the moment that function returned nothing anywhere could
// reach the run. A routine that has gone wrong is exactly the one somebody
// wants to stop, and the only remedy was quitting the application, which stops
...
// run. The single-flight hold is keyed that way because it is a question about
// a ROUTINE; this is a question about a RUN, and the id it answers to is the
// one its record is filed under, so a caller holding a record can reach the
// run it describes and needs nothing else.
//
// WHAT THE ENTRY IS. The run handle itself, the same object the record store
// opened and the ending will close, with the routine key it belongs to and a
// stopper on it. One object rather than a parallel record, so there is nothing
// for a second map to disagree with.
//
// DELIBERATELY NOT RESET BY loadRoutineState, for the same reason the
// in-flight set and the open records are not. A workspace switch drops what
// describes the workspace being left, and a child process that is still
// running is not that. Clearing this would leave a live run unreachable for
// the rest of its life, which is the state this map exists to end.
//
// In-process only, exactly as far as the two sets beside it reach: two copies
// of Rundock over one workspace cannot stop each other's runs.
...
// such hazard and stopped nothing, so the stopper is kept and the next request
// tries again. Dropping it on a failed attempt left the run going with nothing
// able to reach it and every later request answering yes, which is the shape
// this whole block exists to remove.
//
// The asynchronous stopper is held from the moment it is dispatched rather
// than from the moment it resolves, so two requests in the same breath cannot
// put two interrupts on the wire; it is re-armed only if the request comes
// back refused. Re-arming a run that has since ended reaches nothing, because
// a run is unreachable the moment its ending removes it from the map.
 ...
 */
function armRunStopper(run, stop) {
  run.stop = stop;
  if (run.cancelled) stopLiveRun(run);
}
...
function beginRunRecord(agent, routine, key, startedAt) {
  const run = {
    ...
    // The routine this run is held under, carried so the run can say which
    // single-flight key its ending releases without anything having to
    // recompute it from the two names above.
    key,
    // Whether a stop has been delivered for this run, and the thing that can
    // deliver one. Named here with every other field rather than added to the
    // handle afterwards, so a reader of this object meets it whole and no
    // caller has to know to finish building it.
    cancelled: false,
    stop: null,
  ...
  // Registered AFTER the write, so neither claims a run whose record was never
  // opened. Paired with the removals in endRunRecord, these are what tell a
  // record open in this process from one a dead process abandoned, and what
  // makes a run reachable while it lasts.
  //
  // BOTH HERE, because both describe exactly this: a run this process opened
  // and has not yet ended. They are separate stores because they answer
  // different questions, the set for records read back off disk and the map
  // for reaching a run that is still going, but an entry point that opened a
  // record and joined only one of them would leave the two disagreeing about
  // whether a run is live. There is one place to add and one place to remove.
  openRunRecords.add(run.id);
...
function endRunRecord(run, ok, endedAt, err) {
  ...
  // Set hygiene rather than a guard, and named as such: the record it points
  // at now carries an outcome, so the startup close would skip it on the
  // status alone. Without this the set grows for the life of the process, one
  // entry per run.
  openRunRecords.delete(run.id);
  // THE ONE PLACE A RUN STOPS BEING REACHABLE, paired with the one place it
  // becomes reachable. Every ending comes through here, including a start that
  // threw, so there is no ending that leaves a finished run listed as going or
  // a dead handle for a later stop to signal. Same line, same reason, as the
  // set above.
...
function closeAbandonedRunRecords() {
  try {
    ...
    for (const record of readRunRecords()) {
      ...
      if (openRunRecords.has(record.id)) continue;
...
function beginRun(agent, routine, key, run, startedAt) {
  ...
  armRunStopper(run, () => killProcessTree(proc, 'SIGTERM'));
