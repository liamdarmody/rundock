...
function beginRun(agent, routine, key, run, startedAt) {
  ...
  // kernel buffer filled. The child does not then block: its own stdout is
  // asynchronous, so the write returns, the bytes queue in its memory, and the
  // pending writes simply never complete, which means the child never exits.
  // The distinction matters if you are reasoning about sizes, because it means
  // there is no safe volume, only a volume small enough that the buffer
  // happened to take it: 128 KB of output finished in 26 ms and 160 KB never
  // finished at all.
  //
