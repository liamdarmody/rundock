...
function beginRun(agent, routine, key, run, startedAt) {
  ...
  // Node's stdio streams start paused, so the kernel buffer filled and the
  // child blocked on its next write: 128 KB of output completed in 26 ms and
  // 160 KB never finished at all. A child that never finishes never closes, so
  // no outcome was recorded, the single-flight hold below was never released,
  // and the guard refused that routine on every later tick for the life of the
  // process. Its record stayed open and read as interrupted after a restart,
  // reporting a run that was cut short when what happened is that we stopped
  // listening. Verbose output passes 160 KB on the opening enumeration of the
  // available tools alone, before the model has said anything.
  //
  // IF YOU ARE HERE TO READ A ROUTINE'S OUTPUT, this is what has to be true
  // before you put 'pipe' back. The reader is attached in the same statement
  // that opens the pipe, never a tick later, because the child can fill the
  // buffer before your listener is registered. It stays attached for the whole
  // life of the child, including on every path that ends the run early, and
  // whatever it does with the bytes it must always consume them: a filter that
  // stops reading once it has what it wants is the same deadlock wearing a
  // different hat. If any of that is more than you want to own, drain the pipe
  // with resume() and read nothing, or leave this line alone. An unread pipe
  // is not output waiting to be collected later; it is a routine that stops
  // running.
