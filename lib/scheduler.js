...
const { discoverAgents, rosterMessage, invalidateAgentCache } = require('./agents/discovery.js');
const { isRunOnSupported, hasRunnablePrompt, planApproved } = require('./agents/routines.js');
...
function loadRoutineSlots() {
  ...
  try {
    ...
    for (const [key, entry] of Object.entries(saved.routines || {})) {
      ...
      // Absent in state files written before this was recorded: left undefined
      // rather than guessed, because inventing one would either fabricate a
      // history or silently skip a catch-up that is genuinely owed.
      const since = typeof entry.since === 'string' ? entry.since : undefined;
      routineSlots.routines[key] = { due: entry.due, schedule: shape, missed, since };
    }
  } catch (e) { /* missing or unreadable file: start empty */ }
}
...
function recordPassedSlots(key, parsed, observedBefore, now, inService) {
  ...
  if (!entry) {
    ...
    // `since` is what makes "never owed" distinguishable from "owed and
    // missed" later: a slot lands on TODAY'S date, so a routine created this
    // evening for a morning time is due sixteen hours before it existed. See
    // nextRunFor. Recorded here and nowhere else, because first sight is the
    // only moment that fact is known.
    routineSlots.routines[key] = {
      due: current.toISOString(), schedule: shape, missed: [], since: now.toISOString(),
    };
    return;
  }
...
function nextRunFor(key, schedule) {
  ...
  // A SLOT THAT PASSED BEFORE THE ROUTINE EXISTED WAS NEVER OWED. The slot is
  // computed on today's date, so a routine created at 23:30 for a midnight
  // schedule is born due, sixteen hours late, and runs the moment anything
  // stops refusing it. Measured: an approval tap released exactly that run and
  // read as the button having triggered it.
  //
  // Scoped to routines with no run history, and only to slots earlier than the
  // moment the routine was first seen. A routine that was there when its slot
  // went by is still owed it, which is the catch-up a machine that slept
  // through the morning depends on, and the rule a missed row's next-run time
  // stays on today is untouched: that row existed through its slot.
  const seenEntry = routineSlots.routines[key];
  const firstSeen = seenEntry && typeof seenEntry.since === 'string' ? new Date(seenEntry.since) : null;
...
function runningRuns() {
  return [...liveRuns.values()].map(run => ({
    ...
    // Whether the tick started it or a person did, so the row can say which
    // kind of run is going without opening the record.
    trigger: run.trigger,
  }));
}

/**
 * Ask the run with this id to stop, and say whether the ask succeeded.
 *
 * WHAT THE ANSWER MEANS, exactly. True: the stop was successfully asked for,
 * either dispatched to the run's stopper, or remembered before a stopper
 * exists and honoured by construction at the arm and at the pre-turn
 * checkpoints. False: no run answers to this id, or the ask itself could not
 * be sent. A caller told true has a stop in flight; a caller told false has
 * nothing in flight and may ask again.
 *
 ...
 *
 * CANCELLED MEANS A STOP WAS DELIVERED, NOT WISHED FOR. The ending is written
 * by whichever handler the run ends in, not here, and what it writes reads
 * `run.cancelled`, which stopLiveRun sets beside stopSent at the moment of
 * dispatch and its rejection handler clears if the far end refuses. A run
 * whose stop never left, or was refused, therefore ends as whatever it really
 * was, which is the one thing the record most owes the person who pressed
 * stop and was told it did not work.
 *
 ...
 */
function beginRunRecord(agent, routine, key, startedAt, trigger) {
  const run = {
    id: randomUUID(),
    dir: runsDir(),
    agent: agent.id,
    routine: routine.name,
    // Whether the tick started this run or a person pressed Run. Read by
    // both writers below and by nothing that decides scheduling: what a
    // manual run leaves alone is decided in beginRun, off this same word.
    trigger,
    ...
    key,
    // The stop state, in two facts a single field cannot hold, plus the two
    // things that can send one and how far the sending has got.
    //
    // `cancelRequested` is a stop asked for BEFORE a stopper exists. Nothing
    // can deliver it yet, so the scheduler itself honours it: at the arm, and
    // at the two pre-turn checkpoints on the codex path. `cancelled` is a
    // stop that was DELIVERED: set beside `stopSent` at the moment of
    // dispatch, cleared with it if the far end refuses, and it is the field
    // the ending reads to record a run as stopped. They are separate fields
    // because they answer different callers: a request is what the pre-turn
    // window acts on, and a delivery is the only thing the record may claim,
    // so folding them into one word is exactly the requested-reads-as-stopped
    // falsehood this pair exists to remove.
    //
    // `stop` and `forceStop` are likewise two fields because `stop` null
    // would otherwise mean both "nothing has been armed yet" and "the signal
    // has already left", which are opposite instructions to anything deciding
    // whether to send. `stop` is the ordinary stop and `forceStop` the one
    // that cannot be refused, where the runtime has such a thing. `stopSent`
    // and `forceStopSent` are the send-once guards, and `stopSentAt` is when
    // the ordinary one went out, which is what a later request measures its
    // wait against.
    //
  ...
  writeRunRecord(run.dir, {
    ...
    // Written by the OPENING writer as well as the closing one, so a record
    // read while the run is going, or left open by a process that died, still
    // says who started it.
    trigger: run.trigger,
    status: 'running',
    startedAt: run.startedAt,
...
function readRunRecords() {
  ...
  // A RECORD FROM BEFORE RUNS COULD BE PRESSED CARRIES NO TRIGGER, and it is
  // handed over that way rather than filled in here: this reader promises
  // the id and nothing else, and the handler that serves records forwards
  // them whole so a field a later writer adds is never dropped or invented
  // on the way. Nothing but the tick could start a run when those records
  // were written, so every reader that interprets the word reads its absence
  // as scheduled (the run-detail model's triggerOf is the one on the
  // screen), and that is a fact about those records, not a default.
  return records;
}
...
// describe.
//
// PROMPT COMES LAST OF THE FAULTS, because the two above are things somebody
// chose and this is something they left out. A routine that is paused and
// promptless is reported as paused, which is the fact its owner acted on most
// recently.
//
// THE SWITCH IS REPORTED LAST OF ALL, and the ordering is load-bearing for a
// reader this function did not originally have. The word is published beside
// the roster and consumed by the routines row, whose Turn on offer promises
// that flipping the switch starts the routine running. That promise is only
// true when the switch is the ONLY thing in the way, so 'enabled' has to mean
// exactly that: reported only when nothing else would refuse the routine too.
// With the switch checked first, a routine that is off AND promptless said
// 'enabled', the one word the offer rightly ignores, and the row promised a
// run the tick would refuse the moment the switch was flipped. For the tick
// itself the order changes nothing: any word refuses, and the log line names
// whichever fact is reported.
function routineRefusal(routine) {
  ...
  // A plan nobody has approved does not run unattended. Before 'enabled' for
  // the same reason everything is: the switch must mean the switch alone, so
  // a routine that is off AND unapproved reports the approval, and turning
  // it on surfaces the one-tap step instead of a silent refusal.
  if (!planApproved(routine)) return 'approval';
  if (!routine.enabled) return 'enabled';
  return null;
}

// WHY A PRESS IS REFUSED, which is a shorter list than the tick's, and the
// difference is the design. Paused, switched off and unapproved all answer
// one question: may this routine run UNATTENDED. A press is attended, so
// none of them applies to it, and a routine is pressed precisely to find out
// whether it should be approved at all. What remains are the reasons no run
// can be made from the routine whatever consent it has: a run target this
// release cannot run, nothing to send, and a run of it already going.
//
// THE LIST IS DECLARED as well as decided, so the row can name the whole
// set and a test can say it is exactly these three. Grown or shrunk by one
// word, the declaration and the decision below stop agreeing.
const MANUAL_RUN_REFUSALS = ['runOn', 'prompt', 'running'];

function manualRunRefusal(routine, key) {
  if (!isRunOnSupported(routine.runOn)) return 'runOn';
  if (!hasRunnablePrompt(routine)) return 'prompt';
...
/**
 * Start a routine because somebody pressed Run, or say why not.
 *
 * THE SAME ENTRY THE TICK USES, with the trigger that tells the writers and
 * the state slot apart: single-flight is executeRoutine's own guard, so a
 * press during a run holds exactly as a tick during a run holds, and the run
 * it starts is recorded, observed and stoppable exactly as a tick's is. The
 * refusal is asked first so a press that cannot produce a run is answered
 * with the reason rather than with a failed start; the guard is asked again
 * by executeRoutine, and a false from it is the same answer.
 *
 * A start that throws is rethrown, as it is to the tick: the caller decides
 * what to tell whoever pressed, and nothing here writes the state the tick's
 * failure path stamps, because a pressed run never touches that state.
 *
 * @returns {{started: true, runId: string|null}|{started: false, refusal: string}}
 */
function runRoutineNow(agent, routine, key) {
  const refusal = manualRunRefusal(routine, key);
  if (refusal) return { started: false, refusal };
  if (!executeRoutine(agent, routine, key, deps.now(), 'manual')) return { started: false, refusal: 'running' };
  const live = [...liveRuns.values()].find(run => run.key === key);
  return { started: true, runId: live ? live.id : null };
}
        ...
        if (nextRun && now >= nextRun) {
          ...
          if (refusedBy) {
            if (announcedRefusals.get(key) !== refusedBy) {
              ...
              // 'approval' is a refusal WORD, not a routine field, so indexing
              // the routine by it would print "approval is undefined" and read
              // as a missing key rather than a plan awaiting its tap. Every
              // other word names a field, and the existing wording (`paused is
              // true`, `enabled is false`) is pinned by the gating suite, so
              // only this one word is said differently.
              const detail = refusedBy === 'approval'
                ? 'plan not approved'
                : `${refusedBy} is ${String(routine[refusedBy])}`;
              console.log(`[Scheduler] Not running routine: ${routine.name} (${agent.name}): ${detail}`);
            }
            continue;
          }
          ...
          try {
            if (executeRoutine(agent, routine, key, now, 'scheduled')) {
 ...
 */
function executeRoutine(agent, routine, key, now, trigger) {
  ...
  try {
    ...
    // WHO STARTED IT travels on the run from here. A run is either the tick's
    // or somebody's press, and every writer below reads the word off the run
    // rather than being told again, so the two records of one run cannot
    // disagree about it. Anything that is not a press is the tick's: the
    // word is normalised once, here, so a caller that passes nothing gets
    // the run every caller used to get.
    run = beginRunRecord(agent, routine, key, startedAt, trigger === 'manual' ? 'manual' : 'scheduled');
...
function beginRun(agent, routine, key, run, startedAt) {
  ...
  // writing anything.
  //
  // A RUN SOMEBODY PRESSED WRITES NOTHING HERE, and that is the whole of what
  // makes it safe to press. `lastRun` is the only input to double-fire
  // suppression and `nextRunFor` steps past any slot at or before it, so a
  // test run at two in the afternoon would quietly satisfy the evening's
  // slot and move when the routine next fires. A pressed run is watched, not
  // scheduled: it leaves the state the tick decides with exactly as it found
  // it, and is recorded in the run store alone, which is where its trigger
  // says what it was. The row learns it is going from the in-flight fact the
  // roster carries, not from this slot.
  ...
  const recordOutcome = (ok, err) => {
    ...
    //
    // NONE OF WHICH A PRESSED RUN DOES. Its ending is recorded in the run
    // store below and nowhere else, for the reason its start wrote nothing:
    // the state slot is what the tick decides with, and a run somebody
    // watched is not a slot served. The ending word is written twice below
    // and deliberately so: two source walks (the routines list's vocabulary
    // check and the page pin) read the writer's own literals off the
    // `status:` line of this call, so the call keeps its literals, and the
    // pressed run's copy is pinned equal to what a tick's run records.
    if (run.trigger !== 'manual') {
      recordRoutineRun(key, {
    ...
    // READ BACK RATHER THAN RECOMPUTED where there is a record to read back.
    // The recorder assigns the state it is given, so this is the word that
    // was actually recorded, and the log line and the event below cannot
    // come to say something the record does not. A pressed run recorded no
    // state, so its word is decided here, in the same terms.
    const outcome = run.trigger === 'manual'
      ? (run.cancelled ? 'cancelled' : (ok ? 'completed' : 'failed'))
      : routineState[key].status;
    ...
    recordEvent('routine_run', { agent: agent.id, runtime: agent.runtime || 'claude', d: { routine: routine.name, status: outcome, duration, trigger: run.trigger } });
    broadcastRoutineUpdate();
  };
...
function broadcastRoutineUpdate() {
  ...
  // what the scheduler is serving. Built by the one builder, so this sender
  // cannot drift from the seven others.
  //
  // DISCOVERED AFRESH, because this is the one roster that is sent to say a
  // run's state changed. The roster is cached for a couple of seconds, and a
  // run can start and end inside that window; a broadcast served from the
  // cache would then carry the in-flight fact and the state slot as they
  // were before the change it exists to announce, and the row would go on
  // saying a run is going after it has ended, or offering a run that has
  // already started, until something unrelated happened to redraw it.
  invalidateAgentCache();
  const msg = JSON.stringify(rosterMessage());
...
module.exports = {
  ...
  MANUAL_RUN_REFUSALS, manualRunRefusal, runRoutineNow,
