...
function beginRunRecord(agent, routine, key, startedAt, trigger) {
  const run = {
    ...
    // both writers and by nothing that decides scheduling.
    trigger,
  ...
  writeRunRecord(run.dir, {
    ...
    // Written by the opening writer too, so a record read mid-run, or left
    // open by a process that died, still says who started it.
    trigger: run.trigger,
    status: 'running',
    startedAt: run.startedAt,
    endedAt: null,
...
function readRunRecords() {
  ...
  // A RECORD FROM BEFORE RUNS COULD BE PRESSED CARRIES NO TRIGGER, and is
  // handed over that way: this reader promises the id and nothing else, and
  // records cross the wire whole. Every reader that interprets the word reads
  // its absence as scheduled (run-detail-model's triggerOf), because nothing
  // but the tick could start a run then.
  return records;
}
...
// WHY A PRESS IS REFUSED: a shorter list than the tick's, by design. Paused,
// switched off and unapproved all ask whether a routine may run UNATTENDED;
// a press is attended, and is how somebody decides whether to approve at
// all. What remains is what cannot produce a run whatever its consent.
// Declared as well as decided, so a test can say it is exactly these three.
...
/**
 * Start a routine because somebody pressed Run, or say why not. THE SAME
 * ENTRY THE TICK USES: single-flight is executeRoutine's own guard, and the
 * run is recorded, observed and stoppable exactly as a tick's. A start that
 * throws is rethrown, as to the tick, and writes no state.
...
function executeRoutine(agent, routine, key, now, trigger) {
  ...
  try {
    ...
    // WHO STARTED IT travels on the run: both writers read the word off it,
    // so one run's two records cannot disagree. Anything not a press is the
    // tick's, normalised once here.
...
function beginRun(agent, routine, key, run, startedAt) {
  ...
  // writing anything.
  //
  // A RUN SOMEBODY PRESSED WRITES NOTHING HERE, which is what makes it safe
  // to press: `lastRun` is the only input to double-fire suppression and
  // `nextRunFor` steps past any slot at or before it, so a test run would
  // quietly move when the routine next fires. A pressed run is recorded in
  // the run store alone; the row learns it is going from the roster's
  // in-flight fact.
  ...
  const recordOutcome = (ok, err) => {
    ...
    //
    // NONE OF WHICH A PRESSED RUN DOES, for the reason its start wrote
    // nothing. The ending word is written twice below deliberately: two
    // source walks read the writer's literals off this call's `status:`
    // line, and the pressed run's copy is pinned equal to it.
    if (run.trigger !== 'manual') {
      recordRoutineRun(key, {
    ...
    // so the log line and the event cannot say something the record does
    // not. A pressed run recorded no state, so its word is decided here.
...
function broadcastRoutineUpdate() {
  ...
  //
  // DISCOVERED AFRESH: this roster is sent to say a run's state changed, and
  // a run can start and end inside the cache's window, so a cached roster
  // would announce the state it exists to replace.
