...
const deps = {
  ...
  fileIndexInProgress: unwired('fileIndexInProgress'),
...
function handleHttpRequest(req, res) {
  if (req.url === '/' || req.url === '/index.html' || req.url.startsWith('/?') || req.url.startsWith('/index.html?')) {
    ...
    try {
      ...
      // Keyed by source AND target, not target alone: the same bare target
      // written in two different files can now resolve to two different
      // files (the one nearest each source), so the memo must not hand one
      // source the other's answer.
      const resolveOnce = (src, target) => {
        const key = JSON.stringify([src, target]);
        if (!graphMemo.resolved.has(key)) {
          graphResolutionCount += 1;
          graphMemo.resolved.set(key, resolver.findFileInTree(tree, resolver.wikilinkSearchName(target), src));
        }
        return graphMemo.resolved.get(key);
      };
      // Each node carries its modified time from a stat of the file, because
      // the tree holds names and paths only and the map ranks files by
      // recency. A stat that fails (a file gone between the tree and this
      // request) is null: not zero, which would rank it as the oldest thing
      // in the workspace, and not an omission, which would leave the client
      // guessing which of the two it was.
      const workspaceRoot = getWorkspace();
      const modifiedOf = (rel) => {
        try { return Math.round(fs.statSync(path.resolve(workspaceRoot, rel)).mtimeMs); } catch (e) { return null; }
      };
      const nodes = [];
      (function walk(items) {
        for (const item of items) {
          if (item.type === 'file') nodes.push({ path: item.path, name: item.name, modified: modifiedOf(item.path) });
          else if (item.type === 'folder' && item.children) walk(item.children);
        }
      })(tree);
      const links = engine.allLinks().map((link) => ({ ...link, resolved: resolveOnce(link.src, link.target) }));
      // THE THIRD STATE. While the server's incremental warm-up is in flight
      // the links table is still filling, so an indexed answer with few
      // links is not yet a fact about the workspace. The payload says so,
      // and a consumer draws a warming state rather than an empty one.
      const warming = !!deps.fileIndexInProgress();
      res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' });
      res.end(JSON.stringify({ indexed: true, warming, nodes, links }));
    } catch (e) {
  ...
  } else if (req.url === '/api/connectors/machine') {
    // The Connectors tab's user-global sources (piece one of the Connections
    // work): the operator's personal reach, present in every workspace on
    // this machine, as opposed to the workspace's own .mcp.json and
    // .codex/config.toml (both already reachable through /api/file, which is
    // deliberately workspace-scoped). Unlike /api/file this endpoint takes NO
    // path from the client at all: it reads exactly these two fixed
    // locations under the server's own os.homedir() and nothing else, so it
    // carries none of the traversal surface a path parameter would.
    //
    // Raw text goes back for each source, same as /api/file: parsing and
    // rendering (including the never-render-a-value rule for ~/.claude.json,
    // which can carry OAuth tokens) live entirely in the settings view's pure
    // half, the one place the connectors tab's other sources are already
    // parsed and merged.
    const home = os.homedir();
    const readOne = (fullPath) => {
      try {
        return { text: fs.readFileSync(fullPath, 'utf-8'), missing: false, error: null };
      } catch (e) {
        if (e && e.code === 'ENOENT') return { text: null, missing: true, error: null };
        return { text: null, missing: false, error: `Could not read ${fullPath}.` };
      }
    };
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({
      claudeUserGlobal: readOne(path.join(home, '.claude.json')),
      codexUserGlobal: readOne(path.join(home, '.codex', 'config.toml')),
    }));
  ...
  // until the user clicks Allow or Deny (or the 120s timeout fires).
    ...
    req.on('end', () => {
      try {
        ...
        // crossingCovered (lib/workspace/boundary.js) decides this: a
        ...
        // A write to the workspace's own answer files. Not a crossing, so it
        // has no `chosen` crossing to take a path from, and it carries its own
        // resolved path instead. Relayed under its own name for the same reason
        // the advisory list is: this object is a whitelist, and a field the
        // hook sends is dropped unless it is named.
        const answerFile = data.answer_file === true;
        const answerFilePath = answerFile ? (data.resolved_path || null) : null;
        // THE FOLDER THE BUTTON WOULD NAME, from whichever half of the request
        // knows it.
        //
        // A file tool tags its own crossing with the folder holding the file. A
        // shell command has no single crossing to tag, so the hook works one out
        // from all of them and sends it as `grant_dir` on the request instead.
        // Reading only the crossing dropped that on the floor, so the card for a
        // command arrived with no folder and drew no button, which is 68% of
        // boundary cards in real use: the offer existed and was unreachable.
        //
        // It is deliberately NOT copied onto the crossings. `crossingCovered`
        // reads those when deciding whether a STORED grant already covers a
        // request, and a shell request must never be answered that way. This
        // value draws a button; `grantable` decides what a stored grant may
        // answer. Two questions, two fields, and they stay apart.
        const boundaryGrantDir = data.boundary
          ? ((chosen && chosen.grantDir) || data.grant_dir || null)
          : null;
        ...
        deps.pendingPermissionRequests().set(requestId, {
          ...
          answerFile,
          resolvedPath: boundaryPath || answerFilePath,
        ...
        deps.safeSend(JSON.stringify({
          ...
          request: {
            ...
            ...(answerFile && !data.boundary ? { answer_file: true, resolved_path: answerFilePath, grant_dir: null } : {}),
            // A LABEL, RELAYED EXPLICITLY. This object is built from a
            // whitelist, so a field the hook sends is dropped unless it is
            // named here: the advisory list was computed, sent, and discarded,
            // and the card it was meant to word never changed. It is carried
            // under its own name and stays out of every decision path; the
            // router still decides from `crossings` and `grantable` alone.
            ...(Array.isArray(data.advisory_outside_paths) && data.advisory_outside_paths.length
              ? { advisory_outside_paths: data.advisory_outside_paths } : {})
          },
          _conversationId: convoId
        }));
