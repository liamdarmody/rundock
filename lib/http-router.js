...
function handleHttpRequest(req, res) {
  if (req.url === '/' || req.url === '/index.html' || req.url.startsWith('/?') || req.url.startsWith('/index.html?')) {
    ...
    // The workspace's links, as written, each carrying what it resolves to
    // RIGHT NOW. Resolution runs against the server's cached tree through the
    // same resolver every click goes through, so an edge here and a click in
    // a document can never name different files; nothing is recomputed from
    // disk on this path.
    //
    // No index is a statement, not an error: the search engine is absent on
    // runtimes without node:sqlite, and a consumer is told so rather than
    // handed an empty graph indistinguishable from an unlinked workspace.
    try {
      ...
      const links = engine.allLinks().map((link) => ({
        ...link,
        resolved: resolver.findFileInTree(tree, resolver.wikilinkSearchName(link.target)),
      }));
      res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' });
  ...
  // until the user clicks Allow or Deny (or the 120s timeout fires).
    ...
    req.on('end', () => {
      try {
        ...
        const uncovered = grantable ? crossings.filter(c => !boundaryGrantCovers(c.path)) : crossings;
