  ...
  // until the user clicks Allow or Deny (or the 120s timeout fires).
    ...
    req.on('end', () => {
      try {
        ...
        const crossings = Array.isArray(data.crossings) && data.crossings.length
          ? data.crossings
          : (data.resolved_path ? [{ path: data.resolved_path, grantDir: data.grant_dir || null }] : []);
        const uncovered = crossings.filter(c => !(c && c.path && boundaryGrantCovers(c.path)));
        if (data.boundary && crossings.length && uncovered.length === 0) {
        ...
        const boundaryGrantDir = data.boundary ? (chosen ? chosen.grantDir : null) : null;
