...
const { isEmptyWorkspace, detectWorkspaceMode, scaffoldDefaults, scaffoldWorkspace } = require('../../workspace/scaffold.js');
const { readState, writeState } = require('../../store/persistence.js');
const { loadRoutineState } = require('../../scheduler.js');

/**
 * Tell every connected client which workspace the scheduler is now serving.
 *
 * WHY THIS REACHES EVERY CLIENT AND NOT ONLY THE ONE THAT ASKED. There is one
 * scheduler and it serves one workspace, but there can be several windows on
 * this server: a browser on the laptop and one on the phone, which is exactly
 * the setup the always-on documentation recommends. Only the socket that asked
 * for the switch used to be told about it, so every other window went on
 * showing the workspace it opened, with a next-run time against every routine,
 * while the scheduler had moved somewhere else and none of those routines were
 * going to fire. Nothing on that screen was true and nothing on it said so.
 *
 * IT CARRIES THE PATH AND NOT THE ROSTER, deliberately. A window that did not
 * ask to move is not moved: it goes on showing the workspace its reader chose,
 * and the routines list on it now says that Rundock is serving somewhere else
 * and these routines are not running. Pushing the new workspace's roster
 * instead would swap a reader's screen for one they did not ask for.
 *
 * The value is the server's own `getWorkspace()`, which is the same string
 * discovery stamps on every routine it reads, so the two can be compared for
 * equality on the client without either side normalising anything.
 */
function announceServingWorkspace(ctx) {
...
function openWorkspace(ctx, ws, dir) {
  ...
  // Before any of the slow steps below, so a window that is not this one stops
  // promising runs at the first opportunity rather than at the last.
