...
function handleSetWorkspaceMode(ctx, ws, msg, platform = process.platform) {
  ...
  if (mode !== 'code' && mode !== 'knowledge') {
    ...
    try {
      // AF-5: reconcile the block BEFORE persisting the new mode. The old
      // order wrote state.json first, so a settings-read failure (anything
      // but ENOENT) left the mode committed while the block stayed
      // untouched. Reconciling first means the thrown error reaches the
      // catch below before writeState ever runs.
