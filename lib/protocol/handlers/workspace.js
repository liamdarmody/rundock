...
const { isEmptyWorkspace, detectWorkspaceMode, scaffoldDefaults, scaffoldWorkspace, reconcileSandboxForMode, workspaceModeFor, sandboxOwnership } = require('../../workspace/scaffold.js');
...
function openWorkspace(ctx, ws, dir) {
  ...
  ws.send(JSON.stringify({ type: 'workspace_set', path: getWorkspace(), analysis, isEmpty, workspaceMode: state.workspaceMode, setupComplete: !!state.setupComplete, scaffoldError, ...sandboxFieldsFor(getWorkspace()) }));
...
function handleCreateWorkspace(ctx, ws, msg) {
  ...
  if (!rawName) {
    ...
    try {
      ...
      ws.send(JSON.stringify({ type: 'workspace_set', path: getWorkspace(), analysis, isEmpty: true, workspaceMode: state.workspaceMode, setupComplete: false, scaffoldError, ...sandboxFieldsFor(getWorkspace()) }));
      ws.send(JSON.stringify(rosterMessage(agentList)));
...
function handleSetWorkingFolders(ctx, ws, msg, platform = process.platform) {
  ...
  try {
    ...
    ws.send(JSON.stringify({ type: 'workspace_error', message: 'Could not save the working folders: ' + e.message }));
  }
}
...
function isAllowKey(key) {
  return typeof key === 'string' && key.length <= 128 && ALLOW_KEY.test(key);
}
function handleAddToolAllow(ctx, ws, msg) {
  if (!getWorkspace()) {
    ws.send(JSON.stringify({ type: 'workspace_error', message: 'Open a workspace before allowing a tool for it.' }));
    return;
  }
  if (typeof msg.key !== 'string' || !msg.key.trim()) {
    ws.send(JSON.stringify({ type: 'workspace_error', message: 'Could not record that allow: no tool was named.' }));
    return;
  }
  if (!isAllowKey(msg.key.trim())) {
    ws.send(JSON.stringify({ type: 'workspace_error', message: 'Could not record that allow: that is not a tool name.' }));
    return;
  }
  addToolAllow(msg.key);
  sendToolAllows(ws);
}
