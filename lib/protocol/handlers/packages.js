...
function handleEvaluatePackageDecisions(ctx, ws, msg) {
  ...
  try {
    ...
    ws.send(JSON.stringify({
      ...
      // (requestId and the token, echoed below, are the rest).
      operation: 'evaluate',
...
function handleCheckExtensionUpdate(ctx, ws, msg) {
  ...
  try {
    ...
    installFail(ws, 'update-check', null, e, undefined, { name: msg.name });
  }
}

// The reply after a record changed carries the fresh roster, read by the
// same reader the host's roster comes from, so the list and the mount are
// answered from one read and the client can reconcile a live mount from
// this very reply rather than asking again.
function handleUninstallExtension(ctx, ws, msg) {
  ...
  try {
    ...
    ctx.workspace.noteExtensionRecordsChanged();
    ws.send(JSON.stringify({ type: 'extension_uninstalled', operation: 'uninstall', token: null, ...outcome, extensions: listExtensions(workspace) }));
  } catch (e) {
    installFail(ws, 'uninstall', null, e, undefined, { name: msg.name });
  }
}

// ---- The manage page: enablement on the record, and the page's one read.

const { setExtensionEnabled, listReceipts } = require('../../packages/extension-manage.js');
...
function handleSetExtensionEnabled(ctx, ws, msg) {
  ...
  try {
    ...
    ctx.workspace.noteExtensionRecordsChanged();
    ws.send(JSON.stringify({ type: 'extension_state', operation: 'set-enabled', token: null, ...outcome, extensions: listExtensions(workspace) }));
  } catch (e) {
...
function handleGetPackagesPage(ctx, ws) {
  ...
  try {
    ws.send(JSON.stringify({ type: 'packages_page', extensions: listExtensions(workspace), receipts: listReceipts(workspace) }));
  } catch (e) {
    ws.send(JSON.stringify({ type: 'packages_page_error', reason: e && e.message ? e.message : String(e) }));
  }
}
...
module.exports = {
  ...
  handleSetExtensionEnabled, handleGetPackagesPage,
  wireExtensionDeps,
};
