...
function handleListExtensions(ctx, ws) {
  ws.send(JSON.stringify({ type: 'extensions', extensions: listExtensions(getWorkspace()) }));
}

function handleGetExtensionUi(ctx, ws, msg) {
  const result = uiPayload(getWorkspace(), msg.extensionId, msg.rendererId);
  if (!result.ok) {
    ws.send(JSON.stringify({
      type: 'extension_ui_error',
      extensionId: msg.extensionId, rendererId: msg.rendererId, reason: result.reason,
    }));
    return;
  }
  ws.send(JSON.stringify({
...
module.exports = { handlePlanPackageImport, handleApplyPackageImport, handleListExtensions, handleGetExtensionUi };
