...
function beginExtensionPlan(ws, workspace, source) {
  ...
  try {
    ...
    // The workspace this was planned against travels with the offer: the
    // facts on screen, including whether this is a fresh install or a
    // replacement, are true of that workspace and no other, so confirm has
    // to be able to tell whether the world it is about to write into is
    // still the one the trust step described.
    pendingInstalls.set(token, { snapshot, plan, workspace });
...
function handlePlanExtensionUpdate(ctx, ws, msg) {
  ...
  // JSON, an unrecognised schema) or a record missing its own source field
  // must reach the client as a named refusal, the same as every sibling
  // handler answers for the same class of failure, rather than escaping the
  // dispatch as an uncaught exception.
  let source;
  try {
    ...
    // pasted url does, never straight into beginExtensionPlan and a git
    // argv.
...
function handleConfirmExtensionInstall(ctx, ws, msg) {
  ...
  try {
    // The bytes and the workspace an install lands in must be the ones the
    // trust step described. The plan's own facts (whether this replaces an
    // installed version, and which one) are true of the workspace it was
    // read against; a server that has since moved to another workspace
    // must not silently install into whatever is current now.
...
function handleCheckExtensionUpdate(ctx, ws, msg) {
  ...
  try {
    ...
    if (!record) throw new Error(`no extension named "${msg.name}" is installed`);
    if (!record.source || typeof record.source.url !== 'string') {
      throw new Error(`the installed record for "${msg.name}" carries no source url; refusing to check for updates`);
    }
    // The stored url travels with the workspace and is not trusted input,
