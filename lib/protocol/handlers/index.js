...
function buildDispatch() {
  return {
    ...
    run_routine_now: runs.handleRunRoutineNow,
    plan_package_import: packages.handlePlanPackageImport,
    evaluate_package_decisions: packages.handleEvaluatePackageDecisions,
    apply_package_import: packages.handleApplyPackageImport,
    list_extensions: packages.handleListExtensions,
    get_extension_ui: packages.handleGetExtensionUi,
    plan_package_install: packages.handlePlanPackageInstall,
    plan_extension_update: packages.handlePlanExtensionUpdate,
    confirm_extension_install: packages.handleConfirmExtensionInstall,
    confirm_package_install: packages.handleConfirmPackageInstall,
    decline_package_install: packages.handleDeclinePackageInstall,
    check_extension_update: packages.handleCheckExtensionUpdate,
    uninstall_extension: packages.handleUninstallExtension,
    set_extension_enabled: packages.handleSetExtensionEnabled,
    get_packages_page: packages.handleGetPackagesPage,
    ...
    save_connector: team.handleSaveConnector,
    delete_connector: team.handleDeleteConnector,
    ...
    // File pins: three messages, one `pins` reply carrying the workspace's
    // whole list. Stored per machine and per user in the home directory.
    get_pins: pins.handleGetPins,
    pin_file: pins.handlePinFile,
    unpin_file: pins.handleUnpinFile,
  };
}
