 ...
 *
 * WHY THIS IS NOT set_routine_paused WITH A DIFFERENT ARGUMENT. Pausing is a
 * decision the reader already took about a routine that was running, and can
 * take back. `enabled` is the answer to a question nobody had asked them: a
 * routine whose file never carried the key was written by hand before this
 * product could run one, and reads as not enabled until somebody says
 * otherwise. Folding the two into one field would make resuming a paused
 * routine and consenting to run a routine that predates the scheduler the same
 * act, and the file would then have no way to say which of the two happened.
 *
 * Everything else is deliberately the same shape as pausing, including reading
 * the field back rather than diffing bytes: unchanged content means both "it
 * already said this" and "the frontmatter could not be addressed at all", which
 * is what a checkout with Windows line endings produces and which discovery
 * hides by normalising line endings when it READS one. Announcing the second as
 * success is a routine the user believes is now running and which never fires.
 */
function handleSetRoutineEnabled(ctx, ws, msg) {
  ...
  const before = fs.readFileSync(found.filePath, 'utf-8');
  const next = updateRoutineBlock(before, found.name, { enabled }, found.occurrence);
  const written = readRoutineBlock(next, found.name, found.occurrence);
  if (!written || written.enabled !== enabled) {
    fail(`Routine "${found.name}" could not be turned ${enabled ? 'on' : 'off'}.`);
    return;
  }
  // Identical bytes now mean one thing only, because the block above says the
  // field holds what was asked for: a second press, or two clients at once.
  if (next === before) {
    announceRoutineChange(ctx, ws, {
      type: 'routine_enabled', agentId: found.target.id, name: found.name, enabled,
    });
    return;
  }
  fs.writeFileSync(found.filePath, next, 'utf-8');
  console.log(`[Routine] Turned ${enabled ? 'on' : 'off'} on ${found.target.id}: ${found.name}`);
...
module.exports = { handleGetAgents, handleGetRuntimeStatus, handleGetSkills, handleAddToTeam, handleSaveAgent, handleDeleteAgent, handleSaveSkill, handleDeleteSkill, handleSaveRoutine, handleDeleteRoutine, handleSetRoutinePaused, handleSetRoutineEnabled };
