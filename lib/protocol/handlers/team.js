...
const { appendRoutineBlock, removeRoutineBlock, updateRoutineBlock, readRoutineBlock, readRoutineBlocks , computePlanHash, normalizeRoutine, stampPendingApprovals } = require('../../agents/routines.js');
...
function handleSaveAgent(ctx, ws, msg) {
  ...
  if (!ctx.agents.validateAgentSlug(name)) {
    ...
    if (!ctx.workspace.isInsideWorkspace(filePath)) {
      ...
      fs.writeFileSync(filePath, stampPendingApprovals(msg.content), 'utf-8');
