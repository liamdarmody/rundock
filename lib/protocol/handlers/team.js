...
const { appendRoutineBlock } = require('../../agents/routines.js');
...
function handleSaveRoutine(ctx, ws, msg) {
  ...
  let next;
  try {
    next = appendRoutineBlock(fs.readFileSync(filePath, 'utf-8'), routine);
  } catch (e) {
    // The data model refused it: a name it cannot write, or a run target that
    // is reserved. Say so rather than writing something unrunnable.
  ...
  ws.send(JSON.stringify({ type: 'routine_saved', agentId: target.id, name: routine.name }));
  // Invalidate BEFORE discovering, so this first broadcast carries the routine
  // that was just written rather than a warm cache without it.
  ctx.agents.invalidateAgentCache();
  ws.send(JSON.stringify({ type: 'agents', agents: discoverAgents() }));
  ctx.agents.flagRosterRefresh();
}
...
module.exports = { handleGetAgents, handleGetRuntimeStatus, handleGetSkills, handleAddToTeam, handleSaveAgent, handleDeleteAgent, handleSaveSkill, handleDeleteSkill, handleSaveRoutine };
