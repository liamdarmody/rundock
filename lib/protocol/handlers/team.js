...
function handleSaveAgent(ctx, ws, msg) {
  ...
  if (!ctx.agents.validateAgentSlug(name)) {
    ...
    if (!ctx.workspace.isInsideWorkspace(filePath)) {
      ...
      ws.send(JSON.stringify({ type: 'agents', agents: updatedAgents, workspace: getWorkspace() }));
...
function setRoutineFlag(ctx, ws, msg, { field, replyType, verb }) {
  ...
  if (!written || written[field] !== value) {
    fail(`Routine "${found.name}" could not be ${verb(value)}.`);
    return;
  }
  const reply = { type: replyType, agentId: found.target.id, name: found.name, [field]: value };
  if (next === before) {
    announceRoutineChange(ctx, ws, reply);
    return;
  }
  fs.writeFileSync(found.filePath, next, 'utf-8');
  const said = verb(value);
