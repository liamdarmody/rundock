...
const { appendRoutineBlock, removeRoutineBlock, updateRoutineBlock, readRoutineBlock, readRoutineBlocks , computePlanHash, normalizeRoutine, stampPendingApprovals, markApprovalFeatureRan } = require('../../agents/routines.js');
...
function handleSaveAgent(ctx, ws, msg) {
  ...
  if (!ctx.agents.validateAgentSlug(name)) {
    ...
    if (!ctx.workspace.isInsideWorkspace(filePath)) {
      ...
      // step has never seen; stamp any that carry no approval record as
      // pending before the file is written, so an agent-authored routine
      // meets the one tap rather than being grandfathered by the migration.
      // Blocks that already carry a record are untouched, so an edited agent
      // never lapses an approval that stands.
      let stampedContent;
      try {
        stampedContent = stampPendingApprovals(msg.content);
      } catch (e) {
...
function routineActionRefusal(ws, msg) {
  // The optional second argument is the WORD for the refusal, where the
  // refuser has one: the run message answers with the scheduler's own reason
  // beside the sentence, so the row can act on the word and a test can name
  // it. The two writers here have sentences only, and send none.
  return (message, reason) => ws.send(JSON.stringify({
...
function handleApproveRoutinePlan(ctx, ws, msg) {
  ...
  // A tap is the feature operating here in as plain a form as there is, so
  // the workspace is marked: a routine whose approval record is later
  // stripped reads as pending rather than being grandfathered.
  markApprovalFeatureRan(getWorkspace());
  writeRoutineField(ctx, ws, found, {
...
/**
 * save_connector / delete_connector: write the one file an agent cannot.
 *
 * WHY THIS EXISTS AT ALL. Claude Code protects `.mcp.json` wherever it lives,
 * so an agent asked to add or remove a connector is refused by its own file
 * tools, in any workspace, with the user's approval already given. Measured:
 * two agents and three attempts, all refused, and the Connectors tab hands
 * adding a connector to the guide, so without this the hand-off went nowhere.
 * Rundock is not subject to that restriction, so the agent says what it wants
 * written and Rundock writes it. The same division the agent and skill markers
 * already use, for a different reason: there the marker exists so an edit
 * lands where the app reads, here because no other path exists.
 *
 * NEVER WRITTEN FROM BYTES THIS DID NOT READ. A merge built on a failed read
 * drops every server already configured, which is the whole file for anyone
 * with more than one connector. A file that is absent is an empty object and
 * merges normally; a file that exists and cannot be read, or holds something
 * this cannot parse, refuses and says so. The old add form carried this rule
 * on the client and it moves here intact.
 */
function connectorFile(ctx, ws) {
  const file = path.join(getWorkspace(), '.mcp.json');
  if (!ctx.workspace.isInsideWorkspace(file)) {
    ws.send(JSON.stringify({ type: 'connector_error', message: 'Invalid path.' }));
    return null;
  }
  if (!fs.existsSync(file)) return { file, parsed: { mcpServers: {} } };
  let raw;
  try {
    raw = fs.readFileSync(file, 'utf-8');
  } catch (e) {
    ws.send(JSON.stringify({ type: 'connector_error', message: 'The connector file could not be read, so nothing was changed. Its current contents are unknown and writing now would drop whatever it holds.' }));
    return null;
  }
  let parsed;
  try {
    ...
    ws.send(JSON.stringify({ type: 'connector_error', message: 'The connector file is not valid JSON, so nothing was changed. Fix or remove it first: writing over it now would discard whatever was meant to be there.' }));
    return null;
  }
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    ws.send(JSON.stringify({ type: 'connector_error', message: 'The connector file does not hold an object, so nothing was changed.' }));
    return null;
  }
...
// A name the file format can actually hold, and that cannot reach anywhere
// else: a key with a slash or a dot-dot in it is a path, not a server name.
function validConnectorName(name) {
  return typeof name === 'string' && /^[\w.-]{1,64}$/.test(name) && !name.includes('..');
}

function writeConnectorFile(ctx, ws, file, parsed, said, name) {
  try {
    fs.writeFileSync(file, `${JSON.stringify(parsed, null, 2)}\n`, 'utf-8');
  } catch (e) {
    ws.send(JSON.stringify({ type: 'connector_error', message: `The connector file could not be written: ${e.message}` }));
    return;
  }
  console.log(`[Connector] ${said}: ${name}`);
  // Agents read their connectors when they start, so a change is not live in
  // a conversation already running. Said here rather than left to be noticed.
  ws.send(JSON.stringify({ type: 'connector_saved', name, action: said, takesEffect: 'next agent start' }));
}

function handleSaveConnector(ctx, ws, msg) {
  if (!validConnectorName(msg.name)) {
    ws.send(JSON.stringify({ type: 'connector_error', message: 'Invalid connector name.' }));
    return;
  }
  let entry;
  try {
    entry = JSON.parse(msg.content);
  } catch (e) {
    ws.send(JSON.stringify({ type: 'connector_error', message: 'The connector entry is not valid JSON, so nothing was changed.' }));
    return;
  }
  if (!entry || typeof entry !== 'object' || Array.isArray(entry)) {
    ws.send(JSON.stringify({ type: 'connector_error', message: 'A connector entry must be an object.' }));
    return;
  }
  const found = connectorFile(ctx, ws);
  if (!found) return;
  found.parsed.mcpServers[msg.name] = entry;
  writeConnectorFile(ctx, ws, found.file, found.parsed, 'saved', msg.name);
}

function handleDeleteConnector(ctx, ws, msg) {
  if (!validConnectorName(msg.name)) {
    ws.send(JSON.stringify({ type: 'connector_error', message: 'Invalid connector name.' }));
    return;
  }
  const found = connectorFile(ctx, ws);
  if (!found) return;
  if (!Object.prototype.hasOwnProperty.call(found.parsed.mcpServers, msg.name)) {
    // Said rather than silently succeeding: an agent told "done" for a name
    // that was never there reports a removal that did not happen.
    ws.send(JSON.stringify({ type: 'connector_error', message: `No connector named "${msg.name}" is configured in this workspace, so nothing was changed.` }));
    return;
  }
  delete found.parsed.mcpServers[msg.name];
  writeConnectorFile(ctx, ws, found.file, found.parsed, 'removed', msg.name);
}
...
module.exports = {
  handleSaveConnector,
  handleDeleteConnector, handleGetAgents, handleGetRuntimeStatus, handleGetSkills, handleAddToTeam, handleSaveAgent, handleDeleteAgent, handleSaveSkill, handleDeleteSkill, handleSaveRoutine, handleDeleteRoutine, handleSetRoutinePaused, handleSetRoutineEnabled, handleSetRoutineSchedule, handleApproveRoutinePlan,
  // The row's refusal road, shared with the run message in runs.js so a
  // refused press answers on the same road a refused pause does.
  routineActionRefusal };
