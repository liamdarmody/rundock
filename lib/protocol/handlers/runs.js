...
// The sentence for each word the scheduler can refuse a press with; a word
// with no sentence here falls through to one that names the word.
 ...
 * The routine is the roster's, by the same triple every row control sends.
 * THE SCHEDULER DECIDES WHAT IS REFUSED, and it refuses only what cannot
 * produce a run; paused, switched off and unapproved are consent to run
 * UNATTENDED, which a press is not. Each refusal answers on the row's own
 * road with its reason. ALWAYS ANSWERS: a start that throws is answered
 * with the reason it gave, its hold already released and its record closed.
 */
