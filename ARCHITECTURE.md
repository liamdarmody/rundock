...
- **"An agent stays inside your workspace."** True per platform, per act, and per how the target is written, so the honest form is a table. Every cell was measured against the runtime, not reasoned from documentation. **There is one guarantee here and one best-effort check, and the difference is the whole point of the table.**
  ...
  **The agent's own folder (`~/.claude`) is graded by persistence, not by location.** A model tool reading anywhere under it raises no card, with one exception. `.credentials.json` is the **secrets registry**: it cards on any access, read or write, in both modes, and no grant, mode or setting silences it, because a credential leak cannot be undone. **This is a guarantee of the card layer, not of the operating-system layer above:** the writable-roots list in the OS-guarantee row names `~/.claude` as a whole (the runtime's own bookkeeping lives there), so the sandboxed command layer permits a write to the credential file exactly as it permits one to `hooks/`; the permission card is the layer that always refuses it, on every access, in both modes. Writing to `agents/` or `skills/` is **refused outright, not carded**: a pre-existing guard denies the write before it reaches the card, and correctly so, because Rundock reads the *workspace's* agents and skills, never the global ones, so an edit here would land somewhere the app never looks, report success, and change nothing. Writing to any other **persistence surface** (`plugins/`, `commands/`, `hooks/`, or `settings.json`) **splits by which tool is writing, and the split is the runtime's, not this product's.** A model **file-edit tool** (`Write`, `Edit`, `MultiEdit`, `NotebookEdit`) is **refused outright**: measured against the runtime, it rejects a file-edit write anywhere under `~/.claude` as a sensitive file whatever is approved here, so a card offering to allow one is a promise this product cannot keep, and approving it only to be refused underneath teaches the reader that the card means nothing. Nothing is lost by refusing first, because that write could not land either way, and what the reader wants to change almost always lives in the workspace's own `.claude`, which is the copy Rundock reads. A **shell command** reaching the same path is **not covered by that refusal and still cards**, and is approvable. The measurement behind the refusal covers the file-edit tools only, so no claim is made here about how the runtime treats a shell write, nor about whether one lands once approved: what is stated is what this product does, which is to card it rather than refuse it, and a test reads the hook's verdict for both families at one persistence-surface path so this paragraph cannot drift from what the hook does. Writing anywhere else in the folder (`projects/`, `cache/`, `paste-cache/`, `downloads/`, `tasks/`, `file-history/`, `shell-snapshots/`, `session-env/`, `history.jsonl`) is free: it is scratch and transcripts, the agent's own furniture, reached freely by the same agent in a terminal. Every one of those names lives in one registry in `scripts/permission-hook.js` (`SECRET_RELATIVE_PATHS`, `PERSISTENCE_SURFACE_DIRS`, `PERSISTENCE_SURFACE_FILES`); a name this paragraph does not carry is not one the registry enforces, a name the registry does not carry is not one this paragraph may claim, and a fail-loud test binds the two in both directions. There is no narrow remembered grant: an earlier design carded the whole folder and then carved a hole back out of it with a folder-scoped grant labelled for one use and stored for a wider one, which is exactly the mismatch a boundary drawn by persistence rather than by location no longer needs.

  **The other runtime's home (`~/.codex`) is graded on the same three tiers, by the same registry shape.** Rundock spawns Codex as well as Claude, and for a long time only one of the two homes was graded: an agent reading its own configuration was silent on Claude and raised a card on Codex, which Rundock's own Connectors tab reads and displays without asking anyone. `auth.json` is the **secrets registry** for this home, the counterpart of `.credentials.json`: it cards on any access, read or write, in both modes, and no grant, mode or named working folder silences it. That tier is not only about noise. Untiered, `~/.codex` was ordinary outside access, so its card offered a standing grant on the whole folder, and anyone who accepted it to stop being asked about configuration would have silenced every later read of the credentials sitting beside it; the secrets tier refuses a folder grant outright, which is why credentials belong in it and why naming `~/.codex` as a working folder does not exempt them either. `config.toml` is this home's `settings.json`: **free to read, cards on write**, together with `prompts/`. Everything else in the folder is scratch and is free both ways: the session databases, caches, logs, locks and `history.jsonl` are the runtime's own furniture, reached freely by the same agent in a terminal. Those names live in their own registry rows in `scripts/permission-hook.js` (`CODEX_SECRET_RELATIVE_PATHS`, `CODEX_PERSISTENCE_SURFACE_DIRS`, `CODEX_PERSISTENCE_SURFACE_FILES`), kept separate from the Claude rows rather than merged into them, because a shared list joined onto one home would make `~/.claude/auth.json` a secret and `~/.codex/auth.json` nothing at all. The same fail-loud test binds these to this paragraph in both directions.
  <!-- boundary-registry-end -->

  **A workspace can name additional folders its agents work in, and naming one never reaches the tiers above.** The folders are set in Settings under Workspace, stored with the workspace, and passed to each agent as it is spawned; a named folder is compared by the same containment rule the workspace root already uses, so everything beneath it is covered, including directories created later. Naming a folder is a statement about where a team works, not a grant: it is not remembered per card, not per agent, and it says nothing about any folder that was not named. **It stops at the runtime's own home.** A named folder that contains `~/.claude`, the home directory being the obvious one, does not make anything under `~/.claude` inside: the secrets registry still cards on every access, a persistence-surface write is still refused or carded exactly as above, and both outright refusals still apply. Only the workspace a person deliberately opened is exempt from those refusals, and a named folder is not that act, because its whole value is covering folders nobody enumerated. `namedFolderCovers` in `scripts/permission-hook.js` is the single place that rule lives, and a mutation row fails if it is removed.
