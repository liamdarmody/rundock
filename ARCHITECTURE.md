...
- **"An agent stays inside your workspace."** True per platform and per act, so the honest form is a table rather than a sentence. Every cell was measured against the runtime on 2026-08-23, not reasoned from documentation.

  | Is the target visible in the command text? | Act | macOS | Windows | Linux |
  |---|---|---|---|---|
  | Yes | write | approval card | approval card | approval card |
  | Yes | read | approval card | approval card | approval card |
  | No (computed at run time) | write | refused by the operating system | **not caught** | **not caught** |
  | No (computed at run time) | read | **not caught** | **not caught** | **not caught** |

  The two instruments behind that table are not equally strong. `scripts/permission-hook.js` classifies file tools by their path field and shell commands (`Bash`, `PowerShell`) by two signals: the runtime's `dangerouslyDisableSandbox` retry flag, which is the operating system reporting a crossing it already refused, and paths in the command text resolving outside. The second is an escalation heuristic and NOT a guarantee, which is the whole of the bottom two rows: a target a command computes cannot be read out of it. It reports EVERY distinct crossing, and `lib/http-router.js` answers from a standing folder grant only when every one is covered AND the request is grantable. **A shell request is never grantable**, because a folder grant says an agent may touch a folder while approving a shell request says a command may run, and everything in the command runs rather than only the part touching the folder. POSIX and native Windows forms are both recognised (drive letters, UNC, backslash traversal, `~`/`$HOME`/`$env:USERPROFILE`); URL tokens, the null device and the directories interpreters live in are exempt, because carding `2>/dev/null` would put a boundary card on ordinary work.

  `sandboxSettings` in `lib/workspace/scaffold.js` writes the runtime's command sandbox into the settings file the runtime is started with, which decides at syscall time and reads nothing. **It configures `filesystem.allowWrite` only, so it governs writes and not reads:** measured under the shipped block, a command could still read `~/.ssh`, `~/.gitconfig`, `~/.zshrc` and `/etc/hosts`. That is the bottom-right row and it is uncovered on every platform. The write allowlist is ADDITIVE to the runtime's own defaults, so the enforced set is the workspace, the npm cache, and what the runtime already permits (measured: its debug log folder and the temp directory); everything else in the home directory was measured as refused. The macOS column also holds only while two things are true: the installed `claude` supports the setting (Rundock pins no version, it spawns whatever `resolveClaudeBin` finds on PATH) and the workspace's `settings.local.json` does not already carry its own sandbox block, which is never overwritten. macOS only by choice: native Windows has no sandbox, and Linux has one that Rundock has not measured, so no block is written there. Enabling the sandbox also isolates the network, which is why the domain list is deliberately open: with none, every outbound host is refused for shell commands and, unlike a refused write, a refused host produces no retry. A refused write does not guarantee a card either: the retry that raises one is the agent's choice, and it may simply report the failure instead.
