...
- **"An agent stays inside your workspace."** Two instruments, and they are not equally strong. `scripts/permission-hook.js` classifies file tools by their path field and shell commands (`Bash`, `PowerShell`) by two signals: the runtime's `dangerouslyDisableSandbox` retry flag, which is the operating system reporting a crossing it already refused, and paths in the command text resolving outside, which is an escalation heuristic and NOT a guarantee, because a target a command computes rather than spells out cannot be read out of it. It reports EVERY distinct crossing, and `lib/http-router.js` answers from a standing folder grant only when every one of them is covered, so a grant on one folder cannot carry a second target in the same command. POSIX and native Windows forms are both recognised (drive letters, UNC, backslash traversal, `~`/`$HOME`/`$env:USERPROFILE`); the null device and the executable lookup directories are exempt, because carding `2>/dev/null` would put a boundary card on ordinary work. `sandboxSettings` in `lib/workspace/scaffold.js` writes the runtime's own command sandbox into the settings file the runtime is started with, which decides at syscall time and reads nothing. **That second instrument is macOS only.** Native Windows has no sandbox; Linux has one and is deliberately left off because Rundock has not measured it there, and shipping configuration on the strength of a documentation page is what this analysis had to be redone for. On both, the hook is the whole boundary. The macOS allowlist is ADDITIVE to the runtime's own defaults, so the enforced set is the workspace, the npm cache, and what the runtime already permits (measured: its debug log folder and the temp directory); the user-facing copy states that rather than claiming the rest of the machine is sealed. Enabling the sandbox also isolates the network, which is why the domain list is deliberately open: with none, every outbound host is refused for shell commands and, unlike a refused write, a refused host produces no retry. Rundock pins no runtime version; it spawns whatever `claude` resolves to on your PATH (`resolveClaudeBin`), so a version without the setting ignores the block and the hook is what remains.
