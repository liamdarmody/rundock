...
| `child-pids.json` | Running Claude Code subprocess PIDs, used to clean up zombie processes on server restart. Each record carries the command it was spawned as, so a pid the OS has since recycled onto an unrelated process is not signalled. **That check needs a process's command line, and it is not available everywhere.** Linux reads it from `/proc/<pid>/cmdline`, which spawns nothing. Every other platform shells out to `ps -p <pid> -o args=`, so on Windows, and under any sandbox that blocks spawning, there is no answer at all. **When there is no answer the record is assumed to be ours rather than discarded**, which means a recycled pid can be signalled there: an untracked child leaks forever, where a redundant `SIGTERM` costs one signal to a process that is probably ours. `commandLineCapability()` in `lib/runtime/claude.js` reports which source answered, or names both that did not. |
